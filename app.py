...
# --- CONFIGURACIÓN DE GOOGLE DRIVE (NATIVA CLOUD) ---
# Usamos el ID de la carpeta creada en "Mi Unidad"
ID_CARPETA_DRIVE = "16WY9kBUovT_uMGqHnfYFFYSFK_C9JYCW"

def subir_a_google_drive(archivo_datos, nombre_archivo):
    tmp_path = None
    try:
        # Autenticación nivel experto
        ...
        # 2. Preparar el objeto para Drive
        media = MediaFileUpload(tmp_path, mimetype='application/pdf', resumable=True)

        # 3. Metadata vinculada a la carpeta nativa
        file_metadata = {
            'name': nombre_archivo,
            'parents': [ID_CARPETA_DRIVE]
        }

        # 4. Ejecución con Bypass de Cuota (Crucial)
        # Usamos supportsAllDrives para que el archivo use el espacio del DUEÑO de la carpeta
        ...
        # Si algo falla, intentamos limpiar el temporal antes de salir
        if tmp_path and os.path.exists(tmp_path):
            os.remove(tmp_path)
        return False, str(e)
