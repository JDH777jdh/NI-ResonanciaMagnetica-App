...
def generar_pdf(datos):
    ...
    for key, value in datos.items():
        if key not in ["orden_medica", "archivos_previos"]:
...
# ---------------------------------------------------------
# PASO 1: DATOS PERSONALES Y PROCEDIMIENTO
# ---------------------------------------------------------
if st.session_state.step == 1:
    st.title("Norte Imagen - Registro de Paciente")
    ...
    if df is not None:
        ...
        with col1:
            ...
            genero_in = st.selectbox("Identidad de Género", gen_opciones, index=st.session_state.form["genero_idx"])
            
            sexo_bio_in = "Masculino"
            if genero_in == "No binario":
                bio_opciones = ["Masculino", "Femenino"]
                sexo_bio_in = st.selectbox("Sexo asignado al nacer (fines médicos)", bio_opciones, index=st.session_state.form["sexo_bio_idx"])
        ...
        if edad_act < 18:
            ...
            c1, c2 = st.columns(2)
            tutor_nom_in = c1.text_input("Nombre Representante", value=st.session_state.form["nombre_tutor"])
            tutor_rut_in = c2.text_input("RUT Representante", value=st.session_state.form["rut_tutor"])
        ...
        st.write("Cargue la Orden Médica del examen:")
        st.session_state.form["orden_medica"] = st.file_uploader("Subir Orden (PDF o Imagen)", type=["pdf", "jpg", "jpeg", "png"], accept_multiple_files=True, key="up_orden")

        if st.button("CONTINUAR"):
            st.session_state.form.update({
                "rut": formatear_rut(rut_in), "nombre": nombre_in, "genero_idx": gen_opciones.index(genero_in),
                "fecha_nac": fecha_in, "email": email_in, "nombre_tutor": tutor_nom_in,
                "rut_tutor": formatear_rut(tutor_rut_in), "esp_idx": esp_finales.index(esp_sel)
            })
            ...
            st.session_state.sexo_para_calculo = sexo_bio_in if genero_in == "No binario" else genero_in
...
# ---------------------------------------------------------
    ...
    for c, keys in zip([c_a, c_b, c_c], [["ant_ayuno", "ant_asma", "ant_diabetes", "ant_hiperten"], ["ant_hipertiroid", "ant_renal", "ant_dialisis", "ant_implantes"], ["ant_marcapaso", "ant_metformina", "ant_embarazo", "ant_lactancia"]]):
        for k in keys:
            label = k.replace("ant_", "").capitalize()
            st.session_state.form[k] = c.radio(label, opts, index=opts.index(st.session_state.form[k]), horizontal=True)
    ...
    st.session_state.form["otras_cirugias"] = st.text_area("Otras cirugías y año:", value=st.session_state.form["otras_cirugias"])

    st.markdown('<div class="section-header">Tratamientos por Cáncer:</div>', unsafe_allow_html=True)
    st.session_state.form["tipo_cancer"] = st.text_input("¿Que tipo de cancer?", value=st.session_state.form["tipo_cancer"])
    st.write("Seleccione tratamientos:")
    ...
    st.session_state.form["otro_trat_cancer"] = st.text_input("¿Se empleo algún otro tratamiento?", value=st.session_state.form["otro_trat_cancer"])
    ...
    st.session_state.form["archivos_previos"] = st.file_uploader("Subir archivos previos", type=["pdf", "jpg", "png"], accept_multiple_files=True, key="up_prev")

    # RESTAURACIÓN: FÓRMULA VFG
    if st.session_state.tiene_contraste:
        st.divider()
        st.warning("⚠️ REQUERIDO: CÁLCULO DE FUNCIÓN RENAL (Cockcroft-Gault)")
        ...
        st.session_state.form["peso"] = col_v2.number_input("Peso Actual (kg)", value=st.session_state.form["peso"])
    ...
    st.session_state.form["veracidad"] = st.radio("¿Declara que la información clínica es fidedigna?", ["SÍ", "NO"], index=None)
...
# ---------------------------------------------------------
    ...
    if st.session_state.tiene_contraste:
        ...
        <strong>POTENCIALES RIESGOS:</strong> Existe una muy baja posibilidad de reacción adversa al medio de contraste (0.07-2.4%) mayoritariamente leve (náuseas o cefaleas). Pacientes con deterioro importante de la función renal poseen riesgo de desarrollo de fibrosis nefrogénica sistémica.
        </div>
        """, unsafe_allow_html=True)
...
# ---------------------------------------------------------
    ...
    st.success(f"¡Registro exitoso para {st.session_state.form['nombre']}!")
    
    # GENERACIÓN DE PDF PARA DESCARGA
    pdf_data = generar_pdf(st.session_state.form)
    st.download_button(label="📥 Descargar Resumen en PDF", data=pdf_data, file_name=f"Registro_{st.session_state.form['rut']}.pdf", mime="application/pdf")
