...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        with c1:
            ...
            st.session_state.form["genero_idx"] = g_opts.index(st.selectbox("Identidad de Género", g_opts, index=st.session_state.form["genero_idx"]))
        with c2:
        ...
        if st.button("CONTINUAR"):
            if st.session_state.form["nombre"] and (st.session_state.form["rut"] or st.session_state.form["num_doc"]):
                ...
                st.session_state.sexo_para_calculo = g_opts[st.session_state.form["genero_idx"]]
                st.session_state.step = 2; st.rerun()
            else: st.error("Complete los datos obligatorios.")

# PÁGINA 2: CUESTIONARIO DE SEGURIDAD (REESTRUCTURADA)
    ...
    st.session_state.form["bio_implantes"] = c2.radio("Implantes metálicos / dispositivos electrónicos:", opts, index=opts.index(st.session_state.form["bio_implantes"]), horizontal=True)
    st.session_state.form["bio_detalle"] = st.text_area("Detalle de qué tipo y ubicación:", value=st.session_state.form["bio_detalle"], height=70)
    ...
    st.session_state.form["quir_cancer_detalle"] = st.text_input("Tratamiento de cáncer u otra enfermedad (Tipo y fecha):", value=st.session_state.form["quir_cancer_detalle"])
    ...
    st.session_state.form["ex_rx"] = ce1.checkbox("Radiografía", value=st.session_state.form["ex_rx"])
    st.session_state.form["ex_mg"] = ce2.checkbox("Mamografía", value=st.session_state.form["ex_mg"])
    ...
    if st.session_state.tiene_contraste:
        st.markdown('<div class="section-header">5. Cálculo de Velocidad de Filtración Glomerular (VFG)</div>', unsafe_allow_html=True)
        cv1, cv2 = st.columns(2)
        st.session_state.form["creatinina"] = cv1.number_input("Creatinina (mg/dL)", value=st.session_state.form["creatinina"], step=0.01)
        st.session_state.form["peso"] = cv2.number_input("Peso (kg)", value=st.session_state.form["peso"])
        if st.session_state.form["creatinina"] > 0:
            ...
            estilo_vfg = "vfg-critica" if vfg < 30 else ""
            st.markdown(f'<div class="vfg-box {estilo_vfg}">VFG Resultado: <h2>{vfg:.2f} ml/min</h2></div>', unsafe_allow_html=True)
            if vfg < 30: st.error("⚠️ VFG Inferior a 30 ml/min: Riesgo detectado.")
    ...
    if col_nav[1].button("SIGUIENTE"):
        ...
        else: st.error("Debe aceptar que la información es fidedigna para continuar.")
...
# PÁGINA 3: CONSENTIMIENTO INFORMADO
    ...
    gad_texto = " con medio de contraste" if st.session_state.tiene_contraste else ""
    st.subheader(f"{paciente} se realizará una {examen}{gad_texto}")

    if st.session_state.tiene_contraste:
        st.markdown(f"""
        <div class="legal-text">
        <strong>OBJETIVOS</strong><br>
        La Resonancia Magnética (RM) es una segura técnica de Diagnóstico... (Texto íntegro según instrucción)... <br><br>
        ...
        """, unsafe_allow_html=True)
        st.session_state.form["autoriza_gad"] = st.radio("Autorizo la realización del procedimiento y administración de contraste:", ["SÍ", "NO"], index=None)
    ...
    if st.button("FINALIZAR REGISTRO"):
        if st.session_state.form["autoriza_gad"] == "SÍ" and canvas_result.image_data is not None:
            ...
            else: st.error("Debe firmar para finalizar.")
        else: st.error("Debe autorizar el procedimiento.")

# PÁGINA 4: FINALIZACIÓN Y PDF
elif st.session_state.step == 4:
    mostrar_logo(); st.success("Registro Completado Exitosamente")
