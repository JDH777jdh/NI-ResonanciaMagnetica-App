...
# ---------------------------------------------------------
    ...
    with col_b:
        ...
        ant_implantes = st.radio("Implantes metálicos o braquets", ["No", "Sí"], horizontal=True)
    with col_c:
    ...
    otras_cirugias = st.text_area("¿Alguna otra cirugía? Detalle cuál o cuáles y en que año:", placeholder="Ej: Apendicectomía (2015)...")

    st.markdown('<div class="section-header">Tratamientos por cáncer:</div>', unsafe_allow_html=True)
    st.write("Seleccione alternativas:")
    ...
    otro_trat_cancer = st.text_input("Algún otro tratamiento de cáncer:", placeholder="Escriba aquí...")

    st.markdown('<div class="section-header">Posee exámenes anteriores:</div>', unsafe_allow_html=True)
    ...
    otro_examen_prev = st.text_input("Otro examen anterior:", placeholder="Escriba aquí...")
    ...
    if st.session_state.step_data["tiene_contraste"]:
        ...
        with col_v1: creatinina = st.number_input("Creatinina (mg/dL):", min_value=0.00, step=0.01, format="%.2f")
...
# ---------------------------------------------------------
    ...
    if st.session_state.step_data["tiene_contraste"]:
        ...
        autoriza_gadolinio = st.radio(
            f"(SI/NO) ¿Autorizo la realización del procedimiento anteriormente especificado y las acciones que sean necesarias en caso de surgir complicaciones durante el procedimiento? Además, doy consentimiento para que se administren medicamentos y/o infusiones que se requieran para la realización de este.",
            ["SÍ", "NO"], index=None
        )
    else:
...
# ---------------------------------------------------------
    ...
    st.success(f"La ficha de {st.session_state.step_data['nombre']} ha sido generada correctamente y enviada a revisión.")
