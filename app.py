...
def subir_a_google_drive(archivo_datos, nombre_archivo):
    try:
        ...
        # Convertimos a diccionario y limpiamos la llave privada
        creds_dict = dict(st.secrets["gcp_service_account"])
        
# LIMPIEZA DE FUERZA BRUTA PARA PEM (Solución al error InvalidByte)
        if "private_key" in info:
            pk = info["private_key"]
            # Paso 1: Quitamos espacios y comillas accidentales de los extremos
            pk = pk.strip().strip('"').strip("'")
            # Paso 2: Buscamos el inicio real de la llave (ignora cualquier carácter previo)
            if "-----BEGIN" in pk:
                pk = pk[pk.find("-----BEGIN"):]
            # Paso 3: Corregimos los saltos de línea para que Python los entienda
            pk = pk.replace("\\n", "\n")
            info["private_key"] = pk
