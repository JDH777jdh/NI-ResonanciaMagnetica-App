...
# 📥 INYECCIÓN DEL TELÉFONO EN SU PROPIA LÍNEA:
    pdf.set_font('Arial', 'B', 9)
    pdf.cell(15, 5, "Teléfono: ", 0, 0)
    pdf.set_font('Arial', '', 9)
    pdf.cell(w_col - 15, 5, safe_text(datos.get('telefono', 'N/A')), 0, 1)
...
# =============================================================================
    ...
    if ruta_firma_storage_final:
        try:
            ...
            # Verificamos si la IP ya existe en el estado, si no, la capturamos al vuelo
            if "ip_dispositivo" not in st.session_state:
                st.session_state.ip_dispositivo = "IP no detectada"
            
            # Inyectamos la variable al diccionario que SÍ se envía a Firestore
            payload_firestore["ip_paciente"] = str(st.session_state.ip_dispositivo)
