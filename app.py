...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        # CORRECCIÓN PARA EL ERROR DE TYPEERROR (SORTED)
        opciones_especialidad = sorted([str(x) for x in df['ESPECIALIDAD'].unique() if pd.notna(x)])
        esp_sel = st.selectbox("Especialidad", opciones_especialidad)
        
        opciones_procedimiento = sorted([str(x) for x in df[df['ESPECIALIDAD'] == esp_sel]['PROCEDIMIENTO A REALIZAR'].unique() if pd.notna(x)])
        pre_sel = st.selectbox("Procedimiento", opciones_procedimiento)
...
# --- PÁGINA 2: CUESTIONARIO ---
    ...
    # CUADROS DE SUBIDA DE ARCHIVOS (RESTABLECIDOS)
    ...
    st.session_state.form["bio_marcapaso"] = st.radio("¿Tiene Marcapasos?", ["No", "Sí"], horizontal=True)
...
# --- PÁGINA 4: FINAL ---
    ...
    if exito: st.success("✅ Subido a Drive")
    else: st.error(f"❌ Error: {resultado}")
    st.download_button("Descargar PDF", pdf_output, nombre_final, "application/pdf")
    if st.button("Inicio"): st.session_state.clear(); st.rerun()^