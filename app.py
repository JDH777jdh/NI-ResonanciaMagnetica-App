...
def optimizar_cedula_opencv(image_file):
    ...
    filt_img = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
    return filt_img

def extraer_rut_chileno(texto_plano):
    import re
    texto_limpio = texto_plano.replace("\n", " ")
    patron = r'\b(\d{1,2}(?:\.?\d{3}){2}-[\dkK])\b|\b(\d{7,8}-[\dkK])\b'
    ...
    if df is not None:
        ...
        with c1:
            ...
            if st.session_state.form["sin_rut"]:
                ...
                # 📷 --- NUEVA LÓGICA DE UX Y CÁMARA (PASO 3) --- 📷
                col_inputs, col_btn = st.columns([2.5, 1.5], vertical_alignment="bottom")
                with col_inputs:
                ...
                with col_btn:
                    if st.button("📷 Escanear Carnet", width="stretch", key="btn_disparar_cam"):
            ...
            if not st.session_state.form["sin_rut"] and st.session_state.mostrar_camara:
                st.markdown("---")
                st.info("💡 **Consejo:** Ubica el carnet horizontalmente y evita reflejos directos de luces.")
                        ...
                        with st.spinner("⚡ Procesando OCR..."):
                            try:
                                ...
                                if rut_encontrado:
                                    ...
                                    st.error("❌ No se detectó el RUT. Intenta alinearlo mejor y evitar reflejos.")
                            except Exception as e:
                                st.error("⚠️ Error en el motor OCR (Verifica el archivo packages.txt)")
                with c_cam2:
                    if st.button("❌ Cerrar Cámara", width="stretch"):
                        st.session_state.mostrar_camara = False
                        st.rerun()
                st.markdown("---")
                # 📷 --- FIN NUEVA LÓGICA --- 📷
