...
ID_CARPETA_DRIVE = "1pNFZVt65K9kCgMqoa5KsHRGvin8Cvg3e"
...
def subir_a_google_drive(archivo_datos, nombre_archivo):
    try:
        ...
        # 1. PRIMERO: Crear el archivo temporal con los bytes
        with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
            tmp_file.write(archivo_datos)
            tmp_path = tmp_file.name

        # 2. SEGUNDO: Preparar el objeto de medios
        media = MediaFileUpload(tmp_path, mimetype='application/pdf', resumable=True)

        # 3. TERCERO: Definir metadata con el ID de carpeta correcto
        file_metadata = {
            'name': nombre_archivo,
            'parents': [ID_CARPETA_DRIVE] # Asegúrate que ID_CARPETA_DRIVE sea "1pNFZVt65K9kCgMqoa5KsHRGvin8Cvg3e"
        }

        # 4. CUARTO: Ejecutar la subida con los permisos de Drive para evitar error de cuota
