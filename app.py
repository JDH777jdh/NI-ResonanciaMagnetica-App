...
def generar_pdf_clinico(datos):
   ...
   # 6. FUNCION RENAL
    ...
    peso_texto = f"{peso_real} kg" if (peso_real and peso_real > 0 and peso_real != 70.0) else "__________ kg"
    pdf.data_field("Peso", peso_texto)

    vfg_real = datos.get('vfg')
    if vfg_real and vfg_real > 0 and peso_texto != "__________ kg":
        if vfg_real < 30: pdf.set_text_color(255, 0, 0)
        pdf.set_font('Arial', 'B', 9) # Destacamos el resultado
    ...
    w_col = (pdf.w - 30) / 2
    pdf.set_font('Arial', 'B', 8) # Encabezados de tabla interna más pequeños
    ...
    pdf.cell(w_col, 4, safe_text("[  ] Ácido gadotérico (Clariscan)"), 0, 1)
    pdf.cell(w_col, 4, safe_text("[  ] Gadopiclenol (Elucirem)"), 0, 1)
    pdf.cell(w_col, 4, safe_text("[  ] Ácido gadoxético (Primovist)"), 0, 1)
