...
USAR_GOOGLE_DRIVE = False  # <--- AQUÍ LO ACTIVAS O DESACTIVAS
...
# 2. DISTRIBUIDOR DE PÁGINAS (Aquí ocurre el salto lógico)
...
# =====================================================================
# 5. AUTENTICACIÓN GOOGLE DRIVE (GATEKEEPER)
# =====================================================================
...
if USAR_GOOGLE_DRIVE: 
    ...
    if "code" in query_params and st.session_state.credentials is None:
        try:
            ...
            # 🧹 LIMPIEZA QUIRÚRGICA: Borra el código de la URL para evitar el bucle de error
            st.query_params.clear() 
        except Exception as e:
            # Si el código expiró, limpia la URL y detiene suavemente
            st.query_params.clear()
            st.error("⚠️ El código de autorización expiró o es inválido. Por favor, vuelve a conectar.")
            st.stop()

    # Si NO hay credenciales y Drive está activado, mostrar botón y detener
    if st.session_state.credentials is None:
        st.title("Norte Imagen - Conexión de Servidores")
        st.info("⚠️ Para procesar los registros y resguardar los PDFs, debes autorizar la conexión.")
        flow = Flow.from_client_config(
            {"web": {"client_id": CLIENT_ID, "client_secret": CLIENT_SECRET, "auth_uri": "https://accounts.google.com/o/oauth2/auth", "token_uri": "https://oauth2.googleapis.com/token"}},
            scopes=SCOPES,
            redirect_uri=REDIRECT_URI
...
# 3. Función modificada para subir archivos usando TUS 15GB
...
def calcular_edad_exacta(fecha_nacimiento):
    if isinstance(fecha_nacimiento, str):
        try:
            fecha_nac = datetime.strptime(fecha_nacimiento[:10], '%d/%m/%Y').date()
        except:
    ...
    return f"{diferencia.years} años, {diferencia.months} meses, {diferencia.days} días"
...
def generar_pdf_clinico(datos):
    ...
    # CORRECCIÓN QUIRÚRGICA: Leemos desde el diccionario 'datos' en lugar de 'st.session_state'
    # Esto asegura que el PDF reciba el valor correcto incluso en procesos asíncronos.
    is_contraste = datos.get('tiene_contraste', False)
    ...
    edad_formateada = calcular_edad_exacta(datos['fecha_nac'])
    ...
    # --- Integración de Condiciones o Discapacidades (Sección anterior 4) ---
    pdf.set_font('Arial', 'B', 9)
    pdf.cell(0, 5, safe_text("Condiciones o Discapacidades Especiales:"), 0, 1, 'L')
    ...
    pdf.data_field("Enfermedad Oncológica (Cáncer)", datos.get('quir_cancer', 'No')) # <--- LÍNEA NUEVA
   ...
   # 6. FUNCION RENAL
    ...
    if st.session_state.get('tiene_contraste', False):
        ...
        # Diferenciación visual en el PDF Pediátrico vs Adulto
        if calcular_edad(datos['fecha_nac']) < 18:
            talla_real = datos.get('talla', 0.0)
            pdf.data_field("Talla", f"{talla_real} cm" if talla_real > 0 else "__________ cm")
        else:
            peso_real = datos.get('peso', 0.0)
            pdf.data_field("Peso", f"{peso_real} kg" if peso_real > 0 else "__________ kg")

        vfg_real = datos.get('vfg', 0.0)
        if vfg_real > 0:
            mensaje_alerta, _, color_rgb = obtener_alerta_vfg(vfg_real, datos['fecha_nac'])
            formula_usada = datos.get('formula_vfg', 'Cálculo clínico')

            pdf.set_font('Arial', 'B', 9)
            pdf.write(5, safe_text(f"V.F.G ({formula_usada}): {vfg_real:.2f} ml/min"))
            
            pdf.set_text_color(color_rgb[0], color_rgb[1], color_rgb[2])
            pdf.set_font('Arial', 'B', 8)
            pdf.write(5, safe_text(f"  ({mensaje_alerta})\n"))
            pdf.set_text_color(0, 0, 0)
        else:
...
def obtener_ip():
    """Captura la IP de forma silenciosa para no congelar el botón CONTINUAR."""
    import requests
    try:
        # Pide la IP por debajo sin recargar la página de Streamlit
...
if st.session_state.step == 1:
    ...
    # 1. CAPTURA DE IP BLINDADA (Anti-Bucle)
    if "ip_final_detectada" not in st.session_state:
        st.session_state.ip_final_detectada = None

    if not st.session_state.ip_final_detectada:
        ip_detectada = obtener_ip()
        if ip_detectada and ip_detectada not in [0, "0.0.0.0", "Cargando..."]:
            st.session_state.ip_final_detectada = ip_detectada
            st.session_state.form["ip_dispositivo"] = ip_detectada
    
    # Si por alguna razón falló, ponemos un respaldo silencioso para no romper Firebase
    if "ip_dispositivo" not in st.session_state.form:
        st.session_state.form["ip_dispositivo"] = "IP en proceso..."

    # 2. INTERFAZ VISUAL
    mostrar_logo()
    st.title("Registro de Paciente")

# --- NUEVO: PREGUNTA DE PROCEDENCIA ---
st.session_state.form["procedencia"] = st.radio(
    "Procedencia del paciente:", 
    ["Ambulatorio", "Hospitalizado"], 
    horizontal=True,
    index=["Ambulatorio", "Hospitalizado"].index(st.session_state.form.get("procedencia", "Ambulatorio"))
)
st.markdown("---")
...
if df is not None:
    ...
    with c1:
        ...
        if st.session_state.form["sin_rut"]:
            ...
            with col_inputs:
                # 1. Definimos el callback local
                def procesar_rut_paciente():
                    entrada_raw = st.session_state["widget_rut_p_manual"]
                    st.session_state.form["rut"] = formatear_rut(entrada_raw)

                # 2. Renderizamos el campo
                rut_p = st.text_input(
                    "RUT del Paciente", 
                    value=st.session_state.form["rut"], 
                    placeholder="12.345.678-K",
                    key="widget_rut_p_manual"
                )
                
                # 3. Resguardo de sincronización
                st.session_state.form["rut"] = formatear_rut(rut_p)
                st.session_state.form["num_doc"] = ""
                
            with col_btn:
                # 🎨 CSS para agrandar SOLO la cámara sin inflar el botón
                st.markdown("""
                    <style>
                    ...
                    if st.session_state.get("archivo_procesado") != foto_capturada.name:
                        with st.spinner("⚡ Analizando documento de forma segura..."):
                            ...
                            if rut or sexo or nombre:
                                if rut: st.session_state.form["rut"] = rut
                                if nombre: st.session_state.form["nombre"] = nombre
                                if sexo: st.session_state.form["genero_biologico"] = sexo
                                if fecha:
                                    try:
    ...
    with c2:
        ...
        if edad < 18:
            edad_exacta_ui = calcular_edad_exacta(st.session_state.form["fecha_nac"])
            st.warning(f"👦 PACIENTE MENOR DE EDAD ({edad_exacta_ui})") 
            st.session_state.form["nombre_tutor"] = st.text_input("Nombre Representante Legal", value=st.session_state.form["nombre_tutor"])
            
            # --- LOGICA PARENTESCO (MANTENIDA) ---
            opts_parentesco = ["Padre", "Madre", "Abuelo/a", "Tío/a", "Otro"]
            current_parentesco = st.session_state.form.get("parentesco_tutor", "Padre")
            st.session_state.form["parentesco_tutor"] = st.selectbox("Parentesco con el paciente", opts_parentesco, index=opts_parentesco.index(current_parentesco) if current_parentesco in opts_parentesco else 0)
            ...
            if not st.session_state.form["sin_rut_tutor"] and st.session_state.get("mostrar_camara_tutor", False):
                st.markdown("---")
                st.info("💡 **Enfoque la cédula completa del representante.**")
                        ...
                        if st.session_state.get("archivo_procesado_tutor") != foto_tutor.name:
                            with st.spinner("⚡ Procesando OCR..."):
                                ...
                                if rut_tutor or nombre_tutor:
                                    if rut_tutor: st.session_state.form["rut_tutor"] = rut_tutor
                                    if nombre_tutor: st.session_state.form["nombre_tutor"] = nombre_tutor
        ...
        # --- BLOQUE REEMPLAZADO (SIN ON_CHANGE PARA EVITAR REINICIOS) ---
        if "proc_cache" not in st.session_state:
            st.session_state.proc_cache = []

        # Sincronización pasiva: Se ejecuta en cada ciclo de la página, sin disparar eventos extra
        if "widget_proc" in st.session_state:
        ...
        # INYECCIÓN DE LATERALIDAD
        dict_lateralidad = {}
        if pre_sel:
            existe_col_lat = 'REQUIERE_LATERALIDAD' in df.columns
            for proc in pre_sel:
                if existe_col_lat:
                    fila = df[df['PROCEDIMIENTO A REALIZAR'] == proc]
                    if not fila.empty:
                        requiere_lat = fila['REQUIERE_LATERALIDAD'].values[0]
                        if str(requiere_lat).strip().upper() == "SI":
                            opcion_lat = st.selectbox(f"Especifique lateralidad para: {proc}", ["Derecha", "Izquierda", "Ambas"], key=f"lat_{proc}")
                            dict_lateralidad[proc] = opcion_lat

        st.markdown('<div class="section-header">Documentación Médica</div>', unsafe_allow_html=True)
    # Tus elementos estéticos (desactivados)
        st.file_uploader("Cargue la Orden Médica...", type=["pdf", "jpg", "jpeg"], key="up_orden_p1", disabled=True)
        st.file_uploader("Cargue Exámenes Anteriores...", type=["pdf", "jpg", "jpeg"], accept_multiple_files=True, key="up_anteriores_p1", disabled=True)

        # 8 espacios de indentación
        if st.button("CONTINUAR", key="btn_continuar_p1"):
            # 12 espacios de indentación
            if not st.session_state.form.get("nombre"):
                st.error("Por favor, ingresa un nombre.")
            elif not pre_sel:
                st.warning("Por favor, selecciona al menos un procedimiento.")
            else:
                # 12 espacios (continúa el else)
                ...
                # Acciones para avanzar de página
                st.session_state.step = 2
                st.rerun()
                
                # --- CONSTRUCCIÓN DEL NOMBRE FINAL ---
                procedimientos_finales = []
                femeninas = ["RODILLA", "PIERNA", "MANO", "MUÑECA", "CADERA", "MAMA", "ORBITA", "ARTICULACION", "CLAVICULA", "EXTREMIDAD", "MANDIBULA", "PELVIS"]
                
                for proc in pre_sel:
                    if proc in dict_lateralidad:
                        lat = dict_lateralidad[proc]
                        proc_u = str(proc).upper()
                        es_fem = any(palabra in proc_u for palabra in femeninas)
                        
                        if lat == "Ambas":
                            genero_ambos = "AMBAS" if es_fem else "AMBOS"
                            if " DE " in proc_u:
                                partes = proc_u.split(" DE ", 1)
                                sustantivo = partes[1].strip()
                                if not sustantivo.endswith("S"):
                                    sustantivo += "S" if sustantivo[-1] in "AEIOU" else "ES"
                                procedimientos_finales.append(f"{partes[0]} DE {genero_ambos} {sustantivo}")
                            else:
                                procedimientos_finales.append(f"{proc_u} ({genero_ambos})")
                        else:
                            sufijo = "DERECHA" if es_fem else "DERECHO"
                            if lat == "Izquierda":
                                sufijo = "IZQUIERDA" if es_fem else "IZQUIERDO"
                            procedimientos_finales.append(f"{proc_u} {sufijo}")
                    else:
                        procedimientos_finales.append(proc)

                st.session_state.procedimiento = ", ".join(procedimientos_finales)
    ...
    st.session_state.form["bio_detalle"] = st.text_area("Detalle de que tipo, ubicación en el cuerpo y fecha de instalación:", value=st.session_state.form["bio_detalle"], height=70)
    ...
    st.markdown('<div class="section-header">Condiciones o Discapacidades</div>', unsafe_allow_html=True)
    opciones_condicion = [
    ...
    # Lógica: Mostrar cuadro de detalle solo si se ha seleccionado algo y NO es "Ninguna de las anteriores"
    if st.session_state.form["condiciones"] and "Ninguna de las anteriores" not in st.session_state.form["condiciones"]:
        st.session_state.form["otra_condicion"] = st.text_input(
            "Detalle la condición para preparar su atención:", 
            value=st.session_state.form.get("otra_condicion", "")
        )
    else:
        # Si elige "Ninguna de las anteriores" o no hay selecciones, limpiamos el campo de detalle
        st.session_state.form["otra_condicion"] = ""

    # Ajuste de indentación a nivel de tu formulario principal
    st.markdown('<div class="section-header">3. Antecedentes Quirúrgicos y/o Terapéuticos</div>', unsafe_allow_html=True)
    
    # Aseguramos que los valores existan en el session_state antes de renderizar
    if "quir_cirugia_check" not in st.session_state.form: st.session_state.form["quir_cirugia_check"] = "No"
    if "quir_cancer" not in st.session_state.form: st.session_state.form["quir_cancer"] = "No"
    
    st.session_state.form["quir_cirugia_check"] = st.radio(
        "¿Ha sido sometido a alguna cirugía o más de una?", 
        opts, 
        index=opts.index(st.session_state.form["quir_cirugia_check"]), 
        horizontal=True
    )
    
    st.session_state.form["quir_cirugia_detalle"] = st.text_area(
        "Detalle nombre de la cirugía y fecha:", 
        value=st.session_state.form["quir_cirugia_detalle"], 
        height=70
    )

    # NUEVA PREGUNTA ONCOLÓGICA (Corregida la indentación)
    st.session_state.form["quir_cancer"] = st.radio(
        "¿Actualmente cursa o ha cursado alguna enfermedad oncológica o cáncer?", 
        opts, 
        index=opts.index(st.session_state.form["quir_cancer"]), 
        horizontal=True
    )

    st.write("¿Ha tenido alguno de estos tratamientos?")
    ...
    st.session_state.form["quir_otro_trat"] = st.text_input(
        ...
        value=st.session_state.form["quir_otro_trat"]
    )

# Bloque de Exámenes Anteriores corregido con 4 espacios
    st.markdown('<div class="section-header">4. Exámenes anteriores</div>', unsafe_allow_html=True)
    st.info("Selecciona los exámenes anteriores que adjuntaste previamente o que tienes en formato físico que estén asociados a tu examen de hoy.")
    ...
    st.session_state.form["ex_tc"] = ce4.checkbox("Tomografía Computarizada (TC)", value=st.session_state.form["ex_tc"])
    st.session_state.form["ex_rm"] = ce5.checkbox("Resonancia Magnética (RM)", value=st.session_state.form["ex_rm"])
    st.session_state.form["ex_otros"] = st.text_input("Otros estudios:", value=st.session_state.form["ex_otros"])

    if st.session_state.tiene_contraste:
        st.markdown('<div class="section-header">5. Función Renal</div>', unsafe_allow_html=True)
        
        c_renal1, c_renal2 = st.columns(2)
        with c_renal1:
        ...
        with c_renal2:
            if st.session_state.edad_para_calculo < 18:
                ...
                st.session_state.form["peso"] = 0.0 # Oculto para pediátricos
            else:
                st.session_state.form["peso"] = st.number_input("Peso (kg)", value=float(st.session_state.form.get("peso", 0.0)), step=0.1)
                st.session_state.form["talla"] = 0.0 # Oculto para adultos

        # Llamada al motor universal
        vfg, formula = calcular_vfg_universal(
        ...
        st.session_state.form["formula_vfg"] = formula

        if vfg > 0:
            mensaje, color_hex, _ = obtener_alerta_vfg(vfg, st.session_state.form["fecha_nac"])
            st.markdown(f'''
                <div class="vfg-box" style="border-left: 10px solid {color_hex}; padding: 15px; border-radius: 5px;">
                    <p style="margin:0; color: {color_hex}; font-weight: bold;">{mensaje}</p>
                    <small>Fórmula: {formula}</small>
...
# --- PÁGINA 4: FINALIZACIÓN ---
    ...
    if ruta_firma_storage_final:
        try:
            # Capturamos de forma segura las variables de st.session_state.form
            ...
            # Mapeo exacto alineado con los requerimientos de admin.py
            ...
            payload_firestore.update({
                ...
                # 🔥 SOLUCIÓN CRÍTICA: Convertimos objeto date a String para Firestore
                ...
                # --- INYECCIÓN PASO B: IDENTIFICACIÓN ALTERNATIVA ---
                ...
                # --- INYECCIÓN PASO C: IDENTIFICACIÓN Y LOGICA TUTOR ---
