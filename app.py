...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        with col2:
            # CORRECCIÓN DE FORMATO DE FECHA DD/MM/YYYY
        ...
        # LÓGICA DE ORDEN DE ESPECIALIDADES (NEURO PRIMERO)
        esp_raw = sorted([str(e) for e in df['ESPECIALIDAD'].unique() if pd.notna(e)])
        neuro_match = [e for e in esp_raw if "NEURO" in e.upper()]
        otros_match = [e for e in esp_raw if "NEURO" not in e.upper()]
        esp_finales = neuro_match + otros_match
        ...
        # Manejo de índice para procedimiento para evitar errores al cambiar especialidad
