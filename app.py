...
def generar_pdf_clinico(datos):
    pdf = PDF()
    pdf.add_page()
    pdf.data_field("Edad", f"{datos.get('edad', 'No especificada')}")
    
    # --- PÁGINA 1: ENCABEZADO Y FECHA ---
    # La fecha a la derecha (coincidiendo con el registro)
    fecha_actual = datetime.now().strftime("%d/%m/%Y")
    pdf.set_font('Arial', 'B', 10)
    pdf.cell(0, 5, f"Fecha del Examen: {fecha_actual}", 0, 1, 'R')
    pdf.ln(2)
    ...
    pdf.data_field("Edad", f"{datos.get('edad', 'N/A')} años")
