...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        # Especialidad se mantiene en la columna 1 (ce1)
        ...
        # Filtrado de procedimientos según la especialidad seleccionada
        filtered = df[df['ESPECIALIDAD'] == esp_sel]
        list_pre = sorted(filtered['PROCEDIMIENTO A REALIZAR'].dropna().unique().tolist())
        
        # REEMPLAZO: Se quita 'ce2.selectbox' y se define 'st.radio' a lo ancho del contenedor
        pre_sel = st.radio("Seleccione el Procedimiento:", list_pre)
