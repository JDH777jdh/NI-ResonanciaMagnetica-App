...
# --- PÁGINA 4: FINALIZACIÓN ---
    ...
    # ─── BLOQUE INFORMATIVO MEDIANTE COMPONENTES NATIVOS INMUNES A ERRORES ───
    st.warning("📋 **Cuestionario en Custodia Segura**")
    st.info(
        "Sus antecedentes y firma digital han sido respaldados en la base de datos central del hospital.\n\n"
        "**¿Qué viene ahora?** Por favor, tome asiento. El Tecnólogo Médico revisará y validará "
        "internamente esta encuesta en su estación de trabajo antes de llamarlo a su examen."
    )
    
    # Mostrar el ID único de atención transitoria generado en la Página 3 si existe
    if st.session_state.get("ultimo_documento_id"):
        st.code(f"🔑 ID de Operación Firebase: {st.session_state['ultimo_documento_id']}", language="text")
