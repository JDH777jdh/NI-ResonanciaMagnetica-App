...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 5, 'RESONANCIA MAGNÉTICA', 0, 1, 'C')
        self.ln(10)
    ...
    def section_title(self, num, label):
        ...
        self.set_text_color(128, 0, 32)
        self.cell(0, 7, f"{num}. {label}", 0, 1, 'L', 1)
        self.ln(2)
...
def generar_pdf_clinico(datos):
    ...
    pdf.data_field("Medio de contraste", "SÍ" if st.session_state.tiene_contraste else "NO")
    ...
    pdf.data_field("Implantes/Prótesis metálicas", datos['bio_implantes'])
    ...
    clin_txt = (f"Ayuno: {datos['clin_ayuno']} | Asma: {datos['clin_asma']} | HTA: {datos['clin_hiperten']} | "
                ...
                f"Alérgico: {datos['clin_alergico']} | Insuf. Renal: {datos['clin_renal']} | "
                f"Diálisis: {datos['clin_dialisis']} | Embarazo: {datos['clin_embarazo']} | Lactancia: {datos['clin_lactancia']}")
    pdf.set_font('Arial', '', 9)
    pdf.multi_cell(0, 5, clin_txt)
    pdf.ln(4)

    # 4. Quirúrgicos, Terapéuticos y Renal
    pdf.section_title("4", "ANTECEDENTES QUIRÚRGICOS, TERAPÉUTICOS Y FUNCIÓN RENAL")
    pdf.data_field("Cirugías", datos['quir_cirugia_check'])
    pdf.data_field("Detalle cirugías", datos['quir_cirugia_detalle'] if datos['quir_cirugia_detalle'] else "")
    ...
    pdf.section_title("5", "EXÁMENES ANTERIORES")
    exs = []
    ...
    pdf.data_field("Exámenes realizados", ", ".join(exs) if exs else "Ninguno")
    pdf.data_field("Otros exámenes anteriores", datos['ex_otros'] if datos['ex_otros'] else "")

    # PÁGINA 2: INFORMACIÓN PARA EL PACIENTE Y FIRMA
    pdf.add_page()
    ...
    pdf.set_font('Arial', '', 9)
    pdf.multi_cell(0, 5, "La Resonancia Magnética (RM) es una segura técnica de Diagnóstico... (texto omitido por brevedad en código base)")
    pdf.ln(3)

    pdf.set_font('Arial', 'B', 10)
    pdf.cell(0, 5, "CARACTERISTICAS", 0, 1, 'L')
    pdf.set_font('Arial', '', 9)
    pdf.multi_cell(0, 5, "La Resonancia utiliza fuertes campos magnéticos y ondas de radiofrecuencia... (texto omitido por brevedad)")
    pdf.ln(3)

    pdf.set_font('Arial', 'B', 10)
    pdf.cell(0, 5, "POTENCIALES RIESGOS", 0, 1, 'L')
    pdf.set_font('Arial', '', 9)
    pdf.multi_cell(0, 5, "Existe una muy baja posibilidad de que se presente una reacción adversa al medio de contraste... (texto omitido)")
    pdf.ln(10)

    # RECUADRO DE FIRMAS AJUSTADO
    y_pos = pdf.get_y()
    if datos['firma_img']:
        # Ajuste de posición: bajamos la firma un poco más para que no flote tanto sobre la línea
        pdf.image(datos['firma_img'], x=25, y=y_pos + 2, w=40)
    
    pdf.line(15, y_pos + 15, 85, y_pos + 15)
    pdf.set_font('Arial', 'B', 8)
    pdf.text(15, y_pos + 20, f"FIRMA PACIENTE: {datos['nombre'][:30]}")
    
    pdf.line(115, y_pos + 15, 185, y_pos + 15)
    pdf.text(115, y_pos + 20, "FIRMA PROFESIONAL RESPONSABLE")
    ...
    if df is not None:
        ...
        with c1:
            ...
            if gen_sel == "No binario":
                ...
                sexo_bio = st.selectbox("Sexo asignado al nacer", sb_opts, index=st.session_state.form["sexo_bio_idx"])
...
# --- PÁGINA 2: CUESTIONARIO ---
    ...
    st.session_state.form["bio_implantes"] = st.radio("Implantes metálicos, quirúrgicos o dispositivos electrónicos:", opts, index=opts.index(st.session_state.form["bio_implantes"]), horizontal=True)
    st.session_state.form["bio_detalle"] = st.text_area("Detalle ubicación:", value=st.session_state.form["bio_detalle"], height=70)

    st.markdown('<div class="section-header">2. Antecedentes Clínicos</div>', unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    k1 = [("clin_ayuno", "Ayuno"), ("clin_asma", "Asma"), ("clin_hiperten", "HTA")]
    k2 = [("clin_diabetes", "Diabetes"), ("clin_alergico", "Alérgico"), ("clin_metformina", "Metformina")]
    k3 = [("clin_renal", "Falla Renal"), ("clin_embarazo", "Embarazo"), ("clin_lactancia", "Lactancia")]
    ...
    st.markdown('<div class="section-header">3. Exámenes Anteriores</div>', unsafe_allow_html=True)
    st.file_uploader("Suba sus exámenes anteriores (PDF/JPG, Máx 4)", type=["pdf", "jpg", "jpeg"], accept_multiple_files=True, key="ex_anteriores")

    if st.session_state.tiene_contraste:
        st.markdown('<div class="section-header">4. Función Renal (VFG)</div>', unsafe_allow_html=True)
...
# --- PÁGINA 3: INFORMACIÓN OBLIGATORIA (PASO INTERMEDIO SOLICITADO) ---
elif st.session_state.step == 3:
    mostrar_logo(); st.title("Información Importante")
    st.markdown("""<div class="legal-text">
    <h3>OBJETIVOS</h3>
    La Resonancia Magnética (RM) es una segura técnica de Diagnóstico, que permite la adquisición de imágenes de gran sensibilidad en todos los planos del espacio de las estructuras del cuerpo. Tiene como objetivo obtener Información, datos funcionales y morfológicos para detectar precozmente una enfermedad.
    
    <h3>CARACTERÍSTICAS</h3>
    La Resonancia utiliza fuertes campos magnéticos y ondas de radiofrecuencia, por lo que es muy importante dejar fuera de la sala absolutamente todo lo que lleve consigo de tipo metálico y/o electrónico. Es fundamental permanecer quieto durante el estudio y seguir las instrucciones.
    
    <h3>POTENCIALES RIESGOS</h3>
    Existe una muy baja posibilidad de que se presente una reacción adversa al medio de contraste (0.07-2.4%). Pacientes con deterioro renal poseen riesgo de fibrosis nefrogénica sistémica.
    </div>""", unsafe_allow_html=True)
    
    col_nav = st.columns(2)
    if col_nav[0].button("ATRÁS"): st.session_state.step = 2; st.rerun()
    if col_nav[1].button("HE LEÍDO Y DESEO CONTINUAR"): st.session_state.step = 4; st.rerun()

# --- PÁGINA 4: CONSENTIMIENTO Y FIRMA ---
elif st.session_state.step == 4:
    mostrar_logo(); st.title("Consentimiento Informado")
    st.markdown('<div class="legal-text">Autorizo el procedimiento y las acciones necesarias en caso de complicaciones. Doy consentimiento para administrar medicamentos si se requiere.</div>', unsafe_allow_html=True)
    ...
    c_nav = st.columns(2)
    if c_nav[0].button("ATRÁS"): st.session_state.step = 3; st.rerun()
    if c_nav[1].button("FINALIZAR REGISTRO"):
        if st.session_state.form["autoriza_gad"] == "SÍ" and np.any(canvas_result.image_data[:, :, 3] > 0):
            ...
            st.session_state.step = 5; st.balloons(); st.rerun()
        else: st.error("Debe firmar y autorizar.")

# --- PÁGINA 5: FINALIZACIÓN ---
elif st.session_state.step == 5:
    mostrar_logo(); st.success("Registro Completado")
    pdf_bytes = generar_pdf_clinico(st.session_state.form)
    st.download_button(label="📥 Descargar PDF de Registro", data=pdf_bytes, file_name=f"Registro_{st.session_state.form['nombre']}.pdf", mime="application/pdf")
