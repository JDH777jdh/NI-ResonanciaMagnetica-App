...
def procesar_cedula_inteligente(image_file):
    try:
        # 1. Leer imagen a RAM (Seguridad: Nunca toca disco duro, autodestrucción post-análisis)
        file_bytes = np.asarray(bytearray(image_file.read()), dtype=np.uint8)
        img = cv2.imdecode(file_bytes, cv2.IMREAD_COLOR)

        # 2. Estandarización de tamaño (Evita colapsos de RAM en celulares de 48MP)
        h, w = img.shape[:2]
        if w > 1200:
            escala = 1200 / w
            img = cv2.resize(img, (1200, int(h * escala)))

        # 3. Preprocesamiento Óptimo sin "Recortes Mortales"
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        # Filtro adaptativo: Lidia perfecto con brillos de luz de focos o ventanas
        ...
        # 5. Extracción Inmune de RUT
        rut_detectado = None
        patron_rut = r'\b(\d{1,2}(?:\.?\d{3}){2}-[\dkK])\b'
        match_rut = re.search(patron_rut, texto_extraido)
        if match_rut:
            rut_detectado = match_rut.group(0)
        else:
            # Plan B: Si la luz borró un punto, busca números crudos
            match_bruto = re.search(r'(\d{7,8})\s*[-_]?\s*([0-9Kk])', texto_extraido)
            if match_bruto:
                rut_detectado = f"{match_bruto.group(1)}-{match_bruto.group(2).upper()}"

        # 6. Extracción de Sexo Registral
        sexo_biologico = None
