...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        # 1. CAMBIO AQUÍ: multiselect para soportar uno o varios exámenes desplegables
        pre_sel = ce2.multiselect("Procedimiento(s) a realizar", list_pre)
        ...
        # 2. CAMBIO AQUÍ: Nueva lógica adaptada para evaluar múltiples exámenes a la vez
        if st.button("CONTINUAR"):
            if st.session_state.form["nombre"] and pre_sel:
                # Buscamos todos los procedimientos seleccionados en el CSV
                rows = df[(df['ESPECIALIDAD'] == esp_sel) & (df['PROCEDIMIENTO A REALIZAR'].isin(pre_sel))]
                
                # Si AL MENOS UNO de los exámenes requiere contraste, activamos el cuestionario renal
                st.session_state.tiene_contraste = any(str(val).upper() == "SI" for val in rows['MEDIO DE CONTRASTE'].values)
                
                # Guardamos los exámenes como un string separado por comas para que viaje al PDF de forma segura
