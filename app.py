...
def optimizar_cedula_opencv(image_file):
    ...
    h, w = img.shape[:2]
    max_w = 800 # Achicamos la imagen para que el OCR no se pierda en el ruido HD
    if w > max_w:
        img = cv2.resize(img, (max_w, int(h * (max_w / w))), interpolation=cv2.INTER_AREA)
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    
    # Nuevo filtro: Difuminado gaussiano para matar el patrón celeste, luego binarización Otsu
...
def extraer_rut_chileno(texto_plano):
    import re
    # Extracción Agresiva: Eliminamos espacios, puntos y guiones bajos que lee por error el holograma
    texto_limpio = texto_plano.upper().replace(".", "").replace(" ", "").replace("_", "-")
    # Buscamos literalmente cualquier bloque de 7 u 8 números pegados a un dígito o letra K
    patron = r'(\d{7,8})-?([0-9K])'
    match = re.search(patron, texto_limpio)
    if match:
        return f"{match.group(1)}-{match.group(2)}" # Retorna estructurado: 12345678-9
    return None
...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        with c1:
            ...
            # Lógica Asíncrona: Se ejecuta solo si la cámara está activa
            if not st.session_state.form["sin_rut"] and st.session_state.mostrar_camara:
                st.markdown("---")
                st.info("💡 **Alinee la cédula** a lo ancho de la cámara y evite que la luz del techo rebote en el plástico.")
                ...
                with c_cam1:
                    if foto_capturada is not None:
                        with st.spinner("⚡ Procesando OCR avanzado..."):
                            try:
                                ...
                                if rut_encontrado:
                                    ...
                                    st.success(f"✅ RUT Inyectado: {rut_encontrado}")
                                    st.session_state.mostrar_camara = False
                                    st.rerun()
                                else:
                                    st.error("❌ No se detectó el RUT. Intenta alinearlo mejor y tapar los reflejos.")
                            except Exception as e:
                                st.error("⚠️ Error en el motor OCR (Verifica el archivo packages.txt)")
                with c_cam2:
