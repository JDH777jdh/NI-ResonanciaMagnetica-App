...
def optimizar_cedula_opencv(image_file):
    import cv2
    import numpy as np
    file_bytes = np.asarray(bytearray(image_file.read()), dtype=np.uint8)
    img = cv2.imdecode(file_bytes, cv2.IMREAD_COLOR)
    
    # Blindaje: Recortar la parte derecha de la cédula, donde está el RUT
    h, w = img.shape[:2]
    # Recortamos el tercio derecho de la imagen (donde suele estar el RUT)
    img = img[int(h*0.4):int(h*0.9), int(w*0.5):w] 
    
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # Filtro de contraste extremo para resaltar solo los números negros sobre fondo gris
    clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8,8))
    gray = clahe.apply(gray)
    _, thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    return thresh

def extraer_rut_chileno(texto_plano):
    import re
    # Buscamos números seguidos de guion y dígito verificador, ignorando todo lo demás
    matches = re.findall(r'(\d{7,8})-?([0-9Kk])', texto_plano.replace(" ", ""))
    for num, dv in matches:
...
if not st.session_state.form["sin_rut"] and st.session_state.mostrar_camara:
    st.markdown("---")
    st.info("💡 **Tip:** Si la cámara no lee bien, usa 'Subir archivo' desde tu celular.")
    
    # Opción: Cámara o Subir archivo
    opcion_carga = st.radio("¿Cómo quieres proporcionar el documento?", ["📷 Cámara", "📁 Subir Archivo"])
    
    foto_capturada = None
    if opcion_carga == "📷 Cámara":
        foto_capturada = st.camera_input("Enfoque el frente de su cédula", key="lector_cam_webrtc")
    else:
        foto_capturada = st.file_uploader("Sube una foto clara de tu cédula", type=["jpg", "png", "jpeg"])
    ...
    with c_cam1:
        if foto_capturada is not None:
            with st.spinner("⚡ Analizando documento..."):
                try:
                    imagen_preparada = optimizar_cedula_opencv(foto_capturada)
                    import pytesseract
                    texto_leido = pytesseract.image_to_string(imagen_preparada, lang='spa')
                    rut_encontrado = extraer_rut_chileno(texto_leido)
                    
                    if rut_encontrado:
                        st.session_state.form["rut"] = rut_encontrado
                        st.success(f"✅ RUT Detectado: {rut_encontrado}")
                        st.session_state.mostrar_camara = False
                        st.rerun()
                    else:
                        st.error("❌ No se pudo extraer el RUT.")
                        if st.button("📝 Ingresar RUT manualmente"):
                            st.session_state.mostrar_camara = False
                            st.session_state.ingreso_manual = True
                            st.rerun()
                except Exception as e:
                    st.error("⚠️ Error técnico, por favor ingresa el RUT manualmente.")
    
    with c_cam2:
        if st.button("❌ Cerrar Cámara", key="btn_close_cam", use_container_width=True):
            st.session_state.mostrar_camara = False
            st.rerun()

# --- BLOQUE DE INGRESO MANUAL (SI OCR FALLA) ---
if st.session_state.get("ingreso_manual", False):
    st.warning("⚠️ **Ingreso Manual de Datos**")
    st.session_state.form["rut"] = st.text_input("RUT del Paciente", value=st.session_state.form["rut"])
    st.session_state.form["nombre_completo"] = st.text_input("Nombre Completo", value=st.session_state.form.get("nombre_completo", ""))
    # ... agregas aquí los demás campos que necesites ...
    if st.button("✅ Confirmar y Continuar"):
        ...
        if edad < 18:
            ...
            if not st.session_state.form["sin_rut_tutor"] and st.session_state.get("mostrar_camara_tutor", False):
                st.markdown("---")
                st.info("💡 **Alinee la cédula** a lo ancho de la cámara y evite reflejos.")
                
                foto_tutor = st.camera_input("Enfoque el frente de su cédula (Tutor)", key="cam_webrtc_tut")
                ...
                with c_cam_t1:
                    if foto_tutor is not None:
                        with st.spinner("⚡ Procesando OCR..."):
                            try:
                                imagen_prep_tutor = optimizar_cedula_opencv(foto_tutor)
                                import pytesseract
                                texto_leido_t = pytesseract.image_to_string(imagen_prep_tutor, lang='spa')
                                rut_enc_tutor = extraer_rut_chileno(texto_leido_t)
                                
                                if rut_enc_tutor:
                                    st.session_state.form["rut_tutor"] = rut_enc_tutor
                                    st.success(f"✅ RUT Tutor detectado: {rut_enc_tutor}")
                                    st.session_state.mostrar_camara_tutor = False
                                    st.rerun()
                                else:
                                    st.error("❌ No se detectó el RUT. Intenta alinearlo mejor.")
                            except Exception:
                                st.error("⚠️ Error en el motor OCR.")
                with c_cam_t2:
                    if st.button("❌ Cerrar Cámara", key="btn_close_tut", use_container_width=True):
                        st.session_state.mostrar_camara_tutor = False
                        st.rerun()
                st.markdown("---")
...
# --- PÁGINA 3: INFORMACIÓN Y FIRMA ---
    ...
    # =====================================================================
    # ✅ NUEVO: CHECKBOX DE VERACIDAD LEGAL (NORTE IMAGEN)
    # =====================================================================
