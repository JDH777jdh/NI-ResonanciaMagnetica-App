...
def generar_pdf_clinico(datos):
    ...
    # Definimos el ancho de media página (ajustado al margen)
    w_half = (pdf.w - 25) / 2

    # --- FILA 1: Nombre Completo (Ocupa toda la línea por extensión) ---
    pdf.data_field("Nombre Completo", datos['nombre'])
    
    # --- FILA 2: RUT y Edad ---
    pos_y = pdf.get_y()
    pdf.data_field("RUT", doc_id, w=w_half, ln=0)
    pdf.set_x(pdf.get_x() + 5) # Pequeño espacio entre columnas
    pdf.data_field("Edad", f"{edad_calculada} años", w=w_half, ln=1)

    # --- FILA 3: Fecha de Nacimiento y Email ---
    pdf.data_field("Fecha de Nacimiento", datos['fecha_nac'].strftime("%d/%m/%Y"), w=w_half, ln=0)
    pdf.set_x(pdf.get_x() + 5)
    pdf.data_field("Email", datos['email'], w=w_half, ln=1)

    # --- FILA 4: Procedimiento y Contraste ---
    pdf.data_field("Procedimiento", st.session_state.procedimiento, w=w_half, ln=0)
    pdf.set_x(pdf.get_x() + 5)
    pdf.data_field("Medio de contraste", "SI" if st.session_state.tiene_contraste else "NO", w=w_half, ln=1)

    # --- FILA 5: Tutor y RUT Tutor (Solo si aplica, si no dice N/A) ---
    if datos['nombre_tutor'] or datos['rut_tutor']:
        pdf.data_field("Representante legal", datos['nombre_tutor'] if datos['nombre_tutor'] else "N/A", w=w_half, ln=0)
        pdf.set_x(pdf.get_x() + 5)
        pdf.data_field("RUT representante", datos['rut_tutor'] if datos['rut_tutor'] else "N/A", w=w_half, ln=1)
