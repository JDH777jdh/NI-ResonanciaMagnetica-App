...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        # --- LÓGICA DE ORDEN REPARADA (SIN ERRORES) ---
        esp_reales = sorted(df['ESPECIALIDAD'].unique().tolist())
        
        # 1. Identificamos las especialidades clave por búsqueda parcial
        ...
        avanzados = next((e for e in esp_reales if "AVANZADO" in e.upper() or "PROCEDIMIENTO" in e.upper()), None)

        # 2. Creamos el menú final en el orden exacto que pediste
        menu_final = []
        for item in [neuro, musculo, cuerpo, angio, avanzados]:
            if item and item not in menu_final:
                menu_final.append(item)
        
        # 3. Añadimos cualquier otra que falte del CSV
        for e in esp_reales:
            if e not in menu_final:
                menu_final.append(e)

        # Seleccionamos NEURORRADIOLOGIA (o la primera disponible) por defecto
