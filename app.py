...
def generar_pdf_clinico(datos):
    ...
    pdf.data_field("Fecha del Examen", datetime.now().strftime("%d/%m/%Y")) # Punto 1 solicitado
    ...
    # NUEVA PÁGINA: INFORMACIÓN PARA EL PACIENTE (Punto 2 solicitado)
    pdf.add_page()
    ...
    # RECUADRO DE FIRMAS (AL FINAL DE LA SECCIÓN INFORMATIVA)
    y_pos = pdf.get_y()
    if datos['firma_img']:
        pdf.image(datos['firma_img'], x=25, y=y_pos - 10, w=40)
...
# --- PÁGINA 3: CONSENTIMIENTO ---
    ...
    st.subheader(f"Consentimiento para: {st.session_state.procedimiento}")
    ...
    st.session_state.form["autoriza_gad"] = st.radio("¿Autoriza el procedimiento?", ["SÍ", "NO"], index=None)
