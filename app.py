...
def generar_pdf_clinico(datos):
   ...
   # Apartado dinámico: Procedimiento + Contraste
    ...
    if st.session_state.tiene_contraste:
        # Si ya tiene contraste, lo agregamos al texto principal
        texto_procedimiento += " con uso de medio de contraste"
        pdf.multi_cell(0, 7, safe_text(texto_procedimiento), 0, 'L')
    else:
        # Si NO tiene contraste, imprimimos el procedimiento normal...
        pdf.multi_cell(0, 7, safe_text(texto_procedimiento), 0, 'L')
        
        # ...y agregamos la línea adicional para llenado manual
        pdf.set_font('Arial', '', 9) # Letra un poco más pequeña y no negrita
        pdf.ln(1) # Un pequeño espacio extra
        texto_adicional = "☐ Se aplicó medio de contraste adicionalmente"
        pdf.cell(0, 7, safe_text(texto_adicional), 0, 1, 'L')
