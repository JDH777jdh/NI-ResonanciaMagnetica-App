...
# 3. FUNCIONES DE APOYO Y MOTOR PDF PROFESIONAL REDISEÑADO
...
class PDF(FPDF):
    ...
    def section_title(self, num, label):
        ...
        self.set_text_color(128, 0, 32)
        # Corrección: encoding para títulos de sección
        label_fix = str(label).encode('latin-1', 'replace').decode('latin-1')
        self.cell(0, 7, f"{num}. {label_fix}", 0, 1, 'L', 1)
        self.ln(2)

    def data_field(self, label, value):
        self.set_font('Arial', 'B', 9)
        self.set_text_color(50, 50, 50)
        # Corrección: encoding para etiquetas
        label_fix = str(label).encode('latin-1', 'replace').decode('latin-1')
        self.write(5, f"{label_fix}: ")
        self.set_font('Arial', '', 9)
        self.set_text_color(0, 0, 0)
        # Corrección: encoding para valores dinámicos
        value_fix = str(value).encode('latin-1', 'replace').decode('latin-1')
        self.write(5, f"{value_fix}\n")
...
def generar_pdf_clinico(datos):
    ...
    # Corrección: encoding para bloque de texto clínico
    clin_txt_fix = clin_txt.encode('latin-1', 'replace').decode('latin-1')
    pdf.multi_cell(0, 5, clin_txt_fix)
    pdf.ln(4)

    # 4. Quirúrgicos, Terapéuticos y Renal
    ...
    # NUEVA PÁGINA: INFORMACIÓN PARA EL PACIENTE
    pdf.add_page()
    pdf.set_font('Arial', 'B', 12)
    # Corrección: encoding texto fijo página 2
    header_p2 = "LEA ATENTA Y CUIDADOSAMENTE LO SIGUIENTE:".encode('latin-1', 'replace').decode('latin-1')
    pdf.cell(0, 10, header_p2, 0, 1, 'C')
    pdf.ln(5)

    pdf.set_font('Arial', 'B', 10)
    pdf.cell(0, 5, "OBJETIVOS", 0, 1, 'L')
    pdf.set_font('Arial', '', 9)
    texto_obj = ("La Resonancia Magnetica (RM) es una segura tecnica de Diagnostico, que permite la adquisicion de imagenes de gran sensibilidad en todos los planos del espacio de las estructuras del cuerpo. Tiene como objetivo obtener Informacion, datos funcionales y morfologicos para detectar precozmente una enfermedad.\nPara este examen eventualmente se puede requerir la utilizacion de un medio de contraste paramagnetico de administracion endovenosa llamado gadolinio, que permite realzar ciertos tejidos del cuerpo para un mejor diagnostico.")
    pdf.multi_cell(0, 5, texto_obj.encode('latin-1', 'replace').decode('latin-1'))
    pdf.ln(3)

    pdf.set_font('Arial', 'B', 10)
    pdf.cell(0, 5, "CARACTERISTICAS", 0, 1, 'L')
    pdf.set_font('Arial', '', 9)
    texto_carac = ("La Resonancia utiliza fuertes campos magneticos y ondas de radiofrecuencia, por lo que es muy importante dejar fuera de la sala absolutamente todo lo que lleve consigo de tipo metalico y/o electronico (relojes, pulseras, telefonos, tarjetas magneticas, etc). Si lleva material de este tipo en su cuerpo (fijaciones dentales, piercings, algunos tatuajes, balas o esquirlas metalicas) ciertos tipos de protesis (valvulares, de cadera, de rodilla, clips metalicos, etc), o implantes, asi como dispositivos electronicos de caracter medico como bombas de insulina, protesis auditivas, marcapasos, desfibriladores, etc. Avisenos, ya que puede contraindicar de manera absoluta la realizacion de este examen.\nUsted sera posicionado en la camilla del equipo, segun el estudio a realizar y se colocaran cerca de la zona a estudiar unos dispositivos (bobinas) que pueden ser de diversos tamaños. Esta exploracion suele ser larga (entre 20 min y 1 hr segun los casos). Notara ruido derivado del funcionamiento de la RM (por lo que le facilitaremos unos protectores auditivos), todo esto es normal y se le vigilara constantemente desde la sala de control.\nEs muy importante que permanezca quieto durante el estudio y siga las instrucciones del Tecnologo Medico.")
    pdf.multi_cell(0, 5, texto_carac.encode('latin-1', 'replace').decode('latin-1'))
    pdf.ln(3)

    pdf.set_font('Arial', 'B', 10)
    pdf.cell(0, 5, "POTENCIALES RIESGOS", 0, 1, 'L')
    pdf.set_font('Arial', '', 9)
    texto_riesgos = ("Existe una muy baja posibilidad de que se presente una reaccion adversa al medio de contraste (0.07-2.4%) la mayoria de caracter leve fundamentalmente nauseas o cefaleas al momento de la inyeccion.\nPacientes con deterioro importante de la funcion renal, poseen riesgo de desarrollo de fibrosis nefrogenica sistemica.")
    pdf.multi_cell(0, 5, texto_riesgos.encode('latin-1', 'replace').decode('latin-1'))
    pdf.ln(10)

    # TEXTO LEGAL
    pdf.set_font('Arial', '', 9)
    texto_legal = ("He sido informado de mi derecho de anular o revocar posteriormente este documento, dejandolo constatado por escrito y firmado por mi o mi representante.\n\nAutorizo la realizacion del procedimiento anteriormente especificado y las acciones que sean necesarias en caso de surgir complicaciones durante el procedimiento. Ademas, doy consentimiento para que se administren medicamentos y/o infusiones que se requieran para la realizacion de este.")
    pdf.multi_cell(0, 5, texto_legal.encode('latin-1', 'replace').decode('latin-1'))
    pdf.ln(15)
    ...
    # Corrección: encoding firma nombre
    firma_nombre = f"FIRMA PACIENTE: {datos['nombre'][:30]}".encode('latin-1', 'replace').decode('latin-1')
    pdf.text(15, y_pos + 10, firma_nombre)
    
    pdf.line(115, y_pos + 5, 185, y_pos + 5)
    pdf.text(115, y_pos + 10, "FIRMA PROFESIONAL RESPONSABLE")

    # Retornamos los bytes sin intentar decodificar para evitar errores de caracteres
...
# --- PÁGINA 3: INFORMACIÓN Y FIRMA ---
    ...
    st.markdown("""
        ...
        ... [Texto omitido por brevedad en visualización, pero presente en PDF] ...
        <hr>
        Autorizo la realización del procedimiento anteriormente especificado...
        </div>
        """, unsafe_allow_html=True)
...
# --- PÁGINA 4: FINALIZACIÓN Y SUBIDA A DRIVE ---
    ...
    # Intentar subir a Google Drive automáticamente
    with st.spinner("Subiendo registro a Google Drive de Norte Imagen..."):
        exito, resultado = subir_a_google_drive(pdf_output, nombre_final)
        if exito:
            st.info(f"✅ Sincronizado con Drive (ID: {resultado})")
        else:
            st.warning(f"⚠️ No se pudo subir a Drive: {resultado}. Descárguelo manualmente.")

    st.download_button(label="📥 Descargar PDF de Registro", data=pdf_output, file_name=nombre_final, mime="application/pdf")
