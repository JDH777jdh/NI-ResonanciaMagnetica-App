...
# Manejo de estados (Navegación entre pasos)
if 'step' not in st.session_state:
    st.session_state.step = 1
if 'step_data' not in st.session_state:
    st.session_state.step_data = {}

# 2. CARGA DE DATOS (Optimizado)
@st.cache_data
def cargar_datos():
    try:
        # El encoding utf-8-sig elimina el error del \ufeff
...
def formatear_rut(rut_sucio):
    ...
    rut_limpio = rut_sucio.replace(".", "").replace("-", "").upper()
    if len(rut_limpio) < 2:
        return rut_limpio
    ...
    if df is not None:
        with st.container():
            ...
            with col1:
                ...
                rut_input = st.text_input("RUT del Paciente", placeholder="12345678K")
        ...
        if es_menor:
            ...
            with ct2: 
                rut_tutor_input = st.text_input("RUT del Tutor")
                rut_tutor = formatear_rut(rut_tutor_input)
                if rut_tutor_input and rut_tutor != rut_tutor_input:
                    st.caption(f"RUT Tutor: {rut_tutor}")

        st.divider()

        # VERIFICACIÓN Y PRIORIDAD: NEURORRADIOLOGIA por defecto
        esp_reales = df['ESPECIALIDAD'].dropna().unique().tolist()
        
        # 1. Creamos la lista base con las prioridades
        prioridades = ["NEURORRADIOLOGIA", "MUSCULO ESQUELETICO", "CUERPO", "ANGIOGRAFIA POR RM", "ESTUDIOS O PROCEDIMIENTOS AVANZADOS"]
        
        # 2. Filtramos solo las que existen en tu CSV
        menu_ordenado = [e for e in prioridades if e in esp_reales]
        
        # 3. Agregamos cualquier otra especialidad del CSV que no esté en la lista de prioridades
        otras_esp = sorted([e for e in esp_reales if e not in prioridades])
        menu_final = menu_ordenado + otras_esp

        # El selectbox tomará automáticamente el primer elemento (índice 0) como opción por defecto
        esp_sel = st.selectbox("Seleccione Especialidad", menu_final)
        
        list_pre = df[df['ESPECIALIDAD'] == esp_sel]['PROCEDIMIENTO A REALIZAR'].unique().tolist()
        pre_sel = st.selectbox("Seleccione el Procedimiento", sorted(list_pre))
...
# ---------------------------------------------------------
# PASO 3: CONSENTIMIENTO Y FIRMA DIGITAL
# ---------------------------------------------------------
    ...
    st.markdown(f"""
    Yo, **{sujeto}**, RUT **{id_sujeto}**{parentesco}, autorizo la realización del examen 
    **{st.session_state.step_data['procedimiento']}** en la clínica **Norte Imagen**.
    He comprendido los riesgos y declaro que la información entregada es verídica.
    """)
