...
def generar_pdf_clinico(datos):
    ...
    # Encabezado Estilo Médico
    pdf.set_fill_color(128, 0, 32) # Burgundy
    ...
    pdf.cell(95, 7, txt=f"Nombre: {datos['nombre']}", ln=0)
    ...
    if datos['nombre_tutor']:
        ...
        pdf.cell(190, 7, txt=f"Representante Legal: {datos['nombre_tutor']} (RUT: {datos['rut_tutor']})", ln=1)
    
    pdf.ln(5)
    
    # Sección 2: Información Médica y VFG
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(190, 8, txt=" 2. ANTECEDENTES CLÍNICOS", ln=True, fill=True)
    pdf.set_font("Arial", size=10)
    pdf.ln(2)
    
    pdf.cell(95, 7, txt=f"Especialidad: {datos['esp_idx']}", ln=0) # Nota: Aquí idealmente se mapea el nombre
    pdf.cell(95, 7, txt=f"Procedimiento: {st.session_state.procedimiento}", ln=1)
    ...
    # Sección 3: Cuestionario (Resumen de Riesgos)
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(190, 8, txt=" 3. CUESTIONARIO DE SEGURIDAD", ln=True, fill=True)
    pdf.set_font("Arial", size=9)
    
    cuestionario = [
        f"Marcapaso: {datos['ant_marcapaso']}", f"Implantes/Metales: {datos['ant_implantes']}",
        f"Asma: {datos['ant_asma']}", f"Diabetes: {datos['ant_diabetes']}",
        f"Embarazo: {datos['ant_embarazo']}", f"Falla Renal: {datos['ant_renal']}"
    ]
    ...
    pdf.multi_cell(190, 4, txt="El paciente declara que la información proporcionada es fidedigna y autoriza la realización del examen bajo los términos del consentimiento informado aceptado digitalmente.")
...
# ---------------------------------------------------------
# PASO 1: DATOS PERSONALES (ORDEN SOLICITADO)
# ---------------------------------------------------------
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        with col1:
            ...
            st.session_state.form["sin_rut"] = sin_rut
        ...
        if st.button("CONTINUAR"):
            st.session_state.form.update({
                "nombre": nombre_in, "genero_idx": gen_opts.index(genero_in),
...
# ---------------------------------------------------------
# PASO 2: CUESTIONARIO Y SEGURIDAD (SIN RESUMIR)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# PASO 3: CONSENTIMIENTO INFORMADO (ÍNTEGRO)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
    ...
    st.download_button(label="📥 Descargar Documento Clínico (PDF)", data=pdf_bytes, file_name=f"Registro_Clinico_NI.pdf", mime="application/pdf")
