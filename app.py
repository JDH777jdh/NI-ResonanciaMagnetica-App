...
if not firebase_admin._apps:
    try:
        # Hacemos una copia profunda para trabajar con los metadatos de forma aislada
        cred_dict = dict(st.secrets["firebase"])
        
        # 1. Extraemos la URL del bucket de almacenamiento de forma limpia
        ...
        # 2. Reemplazo estricto y doble de caracteres de escape para el certificado PEM (Clave de Python 3.14)
        if "private_key" in cred_dict and isinstance(cred_dict["private_key"], str):
            llave_limpia = cred_dict["private_key"].replace("\\\\n", "\n").replace("\\n", "\n")
            cred_dict["private_key"] = llave_limpia
            
        cred = credentials.Certificate(cred_dict)

        # 3. Inicializamos pasando el storageBucket
        ...
        st.info("💡 Por favor, verifica el formato de tu clave privada en los 'Secrets' de Streamlit Cloud.")
        st.stop()
else:
    firebase_inicializado = True
    try:
...
# --- CONECTORES GLOBALES FINALES (Solo se ejecutan si Firebase está activo) ---
