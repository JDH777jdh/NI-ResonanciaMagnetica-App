...
# FUNCIÓN PARA ELIMINAR TILDES Y NORMALIZAR (Para asegurar coincidencia)
...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        if es_menor:
            ...
            with ct1: nombre_tutor = st.text_input("Nombre del Tutor")
            with ct2: 
        ...
        # --- LÓGICA DE PRIORIDAD INFALIBLE ---
        esp_reales = df['ESPECIALIDAD'].unique().tolist()
        
        # 1. Definimos las prioridades en texto plano (como las escribiría un humano)
        prioridades_raw = ["NEURORRADIOLOGIA", "MUSCULO ESQUELETICO", "CUERPO", "ANGIOGRAFIA POR RM", "ESTUDIOS O PROCEDIMIENTOS AVANZADOS"]
        
        # 2. Creamos un diccionario para mapear el texto normalizado con el texto real del CSV
        # Esto soluciona el problema de las tildes (ej: "NEURORRADIOLOGÍA" coincide con "NEURORRADIOLOGIA")
        mapeo_normalizado = {normalizar_texto(e): e for e in esp_reales}
        
        # 3. Construimos el menú final respetando el orden de prioridades_raw
        menu_final = []
        for p in prioridades_raw:
            p_norm = normalizar_texto(p)
            if p_norm in mapeo_normalizado:
                menu_final.append(mapeo_normalizado[p_norm])
        
        # 4. Añadimos el resto de especialidades que no están en la prioridad
        usadas = set(menu_final)
        otras = sorted([e for e in esp_reales if e not in usadas])
        menu_final.extend(otras)

        # Seleccionamos explícitamente el primer elemento (índice 0)
...
# PASO 2, 3 Y 4 (Igual que el código maestro original)
