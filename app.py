...
def procesar_cedula_inteligente(image_file):
    ...
    try:
        ...
        # 3. Configuración explícita (Forzamos idioma español y modo de detección de bloques)
        # --psm 6 asume un bloque uniforme de texto
        custom_config = r'--oem 3 --psm 6 -l spa' 
        
        texto_extraido = pytesseract.image_to_string(gray, config=custom_config)
        
        # Debug para verificar que LEA ALGO
        print(f"DEBUG OCR (Idioma spa): {texto_extraido[:300]}")
        
        # --- Lógica de extracción ---
        
        # 1. Extracción RUT
        rut_match = re.search(r'(\d{1,2}(?:\.?\d{3}){2}\s*[-_]?\s*[\dkK])', texto_extraido)
        rut = rut_match.group(0).replace(" ", "").replace(".", "") if rut_match else None
        
        # 2. Extracción Sexo
        sexo = None
        if "SEXO" in texto_extraido.upper():
        ...
        # 3. Extracción Fecha de Nacimiento (DD/MM/AAAA)
        nac_match = re.search(r'(\d{2}[-/.]\d{2}[-/.]\d{4})', texto_extraido)
        fecha_nacimiento = nac_match.group(0) if nac_match else None
        
        # 4. Extracción Nombres y Apellidos
        ...
        for i, line in enumerate(lines):
            line_u = line.upper()
            if "APELLIDO" in line_u:
                # Si la línea actual es muy corta (es el título), tomamos la siguiente
                if len(line.split()) <= 2 and i + 1 < len(lines):
                    apellidos_raw = lines[i+1]
                else:
                    apellidos_raw = line.replace("APELLIDOS", "").replace(":", "").strip()
            
            if "NOMBRE" in line_u and "APELLIDO" not in line_u:
                if len(line.split()) <= 2 and i + 1 < len(lines):
                    nombres_raw = lines[i+1]
                else:
                    nombres_raw = line.replace("NOMBRES", "").replace(":", "").strip()
        
        nombre_completo = f"{nombres_raw} {apellidos_raw}".strip()
        if not nombre_completo: nombre_completo = None
            
        return rut, nombre_completo, fecha_nacimiento, sexo
...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        if foto_capturada is not None:
            # Solo procesamos si el nombre del archivo es diferente al último procesado
            if st.session_state.get("archivo_procesado") != foto_capturada.name:
                with st.spinner("Procesando cédula..."):
                    rut, nombre, fecha, sexo = procesar_cedula_inteligente(foto_capturada)
                    
                    # Guardar en sesión
                    if rut: st.session_state.rut = rut
                    if nombre: st.session_state.nombre_paciente = nombre
                    if fecha: st.session_state.fecha_nacimiento = fecha
                    
                    # Marcamos este archivo como procesado para que NO vuelva a entrar aquí
                    st.session_state.archivo_procesado = foto_capturada.name
                    
                    st.success("✅ Datos extraídos correctamente.")
                    # ELIMINAMOS EL st.rerun() AQUÍ PARA ROMPER EL BUCLE
                ...
                with c_cam1:
                    if foto_capturada is not None:
                        with st.spinner("⚡ Analizando documento de forma segura..."):
                            import time
                            rut_enc, sexo_enc = procesar_cedula_inteligente(foto_capturada)
                            
                            if rut_enc or sexo_enc:
                                if rut_enc:
                                    st.session_state.form["rut"] = rut_enc
                                    st.success(f"✅ RUT: {rut_enc}")
                                if sexo_enc:
                                    st.session_state.form["genero_biologico"] = sexo_enc
                                    st.success(f"✅ Sexo Registral: {sexo_enc}")
                ...
                with c_cam_t1:
                    if foto_tutor is not None:
                        with st.spinner("⚡ Procesando OCR de forma segura..."):
                            import time
                            rut_enc_tutor, _ = procesar_cedula_inteligente(foto_tutor)
                            ...
                            if rut_enc_tutor:
                                ...
                                st.error("❌ No se detectó el RUT. Intente con mejor iluminación.")
                with c_cam_t2:
