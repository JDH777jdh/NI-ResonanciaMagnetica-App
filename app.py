...
def generar_pdf_clinico(datos):
    ...
    # === AQUÍ COMIENZA EL FIX DE COLISIÓN ===
    # 1. Dejamos el Medio de Contraste en su propia fila independiente y limpia
    pdf.set_font('Arial', 'B', 9)
    pdf.cell(40, 7, safe_text("Medio de contraste: "), 0, 0)
    pdf.set_font('Arial', '', 9)
    pdf.cell(0, 7, safe_text('SI' if st.session_state.tiene_contraste else 'NO'), 0, 1)

    # 2. El Procedimiento ahora usa multi_cell, ocupando todo el ancho y saltando hacia abajo de forma infinita si es necesario
    pdf.set_font('Arial', 'B', 9)
    pdf.write(6, safe_text("Procedimiento(s): "))
    pdf.set_font('Arial', '', 9)
    pdf.multi_cell(0, 6, safe_text(st.session_state.procedimiento), 0, 'L')
    pdf.ln(1)
...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        # --- PARCHE CSS: Cambia el color de las etiquetas del multiselect a GRIS ---
        st.markdown(
            """
            <style>
            div[data-baseweb="tag"] {
                background-color: #E0E0E0 !important; /* Gris intermedio para la etiqueta */
                border-radius: 4px !important;
            }
            div[data-baseweb="tag"] span {
                color: #262730 !important;            /* Texto oscuro legible */
            }
            </style>
            """,
            unsafe_allow_html=True
        ...
        # Creamos una lista completa combinando Especialidad + Procedimiento para poder buscar libremente
        df_limpio = df.dropna(subset=['ESPECIALIDAD', 'PROCEDIMIENTO A REALIZAR'])
        list_pre_completa = sorted([
            f"[{row['ESPECIALIDAD']}] {row['PROCEDIMIENTO A REALIZAR']}"
            for _, row in df_limpio.iterrows()
        ])
        
        # Un solo buscador ancho y flexible para todas las especialidades a la vez
        pre_sel_raw = st.multiselect("Procedimiento(s) a realizar (Busque libremente de cualquier especialidad)", list_pre_completa)
        
        # Extraemos solo el nombre del procedimiento (quitándole el prefijo con el corchete)
        pre_sel = [p.split("] ", 1)[1] for p in pre_sel_raw] if pre_sel_raw else []
        ...
        if st.button("CONTINUAR"):
            if st.session_state.form["nombre"] and pre_sel:
                # Buscamos en el dataframe general todos los procedimientos que fueron seleccionados
                ...
                # Guardamos los exámenes como un string separado por comas para el PDF
