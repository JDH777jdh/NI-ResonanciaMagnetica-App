...
def generar_pdf_clinico(datos, firma_path=None):
    ...
    # Sección 3: Cuestionario Completo
    pdf.set_font("Arial", 'B', 12)
...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        with col2:
            st.session_state.form["fecha_nac"] = st.date_input("Fecha de Nacimiento", 
        ...
        if edad_act < 18:
            ...
            t_rut = c2.text_input("RUT Representante", value=st.session_state.form["rut_tutor"])
            st.session_state.form["rut_tutor"] = formatear_rut(t_rut)
        ...
        if st.button("CONTINUAR"):
            if not st.session_state.form["nombre"] or (not st.session_state.form["rut"] and not st.session_state.form["num_doc"]):
                st.error("Por favor complete los datos de identificación obligatorios.")
            else:
...
# ---------------------------------------------------------
# PASO 2: CUESTIONARIO Y SEGURIDAD (COMPLETO)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
    ...
    if c_f2.button("FINALIZAR REGISTRO"):
        if st.session_state.form["autoriza_gad"] == "SÍ" and canvas_result.image_data is not None:
            ...
            if np.any(img_data[:, :, 3] > 0): # Verifica si hay algo dibujado
                ...
                st.error("Debe firmar el documento para finalizar.")
        else:
            st.error("Debe autorizar y firmar para finalizar.")
...
# ---------------------------------------------------------
    ...
    st.download_button(label="📥 Descargar Documento Clínico (PDF)", 
                       data=pdf_bytes, 
                       file_name=f"Registro_RM_{st.session_state.form['rut']}.pdf", 
