...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        if st.button("CONTINUAR"):
            if st.session_state.form["nombre"] and pre_sel:
                ...
                # =====================================================================
                # 🩹 CONSTRUCCIÓN DEL NOMBRE FINAL CON LATERALIDAD
                # =====================================================================
                procedimientos_finales = []
                for proc in pre_sel:
                    if proc in dict_lateralidad:
                        procedimientos_finales.append(f"{proc} {dict_lateralidad[proc].upper()}")
                    else:
    ...
    st.markdown('<div class="section-header">3.1 Condiciones o Discapacidades</div>', unsafe_allow_html=True)
    opciones_condicion = [
...
# Bloque de Exámenes Anteriores corregido con 4 espacios
    ...
    # Ejemplo de cómo continuarías los checkbox manteniendo la indentación:
    st.session_state.form["ex_rx"] = ce1.checkbox("Rx", value=st.session_state.form["ex_rx"])
    st.session_state.form["ex_mg"] = ce2.checkbox("MG", value=st.session_state.form["ex_mg"])
    st.session_state.form["ex_eco"] = ce3.checkbox("Eco", value=st.session_state.form["ex_eco"])
    st.session_state.form["ex_tc"] = ce4.checkbox("TC", value=st.session_state.form["ex_tc"])
    st.session_state.form["ex_rm"] = ce5.checkbox("RM", value=st.session_state.form["ex_rm"])
