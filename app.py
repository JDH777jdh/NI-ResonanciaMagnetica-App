...
def generar_pdf_clinico(datos):
    ...
    # Lógica para crear 4 columnas (Opción C)
    # Dividimos por 4.2 para dejar un pequeño margen de seguridad en los bordes
    col_width = pdf.w / 4.2 
    
    # El salto del bucle ahora es de 4 en 4
    for i in range(0, len(clinicos), 4):
        # Fila de 4 elementos
        linea = clinicos[i:i+4]
        
        for item, valor in linea:
            # Bajamos un poco la fuente solo aquí para que quepa el texto en celdas más angostas
            pdf.set_font('Arial', '', 8) 
            texto_col = f"{item}: {valor}"
            
            # Altura reducida a 4.5 para ganar espacio vertical (Opción B)
            pdf.cell(col_width, 4.5, safe_text(texto_col), 0, 0)
        
        # Salto de línea proporcional a la altura de la celda
        pdf.ln(4.5) 

    # Espacio final de la sección reducido
    pdf.ln(2)
