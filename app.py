...
def generar_pdf_clinico(datos):
    ...
    # --- FILA 1: Acceso Venoso y Sitio de Punción ---
    pdf.set_font('Arial', 'B', 9)
    pdf.cell(w_col, 7, safe_text("Acceso venoso:"), 0, 0)
    pdf.cell(w_col, 7, safe_text("Sitio de punción:"), 0, 1)
    
    pdf.set_font('Arial', '', 9)
    # Dibujamos las opciones con guiones para completar
    pdf.cell(w_col, 6, safe_text("[  ] Bránula ____ G   [  ] Mariposa ____ G"), 0, 0)
    pdf.cell(w_col, 6, safe_text("________________________________"), 0, 1)
    
    pdf.ln(2)
    
    # --- FILA 2: Medio de Contraste e Insumos ---
    pdf.set_font('Arial', 'B', 9)
    pdf.cell(w_col, 7, safe_text("Medio de contraste (Intravenoso):"), 0, 0)
    pdf.cell(w_col, 7, safe_text("Cantidad administrada:"), 0, 1)
    
    pdf.set_font('Arial', '', 9)
    # Columna izquierda: Opciones de contraste
    pos_x_actual = pdf.get_x()
    pos_y_actual = pdf.get_y()
    
    # Lista de contrastes con sus respectivos cuadros
    ...
    # Volvemos arriba para la columna de Cantidad
    pdf.set_xy(pos_x_actual + w_col, pos_y_actual)
    pdf.cell(w_col, 7, safe_text("_________ (ml)"), 0, 1)
