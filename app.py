...
def obtener_ip():
    try:
        ...
        if headers:
            # Intentamos obtener la IP real si hay un proxy, si no, la IP remota
            return headers.get("X-Forwarded-For", headers.get("Remote-Addr", "IP Interna"))
    except:
        pass
    return "IP Local"
...
class PDF(FPDF):
    ...
    # IP del dispositivo del hospital
    ip_cliente = obtener_ip()
    
    # ID Registro Final: RUT-Iniciales (IP)
