...
def cargar_datos():
    try:
        ...
        # Convertimos todo a texto y eliminamos filas donde la especialidad esté vacía
        df['ESPECIALIDAD'] = df['ESPECIALIDAD'].fillna("OTRO").astype(str).str.strip()
        return df
...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        # --- LÓGICA DE ORDEN SEGURA (CORRECCIÓN TYPEERROR) ---
        # Obtenemos la lista única y filtramos cualquier valor que no sea texto válido
        ...
        pre_sel = st.selectbox("Seleccione el Procedimiento", sorted([str(p) for p in list_pre if pd.notna(p)]))
...
# PASO 2, 3 Y 4 (Igual al flujo anterior)
