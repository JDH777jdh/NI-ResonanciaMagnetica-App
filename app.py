...
if st.session_state.step == 1:
    ...
    if df is not None:
        with st.container():
            ...
            with col1:
                # CAMBIO APROBADO: Formateo automático de RUT
        ...
        orden_prioridad = ["NEURORRADIOLOGIA", "MUSCULO ESQUELETICO", "CUERPO", "ANGIOGRAFIA POR RM", "ESTUDIOS O PROCEDIMIENTOS AVANZADOS"]
        esp_reales = df['ESPECIALIDAD'].dropna().unique().tolist()
        menu_esp = [e for e in orden_prioridad if e in esp_reales] + sorted([e for e in esp_reales if e not in orden_prioridad])

        esp_sel = st.selectbox("Seleccione Especialidad", menu_esp)
