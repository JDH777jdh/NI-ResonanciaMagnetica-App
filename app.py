...
def generar_pdf_clinico(datos):
   ...
   # 6. FUNCION RENAL (Visible siempre para contingencias)
    pdf.section_title("6", "FUNCION RENAL")
    
    # Si hay datos los pone, si no, deja la línea para escribir a mano
    creatinina_val = f"{datos['creatinina']} mg/dL" if datos['creatinina'] else "__________ mg/dL"
    peso_val = f"{datos['peso']} kg" if datos['peso'] else "__________ kg"
    
    pdf.data_field("Creatinina", creatinina_val)
    pdf.data_field("Peso", peso_val)
    
    # Lógica para la VFG
    if datos['vfg'] and datos['vfg'] > 0:
        if datos['vfg'] < 30:
            pdf.set_text_color(255, 0, 0) # Alerta si ya conocemos el dato
        
        pdf.data_field("RESULTADO VFG", f"{datos['vfg']:.2f} ml/min")
        pdf.set_text_color(0, 0, 0)
    else:
        # Si no hay VFG, dejamos el espacio para el cálculo manual
