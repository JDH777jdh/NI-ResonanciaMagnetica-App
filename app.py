...
if df is not None:
    ...
    with c1:
      ...
      if st.button("CONTINUAR"):
            # 1. Primera validación: Nombre
            if not st.session_state.form.get("nombre"):
                st.error("Por favor, ingresa un nombre.")
            
            # 2. Segunda validación: Selección (el elif va aquí, antes del else)
            elif not pre_sel:
                st.warning("Por favor, selecciona al menos un procedimiento.")
            
            # 3. Si todo es correcto, entramos al else (el "camino feliz")
            else:
                # Lógica de contraste (tal cual la tenías, solo identada un nivel más)
