...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        # NUEVA SECCIÓN: CARGA DE ORDEN MÉDICA
        st.write("Cargue la Orden Médica del examen:")
        st.session_state.form["orden_medica"] = st.file_uploader(
            "Subir Orden (PDF o Imagen)", 
        ...
        if st.button("CONTINUAR"):
            ...
            st.session_state.form["nombre"] = nombre_in
            st.session_state.form["genero_idx"] = gen_opciones.index(genero_in)
            if genero_in == "No binario":
                st.session_state.form["sexo_bio_idx"] = ["Masculino", "Femenino"].index(sexo_bio_in)
            st.session_state.form["fecha_nac"] = fecha_in
            st.session_state.form["email"] = email_in
            st.session_state.form["nombre_tutor"] = tutor_nom_in
            st.session_state.form["rut_tutor"] = formatear_rut(tutor_rut_in)
...
# ---------------------------------------------------------
    ...
    st.session_state.form["tipo_cancer"] = st.text_input("¿Qué tipo de cáncer?", value=st.session_state.form["tipo_cancer"], placeholder="Indique el diagnóstico")
    
    st.write("Seleccione los tratamientos realizados:")
    ...
    st.session_state.form["otro_trat_cancer"] = st.text_input("¿Se empleo algún otro tratamiento?", value=st.session_state.form["otro_trat_cancer"], placeholder="Especifique aquí")
    ...
    st.write("Cargue sus informes o imágenes de exámenes anteriores (Opcional):")
    st.session_state.form["archivos_previos"] = st.file_uploader(
        "Subir archivos previos", 
        type=["pdf", "jpg", "jpeg", "png"], 
        accept_multiple_files=True,
        key="previos_uploader"
    )
    ...
    if col_next.button("Siguiente"):
        ...
        else: st.error("No puede continuar si la información no es fidedigna.")
...
# ---------------------------------------------------------
    ...
    if st.session_state.tiene_contraste:
        ...
        La Resonancia utiliza fuertes campos magnéticos y ondas de radiofrecuencia, por lo que es muy importante dejar fuera de la sala absolutamente todo lo que lleve consigo de tipo metálico y/o electrónico. Avísenos si lleva material de este tipo en su cuerpo (fijaciones dentales, piercings, tatuajes, esquirlas metálicas, clips metálicos, marcapasos, desfibriladores, etc.), ya que puede contraindicar la realización de este examen.<br>
        La exploración suele durar entre 20 min y 1 hr. Notará ruido derivado del funcionamiento de la RM, todo esto es normal y se le vigilará constantemente.<br><br>
        <strong>POTENCIALES RIESGOS</strong><br>
        Existe una muy baja posibilidad de reacción adversa al medio de contraste (0.07-2.4%) mayoritariamente leve (náuseas o cefaleas). Pacientes con deterioro importante de la función renal poseen riesgo de desarrollo de fibrosis nefrogénica sistémica.
        </div>
        """, unsafe_allow_html=True)
        ...
        st.info(f"Yo, **{sujeto}**, RUT **{id_sujeto}**, autorizo la realización del examen **{st.session_state.procedimiento}**.")
    ...
    if col_fin.button("FINALIZAR REGISTRO"):
        if st.session_state.form["autoriza_gad"] == "SÍ" and canvas_result.image_data is not None:
            ...
            st.error("Debe autorizar y firmar para completar el registro.")
