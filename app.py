...
def subir_a_google_drive(archivo_datos, nombre_archivo):
    try:
        ...
        # 1. Subir el archivo inicialmente
        ...
        # 2. TRANSFERENCIA DE PROPIEDAD A TU CUENTA PERSONAL
        # Esto soluciona el error de "Service Accounts do not have storage quota"
        try:
            permission = {
                'type': 'user',
                'role': 'owner',
                'emailAddress': 'jdiaz.bskt7@gmail.com'
            }
            
            # El parámetro transferOwnership=True es fundamental aquí
            ...
            # Si falla la transferencia, lo registramos pero intentamos continuar
            st.warning(f"Nota sobre propiedad: {e_perm}")
...
def generar_pdf_clinico(datos):
    ...
    pdf.multi_cell(0, 5, safe_text(clin_txt))
    pdf.ln(4)
    ...
    pdf.data_field("Detalle cirugias", datos['quir_cirugia_detalle'])
    ...
    pdf.data_field("Detalle de otros tratamientos", datos['quir_otro_trat'])
    ...
    pdf.data_field("Examenes realizados", ", ".join(ex_list) if ex_list else "Ninguno")
    pdf.data_field("Otros examenes anteriores", datos['ex_otros'])
    ...
    for tit, cont in sections.items():
        ...
        pdf.cell(0, 5, tit, 0, 1)
        pdf.set_font('Arial', '', 9)
        pdf.multi_cell(0, 5, safe_text(cont))
        pdf.ln(3)

    pdf.set_font('Arial', '', 9)
    pdf.multi_cell(0, 5, safe_text("He sido informado de mi derecho de anular o revocar posteriormente este documento, dejándolo constatado por escrito y firmado por mi o mi representante.\n\nAutorizo la realización del procedimiento anteriormente especificado y las acciones que sean necesarias en caso de surgir complicaciones durante el procedimiento. Además, doy consentimiento para que se administren medicamentos y/o infusiones que se requieran para la realización de este."))
    
    # Firmas
    pdf.ln(15)
    y_firma = pdf.get_y()
    if datos['firma_img']:
        pdf.image(datos['firma_img'], x=20, y=y_firma - 12, w=50)
    
    pdf.line(15, y_firma + 5, 85, y_firma + 5)
    pdf.set_font('Arial', 'B', 8)
    pdf.text(15, y_firma + 10, f"FIRMA PACIENTE: {safe_text(datos['nombre'][:30])}")
    
    pdf.line(115, y_firma + 5, 185, y_firma + 5)
    pdf.text(115, y_firma + 10, "FIRMA PROFESIONAL RESPONSABLE")
