...
st.markdown("""
    ...
    /* Estilo para las columnas de checkboxes */
...
# ---------------------------------------------------------
# PASO 2: CUESTIONARIO DETALLADO (CHECKBOXES MÚLTIPLES)
# ---------------------------------------------------------
    ...
    # 3. TRATAMIENTOS CÁNCER (CHECKBOXES PARA SELECCIÓN MÚLTIPLE)
    ...
    # 4. EXÁMENES ANTERIORES (CHECKBOXES PARA SELECCIÓN MÚLTIPLE)
    ...
    ex_tc = col_ex3.checkbox("TC (Scanner)")
    ex_rm = col_ex4.checkbox("Resonancia (RM)")
    
    lista_examenes = []
    if ex_rx: lista_examenes.append("Rx")
    if ex_eco: lista_examenes.append("Eco")
    if ex_tc: lista_examenes.append("TC")
    if ex_rm: lista_examenes.append("RM")
    ...
    if col_next.button("Siguiente: Consentimiento"):
        st.session_state.step_data.update({
            "antecedentes": {"Ayuno": ant_ayuno, "Asma": ant_asma, "Diabetes": ant_diabetes, "VFG": vfg},
            "tratamientos_onc": lista_tratamientos,
            "otro_trat_cancer": otro_trat_cancer,
            "examenes_prev": lista_examenes,
            "otro_examen_text": otro_examen,
            "cant_archivos": len(archivos_examenes) if archivos_examenes else 0
        })
        st.session_state.step = 3
        st.rerun()

# ---------------------------------------------------------
# PASO 3 Y 4 (CON SENTIMIENTO Y ÉXITO)
# ---------------------------------------------------------
