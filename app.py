...
if st.session_state.step == 1:
    ...
    if df is not None:
        # =====================================================================
        # 📸 MÓDULO OCR DUAL AVANZADO: DETECCIÓN DE RUT Y SEXO REGISTRAL
        # =====================================================================
        import cv2
        import pytesseract
        import numpy as np
        ...
        # FUNCIÓN INTERNA CON EXTRACCIÓN METADATO BIOLÓGICO Y DE RUT
        def procesar_cedula_avanzado(foto_archivo):
            try:
                file_bytes = np.asarray(bytearray(foto_archivo.read()), dtype=np.uint8)
                img = cv2.imdecode(file_bytes, cv2.IMREAD_COLOR)
                
                # Preprocesamiento optimizado para texto rígido bajo luces de clínica
                ...
                # 1. Extracción de RUT con formato chileno estándar
                patron_rut = r'\b(\d{1,2}(?:\.?\d{3}){2}-[\dkK])\b'
                res_rut = re.search(patron_rut, texto_extraido)
                rut_detectado = res_rut.group(0) if res_rut else None
                
                # 2. Extracción de Sexo Registral (Busca M o F cerca de la etiqueta SEXO)
                sexo_biologico = None
                if re.search(r'\b(SEXO|SEX)\s+[MM]\b', texto_extraido, re.IGNORECASE):
                    sexo_biologico = "Masculino"
                elif re.search(r'\b(SEXO|SEX)\s+[FF]\b', texto_extraido, re.IGNORECASE):
                    sexo_biologico = "Femenino"
                else:
                    # Búsqueda secundaria de caracteres sueltos si hay reflejo en la palabra "SEXO"
                    match_suelto = re.search(r'\b([MF])\b', texto_extraido)
                    if match_suelto:
                        sexo_biologico = "Masculino" if match_suelto.group(1) == "M" else "Femenino"

                return rut_detectado, sexo_biologico
            except Exception as e:
                return None, None

        # --- CONTENEDORES DE ASISTENCIA POR CÁMARA ---
        col_cam1, col_cam2 = st.columns(2)
        
        with col_cam1:
            with st.expander("📸 Escanear Cédula del PACIENTE", expanded=False):
                st.info("Alinee el carnet frente a la cámara para auto-completar el RUT y sexo clínico.")
                foto_paciente = st.camera_input("Capturar Cédula Paciente", key="cam_paciente")
                if foto_paciente:
                    with st.spinner("Procesando documento..."):
                        rut_p, sexo_b = procesar_cedula_avanzado(foto_paciente)
                        if rut_p:
                            st.session_state.form["rut"] = rut_p
                            st.session_state.form["sin_rut"] = False
                            st.success(f"✅ RUT detectado: {rut_p}")
                        if sexo_b:
                            st.session_state.form["genero_biologico"] = sexo_b
                            st.caption(f"🔹 Sexo registral verificado: {sexo_b}")
                        if not rut_p and not sexo_b:
                            st.warning("⚠️ No se detectaron datos claros. Intente con mejor iluminación.")

        with col_cam2:
            with st.expander("📸 Escanear Cédula del REPRESENTANTE", expanded=False):
                st.info("Opcional: Use este visor para capturar el documento del tutor legal o padre.")
                foto_tutor = st.camera_input("Capturar Cédula Tutor", key="cam_tutor")
                if foto_tutor:
                    with st.spinner("Procesando documento del tutor..."):
                        rut_t, _ = procesar_cedula_avanzado(foto_tutor)
                        if rut_t:
                            st.session_state.form["rut_tutor"] = rut_t
                            st.session_state.form["sin_rut_tutor"] = False
                            st.success(f"✅ RUT Tutor detectado: {rut_t}")
                        else:
                            st.warning("⚠️ No se detectó un RUT claro en el documento del representante.")

        st.markdown("<br>", unsafe_allow_html=True)

        # =====================================================================
        # RENDERS DE CAMPOS TRADICIONALES (MUTADOS POR LOS DATOS DEL OCR)
        # =====================================================================
        c1, c2 = st.columns(2)
        with c1:
            ...
            if st.session_state.form["sin_rut"]:
                ...
                # El value se actualiza automáticamente si el OCR inyectó el dato en la sesión
            ...
            if gen_sel == "No binario":
                ...
                # INTELIGENCIA DE TRASPASO: Si el OCR detectó el sexo biológico, lo pre-seleccionamos como default index
                ...
                st.session_state.form["genero_biologico"] = sexo_bio  # Asegura la persistencia para Firestore
                sexo_final = sexo_bio
        ...
        if edad < 18:
            ...
            if st.session_state.form["sin_rut_tutor"]:
                ...
                # El value se actualiza automáticamente si el OCR leyó la cédula del tutor
