...
# === INICIALIZACIÓN ÚNICA, SEGURA Y VISIBLE DE FIREBASE ===
if not firebase_admin._apps:
    try:
        # 1. Copiar los secretos para evitar mutar el estado global de Streamlit
        cred_dict = dict(st.secrets["firebase"])
        
        # 2. Extraer de forma segura la URL del bucket de almacenamiento
        ...
        # 3. Limpieza estricta de la clave privada (Requisito riguroso de Python 3.14)
        if "private_key" in cred_dict and isinstance(cred_dict["private_key"], str):
            raw_key = cred_dict["private_key"]
            # Aislar el contenido Base64 eliminando encabezados/piezas PEM si existieran duplicados
            b64_content = re.sub(r'-----.*?PRIVATE KEY-----', '', raw_key)
            # Remover absolutamente todos los espacios en blanco y saltos de línea literales
            b64_content = re.sub(r'\s+', '', b64_content)
            
            # Fragmentar el Base64 en líneas perfectas de 64 caracteres (Estándar estricto RFC 7468)
            lineas_pem = [b64_content[i:i+64] for i in range(0, len(b64_content), 64)]
            
            # Reconstruir el bloque PEM oficial
            clave_limpia = "-----BEGIN PRIVATE KEY-----\n" + "\n".join(lineas_pem) + "\n-----END PRIVATE KEY-----\n"
            cred_dict["private_key"] = clave_limpia
            
        # 4. Inicializar formalmente el SDK pasando el diccionario limpio y el bucket
        ...
        st.error(f"🚨 Error crítico de infraestructura: No se pudo conectar con Firebase SDK.")
        st.caption(f"Detalle técnico del fallo: {e_firebase}")
        st.stop()  # Detiene la ejecución para evitar fallos derivados aguas abajo

# Asignación de servicios listos para usar en el resto del script
db = firestore.client()
...
# --- PÁGINA 4: FINALIZACIÓN ---
   ...
   # =============================================================================
    ...
    # Usamos 'canvas_result' que se generó arriba en la interfaz gráfica de la firma
    if canvas_result is not None and canvas_result.image_data is not None:
        try:
            # Convertir los datos del canvas de la tablet a una imagen PNG limpia
    ...
    if ruta_firma_storage_final:
        try:
            ...
            payload_firestore = {
                ...
                "creatinina": str(datos_formulario.get('creatinina', 'N/A')),
                "peso": str(datos_formulario.get('peso', 'N/A')),
                "vfg": vfg_calculada,
                ...
                "estado_validacion": "PENDIENTE",  # Crucial: En mayúsculas para activar el filtro .where() en el admin
                "encuesta_validada": False,
                "firma_img": ruta_firma_storage_final, # Apunta al archivo en Storage
