...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        tiene_contraste = pd.notna(info_con) and str(info_con).strip().upper() == "SI"
...
# ---------------------------------------------------------
# PASO 2: CUESTIONARIO (AJUSTE CREATININA)
# ---------------------------------------------------------
    ...
    with st.container():
        ...
        if st.session_state.step_data["tiene_contraste"]:
            ...
            # Ajuste de Creatinina solicitado: Rango 0-7, con 2 decimales y unidad visible
            valor_creatinina = st.number_input(
                "Indique valor de Creatinina (mg/dL):", 
                min_value=0.00, 
                max_value=7.99, 
                value=0.00, 
                step=0.01,
                format="%.2f",
                help="Ingrese el valor numérico con hasta dos decimales."
            )
    ...
    if col_s.button("Ir al Consentimiento"):
        st.session_state.step_data.update({
            ...
            "creatinina": valor_creatinina
        })
        st.session_state.step = 3
        st.rerun()

# ---------------------------------------------------------
# PASO 3 Y 4 (Sin cambios adicionales)
# ---------------------------------------------------------
