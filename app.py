...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        if pre_sel:
            for proc in pre_sel:
                # Verificamos en el CSV si este examen en particular requiere lateralidad
                requiere_lat = df[df['PROCEDIMIENTO A REALIZAR'] == proc]['REQUIERE_LATERALIDAD'].values
                if len(requiere_lat) > 0 and str(requiere_lat[0]).strip().upper() == "SI":
        ...
        if st.button("CONTINUAR"):
            if st.session_state.form["nombre"] and pre_sel:
                # Buscamos en todo el DataFrame si alguno de los exámenes seleccionados requiere contraste (INTACTO)
                ...
                for proc in pre_sel:
                    if proc in dict_lateralidad:
                        # Si requería lateralidad, unimos el nombre con el lado elegido
                        procedimientos_finales.append(f"{proc} {dict_lateralidad[proc].upper()}")
                    else:
                        # Si no requería, pasa el nombre estándar
                        procedimientos_finales.append(proc)

                # Unimos los procedimientos finales con coma para el motor del PDF y Firestore
