...
def cargar_datos():
    try:
        ...
        # Limpieza de espacios en blanco en las categorías para evitar fallos de coincidencia
...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        # --- RE-ESTRUCTURACIÓN DEL ORDEN (FORZADO) ---
        esp_reales = df['ESPECIALIDAD'].dropna().unique().tolist()
        
        # Definimos el orden exacto que tú quieres de arriba hacia abajo
        orden_deseado = ["NEURORRADIOLOGIA", "MUSCULO ESQUELETICO", "CUERPO", "ANGIOGRAFIA POR RM", "ESTUDIOS O PROCEDIMIENTOS AVANZADOS"]
        
        # Filtramos para quedarnos solo con lo que existe en el CSV respetando el orden de arriba
        menu_final = [e for e in orden_deseado if e in esp_reales]
        
        # Agregamos cualquier otra cosa que esté en el CSV pero no en nuestra lista (al final)
        otras = sorted([e for e in esp_reales if e not in orden_deseado])
        menu_final.extend(otras)

        # Usamos index=0 para asegurar que la primera opción de la lista (NEURORRADIOLOGIA) sea la activa por defecto
...
# PASO 2 Y 3 SE MANTIENEN IGUAL (Para ahorrar espacio, pero están en el código real)
