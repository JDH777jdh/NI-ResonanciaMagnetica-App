...
if not firebase_admin._apps:
    try:
        fb_credentials = {
            ...
            # CORREGIDO: Reemplaza la barra invertida doble por un salto de línea real (Misma lógica que admin.py)
        ...
        st.error(f"Error crítico al inicializar credenciales de Firebase: {e_init}")
...
# --- PÁGINA 4: FINALIZACIÓN ---
    ...
    # =====================================================================
    # 🔒 CIRCUITO 1: INDEPENDIENTE - GUARDAR EN FIREBASE FIRESTORE 
    # =====================================================================
    with st.spinner("Registrando antecedentes en la Base de Datos del Hospital..."):
        try:
            # Generar ID único usando RUT y estampa de tiempo
            id_documento = f"{st.session_state.form['rut']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

            datos_encuesta = {
                "rut": st.session_state.form["rut"],
                "nombre": st.session_state.form["nombre"],
                "edad": calcular_edad(st.session_state.form["fecha_nac"]),
                "vfg": st.session_state.form["vfg"],
                "alergias": st.session_state.form["clin_alergico"],
                "creatinina": st.session_state.form["creatinina"],
                "peso": st.session_state.form["peso"],
                "estado_validacion": "PENDIENTE", # Clave para que aparezca en el Admin
                "fecha_creacion": datetime.now(tz_chile).strftime("%d/%m/%Y %H:%M:%S")
            }

            # Guardar de forma directa en la colección unificada
            db.collection("encuestas").document(id_documento).set(datos_encuesta)
            st.toast("📥 ¡Datos del paciente sincronizados en Firestore!", icon="ℹ️")
            
        except Exception as e_fb:
            st.error(f"🚨 Error de conexión con la Base de Datos (Firestore): {e_fb}")
            st.warning("Se continuará con el respaldo en Google Drive de igual forma.")

    # =====================================================================
    # ☁️ CIRCUITO 2: INDEPENDIENTE - SUBIR RESPALDOS A GOOGLE DRIVE
    # =====================================================================
    with st.spinner("Sincronizando archivos médicos con Google Drive..."):
        try:
            ...
            if exito: 
                st.info(f"✅ Archivos respaldados en Drive (ID: {resultado})")
            else: 
                st.warning(f"⚠️ Error al subir PDF a Drive: {resultado}")
        except Exception as e_drive:
            st.error(f"🚨 Error operativo en módulo Drive: {e_drive}")

    # --- BOTONES DE INTERFAZ FINAL ---
