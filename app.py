...
# Campo de fecha reforzado con rangos universales
    fecha_minima = date(1900, 1, 1)
    fecha_maxima = date.today()
    
    st.session_state.form["f_nac"] = st.date_input(
        "Fecha de nacimiento", 
        value=st.session_state.form["f_nac"],
        min_value=fecha_minima,
        max_value=fecha_maxima,
        format="DD/MM/YYYY"
    )

    # Lógica de detección de menores de edad (Cálculo exacto)
    edad = calcular_edad(st.session_state.form["f_nac"])
    
    if edad < 18:
        st.error(f"👦 PACIENTE MENOR DE EDAD ({edad} años)")
        st.markdown("---")
        st.session_state.form["n_tutor"] = capitalizar_nombre(st.text_input("Nombre completo del Representante legal", value=st.session_state.form["n_tutor"]))
        
        ct1, ct2 = st.columns([2, 1])
        st.session_state.form["sin_rut_tutor"] = ct2.checkbox("Sin RUT, poseo otro documento (Tutor)", value=st.session_state.form["sin_rut_tutor"])
        
        if not st.session_state.form["sin_rut_tutor"]:
            st.session_state.form["r_tutor"] = formatear_rut(ct1.text_input("RUT del representante legal", value=st.session_state.form["r_tutor"], placeholder="12.345.678-9"))
        else:
            st.session_state.form["tipo_doc_tutor"] = ct1.selectbox("Tipo de documento (Tutor)", ["Pasaporte", "Cédula de extranjero"])
            st.session_state.form["num_doc_tutor"] = st.text_input("Registro numérico del documento (Tutor)", value=st.session_state.form["num_doc_tutor"])
        st.markdown("---")
...
# Lógica corregida para mostrar prestaciones según especialidad
    if df_csv is not None:
        # Filtramos el CSV asegurando que coincida la especialidad seleccionada
        opciones_filtradas = df_csv[df_csv['ESPECIALIDAD'] == esp_sel]
        
        if not opciones_filtradas.empty:
            # Ordenamos alfabéticamente de la A a la Z como solicitaste
            lista_procedimientos = sorted(opciones_filtradas['PROCEDIMIENTO A REALIZAR'].unique())
            
            st.session_state.form["procedimiento"] = st.selectbox(
                "Procedimiento", 
                lista_procedimientos,
                index=0 if st.session_state.form["procedimiento"] not in lista_procedimientos else lista_procedimientos.index(st.session_state.form["procedimiento"])
            )
            
            # Verificación automática de medio de contraste
            row = opciones_filtradas[opciones_filtradas['PROCEDIMIENTO A REALIZAR'] == st.session_state.form["procedimiento"]]
            st.session_state.form["contraste"] = "SI" if str(row['MEDIO DE CONTRASTE'].values[0]).upper() == "SI" else "NO"
        else:
            st.warning("No se encontraron procedimientos para esta especialidad en el CSV.")
