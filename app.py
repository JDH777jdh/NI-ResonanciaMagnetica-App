...
def procesar_cedula_inteligente(image_file):
    ...
    try:
        ...
        # 3. Extracción Fecha (Captura formato DD/MM/AAAA o 19 ABR 1991)
        ...
        fecha_nacimiento = fecha_match.group(0) if fecha_match else ""
        
        # 4. Extracción Apellidos (Buscamos desde APELLIDOS hasta encontrar NOMBRES)
        # El [\s\n]+ permite saltos de línea. (.*?) captura TODO hasta "NOMBRES"
        ...
        # Unimos, asegurando que no se pierda nada
        nombre_completo = f"{nombres_raw} {apellidos_raw}".strip()
            
        # --- LIMPIEZA FINAL (Segura ante nulos) ---
        ...
        # --- AQUÍ APLICAMOS LA LIMPIEZA SEGURA ---
        # Pasamos todo por tu función de limpieza que ya definimos arriba
        rut_limpio = limpiar_datos_ocr(rut, "rut")
        nombre_limpio = limpiar_datos_ocr(nombre_completo, "nombre")
        fecha_limpia = limpiar_datos_ocr(fecha_nacimiento, "fecha") # ¡Aquí el limpiador convierte ABR a 04!
        sexo_limpio = limpiar_datos_ocr(sexo, "sexo")
