...
if df is not None:
    ...
    with c1:
        ...
        if edad < 18:
            ...
            st.warning(f"👦 PACIENTE MENOR DE EDAD ({edad_exacta_ui})") # <--- AHORA MUESTRA AÑO, MES Y DÍA
        ...
        # =====================================================================
        # 🩹 INYECCIÓN DE LATERALIDAD (Se muestra dinámicamente)
        # =====================================================================
        dict_lateralidad = {}
        if pre_sel:
            # Verificamos si la columna existe antes de entrar al bucle para evitar el KeyError
            existe_col_lat = 'REQUIERE_LATERALIDAD' in df.columns
            
            for proc in pre_sel:
                if existe_col_lat:
                    # Filtramos el dataframe para obtener solo la fila del procedimiento
                    ...
                    if not fila.empty:
                        ...
                        if str(requiere_lat).strip().upper() == "SI":
                            # Si requiere, desplegamos un selectbox solo para este procedimiento
                    ...
                    # Opcional: puedes dejar un comentario o log si la columna no existe
                    pass

        # =====================================================================
        # CONTINUACIÓN DE TU LÓGICA (Sección Documentación)
        # =====================================================================
        ...
        if st.button("CONTINUAR"):
            # 1. Validación: Verificamos si los campos requeridos existen
            if st.session_state.form.get("nombre") and pre_sel:
                
                # 2. Tu lógica actual de cálculo
                ...
                # 3. CAMBIO CRÍTICO: Actualizar el estado de la navegación
                # Supongamos que usas una variable 'paso' para controlar las páginas
                st.session_state['paso'] = 2 
                
                # 4. Forzar el refresco de la pantalla
                st.rerun()
            
            else:
                # Opcional: Feedback si falta completar algo
                st.error("Por favor, completa los campos requeridos antes de continuar.")
                
                # =====================================================================
                # 🩹 CONSTRUCCIÓN DEL NOMBRE FINAL CON LATERALIDAD INTELIGENTE
                # =====================================================================
                ...
                for proc in pre_sel:
                    if proc in dict_lateralidad:
                        ...
                        # Si no requiere lateralidad, entra tal cual
...
# =============================================================================
    ...
    if ruta_firma_storage_final:
        try:
            ...
            # --- CORRECCIÓN QUIRÚRGICA: EVITAR DUPLICACIÓN DE REGISTROS ---
            # Si no ha sido enviado en esta sesión, procedemos a guardar
            if "registro_enviado_db" not in st.session_state:
                try:
                    db = firestore.client()
                    db.collection("encuestas").document(id_documento).set(payload_firestore)
                    
                    # Marcamos como enviado en el estado de sesión para bloquear re-ejecuciones
                ...
                # Si ya se envió, simplemente mostramos el aviso sin re-escribir en BD
                st.caption("✅ Registro ya sincronizado y validado previamente.")

        # --- AQUÍ ESTABA LA PIEZA FALTANTE ---
        # Este except cierra el bloque try principal de la línea 1865
        except Exception as e_main_payload:
            st.error(f"🚨 Error estructurando el documento para Firestore: {e_main_payload}")
    ...
    try:
        ...
        # ID de documento local para trazabilidad en la carga de archivos
        id_documento_drive = f"{payload_firestore['rut']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
