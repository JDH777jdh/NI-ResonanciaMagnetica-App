...
def generar_pdf_clinico(datos):
    ...
    Aquí tienes el código de la Página 2 actualizado con todas tus mejoras: títulos centrados, el apartado dinámico de procedimiento/contraste y el formato de firmas profesional.

Copia este bloque y reemplaza la sección de la Página 2 en tu función:

Python
    # --- PÁGINA 2 ---
    pdf.add_page()
    
    # Encabezado centrado de la Página 2
    pdf.set_font('Arial', 'B', 14)
    pdf.set_text_color(128, 0, 32)
    pdf.cell(0, 10, 'ENCUESTA Y CONSENTIMIENTO INFORMADO', 0, 1, 'C')
    pdf.set_font('Arial', 'B', 12)
    pdf.cell(0, 5, 'RESONANCIA MAGNETICA', 0, 1, 'C')
    pdf.ln(5)
    ...
    # Firma del Paciente (Imagen de Canva)
    if datos.get('firma_img'):
        # Ajustamos posición para que quede sobre la línea
        pdf.image(datos['firma_img'], x=20, y=y_firma - 15, w=45)
    
    # Línea y Texto Paciente
    ...
    pdf.text(15, y_firma + 9, f"Nombre: {safe_text(nombre_firmante[:40])}")
    
    # Línea y Texto Profesional
