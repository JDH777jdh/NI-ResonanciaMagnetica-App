        ...
        with st.container():
            ...
            with col2:
                ...
                fecha_nac = st.date_input(
                    "Fecha de Nacimiento", 
                    value=date(1990, 1, 1),
                    min_value=date(1900, 1, 1), # Permite adultos mayores
                    max_value=date.today(),      # Permite recién nacidos
                    format="DD/MM/YYYY"
                )
...
# ---------------------------------------------------------
    ...
    if st.session_state.step_data["tiene_contraste"]:
        ...
        La Resonancia Magnética (RM) es una segura técnica de Diagnóstico... (texto completo omitido por brevedad en la vista previa pero incluido en el código real)
        </div>
        """, unsafe_allow_html=True)
        
        autoriza_gadolinio = st.radio(
            f"¿Autorizo la realización del procedimiento anteriormente especificado?",
            ["SÍ", "NO"], index=None
        )
    else:
        st.info(f"Yo, **{sujeto}**, RUT **{id_sujeto}**, autorizo el examen **{st.session_state.step_data['procedimiento']}**.")
