...
def obtener_ip():
    import requests # Necesitas tener esta librería (viene por defecto en la mayoría de entornos)
    try:
        # Consultamos a un servicio externo que nos diga desde qué IP lo estamos llamando
        response = requests.get('https://api.ipify.org?format=json', timeout=2)
        ip_data = response.json()
        return ip_data['ip']
    except Exception:
        # Si el servicio externo falla, intentamos el método interno de Streamlit
        try:
            from streamlit.web.server.websocket_headers import _get_websocket_headers
            headers = _get_websocket_headers()
            if headers:
                return headers.get("X-Forwarded-For", "No detectada").split(',')[0]
        except:
            pass
    return "0.0.0.0"
...
def generar_pdf_clinico(datos):
    ...
    # Definimos el ancho de cada columna (aprox mitad de página)
    w_col = (pdf.w - 25) / 2

    # --- FILA 1: Nombre (Usamos tu función original que sí funciona) ---
    pdf.data_field("Nombre Completo", datos['nombre'])
    
    # --- FILA 2: RUT y Edad (Usamos celdas manuales para las 2 columnas) ---
    pdf.set_font('Arial', 'B', 9)
    pdf.cell(w_col, 7, safe_text(f"RUT: {doc_id}"), 0, 0)
    pdf.set_x(pdf.get_x() + 5) # Espacio entre columnas
    ...
    # --- FILA 4: Procedimiento y Contraste ---
    ...
    # --- FILA 5: Representante (Solo si existe) ---
    ...
    # Lógica para crear 4 columnas (Opción C + Opción B integrada)
    col_width = pdf.w / 4.2 
    
    for i in range(0, len(clinicos), 4):
        # Ahora tomamos grupos de 4 elementos
        linea = clinicos[i:i+4]
        
        for item, valor in linea:
            # Bajamos la fuente a 8 para que los textos no se traslapen en 4 columnas
            pdf.set_font('Arial', '', 8) 
            texto_col = f"{item}: {valor}"
            
            # Reducimos altura de 5 a 4.5 para ganar espacio vertical
            pdf.cell(col_width, 4.5, safe_text(texto_col), 0, 0)
        
        # Salto de línea acorde a la nueva altura
        pdf.ln(4.5) 

    # Reducimos el margen final de la sección de 4 a 2 para compactar
    pdf.ln(2)
   ...
   # --- 6. FUNCION RENAL (Blindada para evitar 0.0 y 70.0) ---
    pdf.section_title("6", "FUNCION RENAL")
    
    # 1. Validación de Creatinina (evita 0, 0.0 y None)
    crea = datos.get('creatinina')
    if crea and crea > 0:
        creatinina_val = f"{crea} mg/dL"
    else:
        creatinina_val = "__________ mg/dL"
    
    pdf.data_field("Creatinina", creatinina_val)

    # 2. Validación de Peso (evita 0, 0.0, 70, 70.0 y None)
    peso_real = datos.get('peso')
    # Si el peso es 0 o el 70 estándar, lo tratamos como "no ingresado"
    if peso_real and peso_real > 0 and peso_real != 70.0 and peso_real != 70:
        peso_texto = f"{peso_real} kg"
    else:
        peso_texto = "__________ kg"
    
    pdf.data_field("Peso", peso_texto)

    # 3. Validación de VFG
    vfg_real = datos.get('vfg')
    # Solo mostramos VFG si el peso es válido (no es 70) y la VFG es mayor a 0
    if vfg_real and vfg_real > 0 and peso_texto != "__________ kg":
        if vfg_real < 30:
            pdf.set_text_color(255, 0, 0) # Alerta roja
        pdf.data_field("RESULTADO VFG", f"{vfg_real:.2f} ml/min")
        pdf.set_text_color(0, 0, 0)
    else:
    ...
    pdf.section_title("7", "REGISTRO DE ADMINISTRACION DE MEDIO DE CONTRASTE")
    
    # Configuramos el ancho de las columnas (con margen de seguridad)
    w_col = (pdf.w - 30) / 2
    
    # --- FILA 1: Títulos de Acceso y Punción ---
    pdf.set_font('Arial', 'B', 9)
    pdf.set_fill_color(245, 245, 245)  # Gris muy tenue para el fondo del encabezado
    pdf.cell(w_col, 6, safe_text(" Acceso venoso:"), 0, 0, 'L', fill=True)
    pdf.cell(10, 6, "", 0, 0) # Espacio separador entre columnas
    pdf.cell(w_col, 6, safe_text(" Sitio de punción:"), 0, 1, 'L', fill=True)
    
    # --- FILA 2: Opciones de llenado ---
    pdf.set_font('Arial', '', 9)
    pdf.ln(1)
    
    # Columna Izquierda: Bránula/Mariposa
    # Usamos espacios fijos para que los campos de escritura (____) no se muevan
    opciones_acceso = "[  ] Bránula: _______ G     [  ] Mariposa: _______ G"
    pdf.cell(w_col, 8, safe_text(opciones_acceso), 0, 0, 'L')
    
    pdf.cell(10, 8, "", 0, 0) # Espacio separador
    
    # Columna Derecha: Línea de punción
    pdf.cell(w_col, 8, safe_text("________________________________"), 0, 1, 'L')
    
    pdf.ln(2)
    
    # --- FILA 3: Medio de Contraste e Insumos ---
    pdf.set_font('Arial', 'B', 9)
    ...
    # --- ÁREA FINAL: Contrastes vs Cantidad ---
    pdf.set_font('Arial', '', 9)
    pos_y_bloque = pdf.get_y()
    pos_x_bloque = pdf.get_x()
    
    # Listado de fármacos (Columna Izquierda)
    pdf.cell(w_col, 5, safe_text("[  ] Ácido gadotérico (Clariscan)"), 0, 1)
    pdf.cell(w_col, 5, safe_text("[  ] Gadopiclenol (Elucirem)"), 0, 1)
    pdf.cell(w_col, 5, safe_text("[  ] Ácido gadoxético (Primovist)"), 0, 1)
    
    # Volvemos arriba para posicionar la Cantidad (Columna Derecha)
    pdf.set_xy(pos_x_bloque + w_col + 10, pos_y_bloque)
