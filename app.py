...
# === NUEVA INTEGRACIÓN CLÍNICA: FIREBASE SDK ===
import firebase_admin
from firebase_admin import credentials, firestore, storage

# === INICIALIZACIÓN SEGURA DEL SDK ADMIN DE FIREBASE ===
if not firebase_admin._apps:
    try:
        # Reconstrucción dinámica del certificado usando tus Secrets validados
        fb_credentials = {
            "type": st.secrets["firebase"]["type"],
            "project_id": st.secrets["firebase"]["project_id"],
            "private_key_id": st.secrets["firebase"]["private_key_id"],
            # Procesa correctamente los caracteres \n de tu clave RSA
        ...
        # Conexión al Admin SDK e inyección del bucket de Storage de us-central
        cred = credentials.Certificate(fb_credentials)
        firebase_admin.initialize_app(cred, {
            'storageBucket': st.secrets["firebase"]["bucket_url"]
        })
    except Exception as e:
        st.error(f"🚨 Error crítico al enlazar las credenciales de Firebase: {e}")

# Instanciación de variables de base de datos y archivos accesibles globalmente
db = firestore.client()
...
# --- NUEVA CONFIGURACIÓN DE FIREBASE (OPCIÓN A) ---
# ==============================================================================
import firebase_admin
from firebase_admin import credentials, firestore, storage

if not firebase_admin._apps:
    # Vinculación directa con las credenciales que pondrás en Streamlit Secrets
...
if 'form' not in st.session_state:
    st.session_state.form = {
        ...
        # ─── NUEVOS CAMPOS DE AUDITORÍA Y DOBLE FIRMA HOSPITALARIA ───
        "profesional_nombre": "",
...
def generar_pdf_clinico(datos):
    ...
    # Fila 1: Nombre del Paciente (Se reduce altura de 7 a 5 para compactar)
    pdf.set_font('Arial', 'B', 10)
    pdf.cell(32, 5, "Nombre Completo: ", 0, 0)
    pdf.set_font('Arial', '', 10)
    pdf.cell(0, 5, safe_text(datos['nombre']), 0, 1) # ln=1 baja a la siguiente línea limpiamente

    # Fila 2: RUT y Edad (En paralelo, altura 5)
    pdf.set_font('Arial', 'B', 9)
    pdf.cell(18, 5, "RUT: ", 0, 0)
    pdf.set_font('Arial', '', 9)
    pdf.cell(w_col - 18, 5, safe_text(doc_id), 0, 0)
    
    pdf.set_x(pdf.get_x() + 5) # Mantiene tu misma separación de columnas
    
    pdf.set_font('Arial', 'B', 9)
    pdf.cell(12, 5, "Edad: ", 0, 0)
    pdf.set_font('Arial', '', 9)
    pdf.cell(w_col - 12, 5, f"{edad_calculada} años", 0, 1) # ln=1 baja a la siguiente línea

    # Fila 3: Fecha de Nacimiento y Email (En paralelo, altura 5)
    pdf.set_font('Arial', 'B', 9)
    pdf.cell(35, 5, "Fecha Nacimiento: ", 0, 0)
    pdf.set_font('Arial', '', 9)
    pdf.cell(w_col - 35, 5, datos['fecha_nac'].strftime('%d/%m/%Y'), 0, 0)
    ...
    pdf.set_font('Arial', '', 9)
    pdf.cell(w_col - 12, 5, safe_text(datos['email']), 0, 1) # ln=1 baja a la siguiente línea

    # Fila 4: Medio de Contraste
    pdf.set_font('Arial', 'B', 9)
    pdf.cell(35, 5, "Medio de contraste: ", 0, 0)
    pdf.set_font('Arial', '', 9)
    pdf.cell(0, 5, 'SI' if st.session_state.tiene_contraste else 'NO', 0, 1)

    # Fila 5: Procedimientos (Mantiene el formato para textos largos, ajustado a interlineado 5)
    pdf.set_font('Arial', 'B', 9)
    pdf.write(5, "Procedimiento(s): ")
    pdf.set_font('Arial', '', 9)
    pdf.multi_cell(0, 5, safe_text(st.session_state.procedimiento), 0, 'L')

    # Fila 6: Datos del Tutor (Si aplica)
    if datos['nombre_tutor']:
        pdf.ln(1) # Un pequeño respiro antes del tutor
        pdf.set_font('Arial', 'B', 9)
        pdf.cell(28, 5, "Representante: ", 0, 0)
        pdf.set_font('Arial', '', 9)
        pdf.cell(w_col - 28, 5, safe_text(datos['nombre_tutor']), 0, 0)
        ...
        pdf.set_font('Arial', '', 9)
        pdf.cell(w_col - 35, 5, safe_text(datos['rut_tutor']), 0, 1)
...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        # --- PARCHE CSS: Garantiza las etiquetas en color GRIS ---
        st.markdown(
            """
            <style>
        ...
        # 1. Recuperamos tus dos columnas originales para las dos cajas independientes
        ...
        # Filtramos las opciones según la especialidad de la caja 1
        filtered = df[df['ESPECIALIDAD'] == esp_sel]
        list_pre = sorted(filtered['PROCEDIMIENTO A REALIZAR'].dropna().unique().tolist())
        
        # Inicializamos un estado interno si no existe para recordar las selecciones acumuladas
        if "acumulados" not in st.session_state:
            st.session_state.acumulados = []

        # 2. Caja de Procedimientos en la columna 2 como multiselect
        # El truco es combinar las opciones del filtro actual con lo que el usuario ya seleccionó antes
        opciones_visibles = sorted(list(set(list_pre + st.session_state.acumulados)))
        
        pre_sel = ce2.multiselect(
            "Procedimiento(s) a realizar", 
            options=opciones_visibles,
            default=st.session_state.acumulados
        )
...
# --- PÁGINA 4: FINALIZACIÓN ---
    ...
    # ─── BLOQUE INFORMATIVO INTEGRADO EN UNA SOLA LÍNEA FLUIDA ───
    texto_html = "<div style='background-color: #e3f2fd; padding: 18px; border-radius: 8px; border-left: 5px solid #2196f3; margin-bottom: 20px;'><h4 style='color: #0d47a1; margin-top: 0; margin-bottom: 8px;'>📋 Cuestionario en Custodia Segura</h4><p style='color: #1565c0; font-size: 15px; margin-bottom: 5px;'>Sus antecedentes y firma digital han sido respaldados en la base de datos central del hospital.</p><p style='color: #1c3d5a; font-size: 13.5px; margin-bottom: 0;'><b>¿Qué viene ahora?</b> Por favor, tome asiento. El Tecnólogo Médico revisará y validará internamente esta encuesta en su estación de trabajo antes de llamarlo a su examen.</p></div>"
    
    st.markdown(texto_html, unsafe_html=True)
    
    # Mostrar el ID único de atención transitoria generado en la Página 3
    if st.session_state.get("ultimo_documento_id"):
        st.info(f"🔑 **ID de Operación Firebase:** {st.session_state['ultimo_documento_id']}")
    
    # Mantener intacta tu estructura de generación de PDF original
    ...
    # Mantener intacta tu estructura de sincronización con Google Drive
    ...
    # Mantener tu botón de descarga que ya sirve
    st.download_button("📥 Descargar Copia PDF", data=pdf_bytes, file_name=nombre_final, mime="application/pdf")
    
    # Mantener tu botón de reinicio estructural para limpiar la tablet o tótem
