...
def generar_pdf_clinico(datos):
    ...
    # --- LÓGICA BLINDADA PARA EL ID DEL PACIENTE ---
    ...
    pdf.cell(0, 5, safe_text(datos['nombre']), 0, 1)

    # Fila 2: RUT/ID y Edad
    pdf.set_font('Arial', 'B', 9)
    pdf.cell(18, 5, "RUT/ID: ", 0, 0) # Cambiado RUT a RUT/ID para mayor claridad
    ...
    pdf.set_font('Arial', '', 9)
    pdf.cell(w_col - 12, 5, f"{edad_calculada} años", 0, 1)
  ...
  # Apartado dinámico: Procedimiento + Contraste
    ...
    # --- SECCIÓN DE FIRMAS (ALINEACIÓN PROFESIONAL Y SIMÉTRICA) ---
    pdf.ln(5)
    y_pos_firmas = pdf.get_y()
    
    # Datos de firmas
    ruta_p_local = datos.get('firma_img')
    # En app.py no hay firma del TM, se deja el espacio reservado
    
    # 1. ESTAMPAMOS LAS IMÁGENES PNG (Si existen)
    if ruta_p_local and os.path.exists(ruta_p_local):
        pdf.image(ruta_p_local, 35, y_pos_firmas, 45, 12) # Centrado en col 95
    ...
    pdf.cell(95, 4, "PENDIENTE VISACIÓN TM", 0, 1, 'C') 
...
if st.session_state.step == 1:
    ...
    if df is not None:
        ...
        /* 2. Estilo para Cajas de Texto y Textareas (UX mejorada) */
        ...
        /* 3. Efecto corporativo al hacer click (Focus) */
