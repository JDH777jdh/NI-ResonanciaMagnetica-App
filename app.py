...
def generar_pdf_clinico(datos):
  ...
  # Apartado dinámico: Procedimiento + Contraste
    ...
    if st.session_state.tiene_contraste:
        # Escenario A: Ya tiene contraste definido en la App
        texto_procedimiento += " con uso de medio de contraste."
        pdf.multi_cell(0, 7, safe_text(texto_procedimiento), 0, 'L')
    else:
        # Escenario B: Examen simple, agregamos el cuadro manual
        pdf.multi_cell(0, 7, safe_text(texto_procedimiento), 0, 'L')
        
        # --- Dibujo del Checkbox Manual ---
        pdf.ln(1) # Pequeño espacio para no pegar el cuadro al texto de arriba
        pdf.set_font('Arial', '', 9)
        ...
        # Dibujamos el rectángulo (x, y, ancho, alto)
        # Ajustamos pos_y + 1.5 para que el cuadro quede centrado con el texto
        pdf.rect(pos_x + 1, pos_y + 1.5, 3.5, 3.5) 
        
        # Movemos la "X" a la derecha del cuadro (6mm de espacio)
        pdf.set_x(pos_x + 7) 
        
        # Escribimos el texto descriptivo
        pdf.cell(0, 7, safe_text("¿Se aplicó medio de contraste adicionalmente?"), 0, 1, 'L')
