...
# Añadimos col_t para la talla necesaria en pediatría
...
with col_p:
    ...
    peso_profesional = st.number_input(
        ...
        help="Deje en 0.0 para estudios basales."
    )
...
with col_t:
    ...
    talla_profesional = st.number_input(
        ...
        help="Obligatorio para cálculo en pacientes pediátricos."
    )

# 3. Cálculo automatizado de VFG en tiempo real con lógica diferenciada
vfg_dinamico = 0.0

# 🚀 0. EXTRACCIÓN PREVIA DE DATOS DE PACIENTE
import re
edad_raw = str(datos_doc.get('edad', datos_doc.get('edad_num', '40')))
numeros = re.findall(r'\d+', edad_raw)
edad_paciente = int(numeros[0]) if numeros else 40
es_pediatrico = edad_paciente < 18

if peso_profesional > 0 and creatinina_profesional > 0:
    
    # LÓGICA PEDIÁTRICA (Fórmula Schwartz)
    if es_pediatrico:
        if talla_profesional > 0:
            # 0.413 * Talla / Creatinina
            vfg_bruto = (0.413 * talla_profesional) / creatinina_profesional
            vfg_dinamico = round(vfg_bruto, 2)
        else:
            vfg_dinamico = 0.0
            
    # LÓGICA ADULTOS (Cockcroft-Gault)
    else:
        sexo_raw = str(datos_doc.get('sexo', datos_doc.get('genero', 'M'))).lower()
        es_mujer = 'fem' in sexo_raw or 'mujer' in sexo_raw or datos_doc.get('es_mujer', False)
        factor_sexo = 0.85 if es_mujer else 1.0
        
        try:
            vfg_bruto = ((140 - edad_paciente) * peso_profesional) / (72 * creatinina_profesional) * factor_sexo
            vfg_dinamico = round(vfg_bruto, 2)
