...
# =====================================================================
# 🟢 RENDERIZADO EN 2 COLUMNAS CON DISTRIBUCIÓN ESPECÍFICA (SIN SANGRE)
# =====================================================================
if datos_doc:
    st.divider()
    ...
    with c1:
        ...
        with st.expander("🧲 2. BIOSEGURIDAD MAGNÉTICA", expanded=True):
            st.write(f"**Marcapasos cardíaco:** {'✅ SÍ' if datos_doc.get('bio_marcapaso') else '🔴 NO'}")
            st.write(f"**Implantes / Prótesis / Dispositivos:** {'✅ SÍ' if datos_doc.get('bio_implantes') else '🔴 NO'}")
            st.write("**Detalle Bioseguridad:**")
            st.info(datos_doc.get('bio_detalle') if datos_doc.get('bio_detalle') else "Sin observaciones")

        # --- C. ANTECEDENTES CLÍNICOS (TABLA DE RIESGOS) ---
        with st.expander("📋 3. ANTECEDENTES CLÍNICOS", expanded=True):
            data_riesgos = {
                ...
                "Estado": [
                    "✅ SÍ" if datos_doc.get('clin_ayuno') else "🔴 NO",
    ...
    with c2:
        ...
        with st.expander("🧪 6. EVALUACIÓN DE LA FUNCIÓN RENAL", expanded=True):
            ...
            if vfg > 0:
                if vfg <= 30.0:
                    st.error(f"🚨 **VFG Crítica:** {vfg:.2f} ml/min (Alto Riesgo para la administración de medio de Contraste)")
                elif vfg <= 59.0:
                    st.warning(f"⚠️ **VFG Intermedia:** {vfg:.2f} ml/min (Riesgo Intermedio para la administración de medio de Contraste)")
                else:
                    st.success(f"✅ **VFG Normal:** {vfg:.2f} ml/min (Sin Riesgo para la administración de medio de Contraste)")
        ...
        with st.expander("🏥 4. ANTECEDENTES QUIRÚRGICOS Y TERAPÉUTICOS", expanded=True):
            ...
            trats_activos = []
            if datos_doc.get('rt'): trats_activos.append("RT")
            if datos_doc.get('qt'): trats_activos.append("QT")
            if datos_doc.get('bt'): trats_activos.append("BT")
            if datos_doc.get('it'): trats_activos.append("IT")
