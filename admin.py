...
if st.session_state.get("doc_completo") is not None:
    ...
    # 👨‍👦 Datos del Tutor (Solo si es menor de edad, ej: < 18 años)
    # Nota: Si el bloque except se ejecuta, edad_int será 0, por lo que también entraría aquí como precaución.
    if edad_int < 18:
    ...
    # ⚠️ Mensajes de recomendación clínica personalizados
    nota_marcapaso = ""
    nota_implante = ""

    # Normalizamos el texto a mayúsculas y quitamos espacios para evitar fallos si viene "si " o "Sí"
    ...
    # ⚠️ Notas de Recomendación de Triaje Clínico
    nota_alergico = ""
    nota_dialisis = ""
    nota_renal = ""
    nota_embarazo = ""
    ...
    # Evaluación tolerante a variaciones de texto (Mayúsculas/Tildes/Espacios)
    ...
    if str(clin_dialisis).strip().upper() in ["SI", "SÍ"]:
        nota_dialisis = "No se debe inyectar medio de contraste basado en Gadolinio."

    if str(clin_renal).strip().upper() in ["SI", "SÍ"]:
        nota_renal = "Se debe considerar la VFG para la administración de medio de contraste."

    if str(clin_embarazo).strip().upper() in ["SI", "SÍ"]:
        nota_embarazo = "Precaución, paciente de alto cuidado."

    if str(clin_claustro).strip().upper() in ["SI", "SÍ"]:
        nota_claustro = "Puede requerir atención personalizada."
...
if datos_doc:
    ...
    with c1:
        ...
        with st.expander("🧲 2. BIOSEGURIDAD MAGNÉTICA", expanded=True):
            # 1. Marcapasos cardíaco y su mensaje asociado
            tiene_marcapaso = datos_doc.get('bio_marcapaso')
            ...
            # 2. Implantes / Prótesis / Dispositivos y su mensaje asociado
            tiene_implantes = datos_doc.get('bio_implantes')
        ...
        with st.expander("📋 3. ANTECEDENTES CLÍNICOS", expanded=True):
            data_riesgos = {
                ...
                "Estado": [
                    "✅ SÍ" if datos_doc.get('clin_ayuno') else "🔴 NO",  # El ayuno cumplido es positivo
                    "🔴 SÍ" if datos_doc.get('clin_asma') else "✅ NO",
                    "🔴 SÍ" if datos_doc.get('clin_alergico') else "✅ NO",
                    "🔴 SÍ" if datos_doc.get('clin_hiperten') else "✅ NO",
                    "🔴 SÍ" if datos_doc.get('clin_hipertiroid') else "✅ NO",
                    "🔴 SÍ" if datos_doc.get('clin_diabetes') else "✅ NO",
                    "🔴 SÍ" if datos_doc.get('clin_metformina') else "✅ NO",
                    "🚨 SÍ" if datos_doc.get('clin_renal') else "✅ NO",
                    "🚨 SÍ" if datos_doc.get('clin_dialisis') else "✅ NO",
                    "🚨 SÍ" if datos_doc.get('clin_embarazo') else "✅ NO",
                    "🔴 SÍ" if datos_doc.get('clin_lactancia') else "✅ NO",
                    "🔴 SÍ" if datos_doc.get('clin_claustro') else "✅ NO"
                ],
                "Recomendación / Alerta": [
                    "", # Ayuno
                    "", # Asma
                    datos_doc.get('nota_alergico', '') if datos_doc.get('clin_alergico') else "",
                    "", # Hipertensión
                    "", # Hipotiroidismo
                    "", # Diabetes
                    "", # Metformina
                    datos_doc.get('nota_renal', '') if datos_doc.get('clin_renal') else "",
                    datos_doc.get('nota_dialisis', '') if datos_doc.get('clin_dialisis') else "",
                    datos_doc.get('nota_embarazo', '') if datos_doc.get('clin_embarazo') else "",
                    "", # Lactancia
                    datos_doc.get('nota_claustro', '') if datos_doc.get('clin_claustro') else ""
                ]
            }
    ...
    with c2:
        ...
        with st.expander("🧪 6. EVALUACIÓN DE LA FUNCIÓN RENAL", expanded=True):
            try:
                ...
                crea, peso, vfg = 0.0, 0.0, 0.0

            col_param1, col_param2, col_param3 = st.columns(3)
            col_param1.metric("Creatinina", f"{crea:.2f} mg/dL" if crea > 0 else "__________")
            col_param2.metric("Peso", f"{peso:.1f} Kg" if peso > 0 else "__________")
            col_param3.metric("VFG", f"{vfg:.2f}" if vfg > 0 else "__________", "mL/min")

            if vfg > 0:
                if vfg <= 30.0:
                    st.error(f"🚨 **VFG Crítica:** {vfg:.2f} ml/min (Alto Riesgo Contraste)")
                elif vfg <= 59.0:
                    st.warning(f"⚠️ **VFG Intermedia:** {vfg:.2f} ml/min (Riesgo Intermedio)")
                else:
                    st.success(f"✅ **VFG Normal:** {vfg:.2f} ml/min (Sin Riesgo)")

        # --- B. ANTECEDENTES QUIRÚRGICOS Y TERAPÉUTICOS ---
        with st.expander("🏥 4. ANTECEDENTES QUIRÚRGICOS Y TERAPÉUTICOS", expanded=True):
            st.write(f"**Cirugías:** {'🔴 SÍ' if datos_doc.get('quir_cirugia_check') else '✅ NO'}")
            ...
            with st.spinner("Estampando firma del profesional y consolidando documento..."):
                try:
                   ...
                   # =====================================================================
                    ...
                    nombre_firmante = nombre_tutor if nombre_tutor else paciente_nombre
                    ...
                    pdf.cell(ancho_linea, 4, f" Registro SIS: {safe_text(profesional_registro)}", 0, 1, 'L')
