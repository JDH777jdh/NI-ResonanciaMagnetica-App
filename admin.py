...
c1, c2 = st.columns([1, 1])
            ...
            with c1:
                ...
                with st.expander("📋 3. ANTECEDENTES CLÍNICOS", expanded=True):
                    # Grilla de 3 columnas para optimizar espacio en pantalla
                    col_clin1, col_clin2, col_clin3 = st.columns(3)
                    
                    clinicos_lista = [
                        ("Ayuno 2hrs+", 'clin_ayuno'),
                        ("Asma", 'clin_asma'),
                        ("Alergias", 'clin_alergico'),
                        ("Hipertensión", 'clin_hiperten'),
                        ("Hipotiroidismo", 'clin_hipertiroid'),
                    ...
                    for idx, (label, key) in enumerate(clinicos_lista):
                        estado_val = "🔴 SÍ" if datos_doc.get(key) else "✅ NO"
                        if idx % 3 == 0:
                            col_clin1.write(f"**{label}:** {estado_val}")
                        elif idx % 3 == 1:
                            col_clin2.write(f"**{label}:** {estado_val}")
                        else:
                            col_clin3.write(f"**{label}:** {estado_val}")
            ...
            with c2:
                ...
                # --- B. ANTECEDENTES QUIRÚRGICOS Y TERAPEUTICOS ---
                with st.expander("🏥 4. ANTECEDENTES QUIRÚRGICOS Y TERAPÉUTICOS", expanded=True):
                    ...
                    # Tratamientos oncológicos cruzados
                    trats_activos = []
