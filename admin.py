        ...
        if canvas_profesional is not None and canvas_profesional.json_data is not None and len(canvas_profesional.json_data["objects"]) > 0:
            with st.spinner("Estampando firma del profesional y consolidando documento..."):
                try:
                    ...
                    # =====================================================================
# 📄 GENERACIÓN DEL PDF: DISEÑO EXACTO NORTE IMAGEN (VERSIÓN OPTIMIZADA)
# =====================================================================
st.info("🔄 Compilando formato institucional Norte Imagen...")
...
# --- 1. EXTRACCIÓN Y NORMALIZACIÓN DE VARIABLES DESDE FIREBASE ---
datos_doc = doc_completo
paciente_nombre = datos_doc.get('nombre', 'Paciente No Identificado')
paciente_rut = datos_doc.get('rut', datos_doc.get('run', 'S/R'))
...
# Normalización estricta de género para el filtro obstétrico
genero = str(datos_doc.get('genero', datos_doc.get('sexo', 'No especificado'))).strip().capitalize()

# Validación e interpretación matemática de la edad (Lógica Pediátrica)
try:
...
# Lógica de detección de Medio de Contraste
...
# Variables de tutela legal
rep_nombre = datos_doc.get('representante_nombre', '')
rep_rut = datos_doc.get('representante_rut', '')

# Trazabilidad digital e imágenes de auditoría
ip_cliente = datos_doc.get('ip_paciente', datos_doc.get('ip_dispositivo', 'No detectada'))
ruta_firma_paciente_storage = datos_doc.get('firma_img', '')

st.session_state.paciente_nombre_val = paciente_nombre

# Funciones auxiliares de codificación y traducción clínica
def safe_text(txt):
    return str(txt).encode('latin-1', 'replace').decode('latin-1')

def parse_bool_clinico(val, campo_nombre=""):
    # Si el género es explícitamente masculino, omitimos variables gineco-obstétricas
...
# --- 2. DEFINICIÓN DE LA CLASE FPDF CON CONSTRUCTOR DE TRAZABILIDAD ---
...
# --- 3. INICIALIZACIÓN FORMAL DEL DOCUMENTO ---
pdf = PDF_Institucional(paciente_nombre, paciente_rut, ip_cliente, fecha_validacion_str)
pdf.alias_nb_pages()
pdf.add_page()
pdf.set_auto_page_break(auto=True, margin=12)

# El ancho útil real descontando márgenes laterales estándar (10mm izquierdo + 10mm derecho)
ancho_util = pdf.w - 20

# =====================================================================
# 📄 RENDERIZADO: PÁGINA 1 (DATOS CLÍNICOS E IDENTIFICACIÓN)
# =====================================================================
pdf.section_title("1", "IDENTIFICACION DEL PACIENTE")
pdf.set_text_color(0, 0, 0)
w_col = ancho_util / 2

# Condicional Pediátrico: Estampado destacado si es menor de edad
if 0 < edad_int < 18:
...
# Despliegue dinámico del tutor legal
...
# --- SECCIÓN 3: ANTECEDENTES CLÍNICOS (GRILLA RECALCULADA SIMÉTRICA) ---
...
col_width_4 = ancho_util / 4  # División exacta en 4 columnas dentro de márgenes
...
# --- SECCIÓN 5: EXÁMENES ANTERIORES (GRILLA RECALCULADA DE 5 COLUMNAS) ---
...
col_width_5 = ancho_util / 5 # Distribución matemática perfecta
...
# =====================================================================
# 📄 RENDERIZADO: PÁGINA 2 (BASES LEGALES Y MATRIZ DE FIRMAS EN PARALELO)
# =====================================================================
pdf.add_page()
...
# --- CONTROL PREVENTIVO DE DESBORDAMIENTO EN BLOQUE DE FIRMAS ---
# Si queda poco espacio vertical en la página, forzamos salto para evitar firmas huérfanas
if pdf.get_y() > 225:
    pdf.add_page()
...
# Variable centinela para garantizar el borrado del archivo en el bloque finally
ruta_p_local = None

try:
    # --- ESTAMPADO GRÁFICO: COLUMNA IZQUIERDA (PACIENTE) ---
    ...
    # --- ROTULACIÓN INFRAESTRUCTURA PROFESIONAL RESPONSABLE ---
    pdf.set_font('Arial', 'B', 8)
    ...
    # --- LIMPIEZA ABSOLUTA DE MEMORIA EN DISCO ---
    # Eliminamos el archivo temporal inmediatamente después del renderizado para evitar fugas de almacenamiento
...
# --- 4. EXPORTACIÓN HÍBRIDA ADAPTATIVA PARA STREAMLIT ---
try:
    # Intento moderno (fpdf2): sin argumentos devuelve bytes/bytearray directamente
    pdf_output = pdf.output()
except TypeError:
    # Caída heredada (fpdf clásico): requiere forzar dest='S'
    pdf_output = pdf.output(dest='S')

# Normalización del formato de salida a Bytes nativos de Python
