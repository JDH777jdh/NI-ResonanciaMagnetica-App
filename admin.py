...
if datos_doc:
    ...
    # =============================================================================
    with c2:
        # --- A. EVALUACIÓN DE LA FUNCIÓN RENAL (CORREGIDO CON PESO) ---
        with st.expander("🧪 6. EVALUACIÓN DE LA FUNCIÓN RENAL", expanded=True):
            try:
                crea = float(datos_doc.get('creatinina', 0.0))
                peso = float(datos_doc.get('peso', 0.0)) # Recuperamos el peso
                vfg = float(datos_doc.get('vfg', 0.0))
                
                # Determinación de estado para la lógica visual
                if vfg <= 30.0:
                ...
                riesgo_estado, color_indicador, flecha = "N/A", "#888", ""

            # Renderizado optimizado con HTML (3 columnas para incluir el peso)
            st.markdown(f"""
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 5px;">
                <div style="text-align: center;">
                    <span style="font-size: 11px; color: #666;">Creat.</span><br>
                    <span style="font-size: 16px; font-weight: bold;">{crea:.2f} <span style="font-size: 10px;">mg/dL</span></span>
                </div>
                <div style="text-align: center;">
                    <span style="font-size: 11px; color: #666;">Peso</span><br>
                    <span style="font-size: 16px; font-weight: bold;">{peso:.1f} <span style="font-size: 10px;">Kg</span></span>
                </div>
                <div style="text-align: center;">
                    <span style="font-size: 11px; color: #666;">VFG</span><br>
                    <span style="font-size: 16px; font-weight: bold; color: {color_indicador};">{flecha} {vfg:.2f}</span>
                </div>
            </div>
            """, unsafe_allow_html=True)
            
            st.markdown(f"<div style='text-align: center; color: {color_indicador}; font-weight: bold; margin-top: 8px;'>{riesgo_estado}</div>", unsafe_allow_html=True)
            ...
            with st.spinner("Estampando firma del profesional y consolidando documento..."):
                try:
                   ...
                   # =====================================================================
                    ...
                    # 1. Nombres dinámicos (Impresos en la parte superior)
                    pdf.set_font('Arial', '', 8)
                    nombre_paciente_pdf = datos_doc.get('nombre', 'Paciente').strip()
                    nombre_tm_pdf = datos_doc.get('nombre_tm', datos_doc.get('tecnologo_nombre', 'Tecnólogo Médico')).strip()
                    pdf.cell(95, 4, safe_text(nombre_paciente_pdf), 0, 0, 'C')
                    pdf.cell(95, 4, safe_text(nombre_tm_pdf), 0, 1, 'C')
                    
                    # Guardamos la posición Y justo debajo de los nombres para estampar las firmas PNG
                    y_pos_firmas = pdf.get_y()
                    
                    # --- COLUMNA 1: CENTRADO DE FIRMA PACIENTE / TUTOR (IMAGEN PNG) ---
                    ...
                    # --- COLUMNA 2: CENTRADO DE FIRMA PROFESIONAL A CARGO (IMAGEN PNG) ---
                    if 'ruta_tm_local' in locals() and ruta_tm_local and os.path.exists(ruta_tm_local):
                        # Segunda columna: margen inicial 10mm + 95mm + (95mm - 45mm)/2 = 130mm
                        pdf.image(ruta_tm_local, 130, y_pos_firmas, 45, 12)
                    
                    # Ajustamos la posición Y para saltar el espacio que ocupan las imágenes (12mm)
                    pdf.set_y(y_pos_firmas + 13)
                    
                    # 2. Líneas de firma (Debajo de los PNG)
                    pdf.set_font('Arial', '', 8)
                    pdf.cell(95, 4, "________________________________________", 0, 0, 'C')
                    pdf.cell(95, 4, "________________________________________", 0, 1, 'C')
                    
                    # 3. Etiquetas institucionales de firma (En Negrita)
                    ...
                    # 4. Antecedentes R.L y Títulos Profesionales
                    pdf.set_font('Arial', '', 8)
                    ...
                    # 5. Registro SIS dinámico del profesional
                    registro_sis = datos_doc.get('registro_sis', datos_doc.get('sis', '_________')).strip()
                    pdf.cell(95, 4, "", 0, 0, 'C')
                    pdf.cell(95, 4, safe_text(f"Registro SIS: {registro_sis}"), 0, 1, 'C')
