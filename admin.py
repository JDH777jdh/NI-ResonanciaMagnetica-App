...
if datos_doc:
    ...
    # =============================================================================
    with c2:
        # --- A. EVALUACIÓN DE LA FUNCIÓN RENAL (EDICIÓN Y CÁLCULO) ---
with st.expander("🧪 6. EVALUACIÓN DE LA FUNCIÓN RENAL", expanded=True):
    ...
    # Extraer valores de la encuesta (para referencia)
    peso_survey = float(datos_doc.get('peso', 0.0))
    crea_survey = float(datos_doc.get('creatinina', 0.0))
    
    # Inicializar estado si está vacío
    if 'pdf_peso' not in st.session_state: st.session_state.pdf_peso = peso_survey
    if 'pdf_creatinina' not in st.session_state: st.session_state.pdf_creatinina = crea_survey

    # 2. Layout en 3 columnas
    col1, col2, col3 = st.columns(3)
    
    # --- COLUMNA 1: PESO ---
    with col1:
        peso_profesional = st.number_input(
            "Peso (kg):", min_value=0.0, max_value=250.0, 
            value=float(st.session_state.pdf_peso), step=1.0
        )
        st.caption(f"Encuesta: {peso_survey} kg")
        st.session_state.pdf_peso = peso_profesional

    # --- COLUMNA 2: CREATININA ---
    with col2:
        creatinina_profesional = st.number_input(
            "Creatinina (mg/dL):", min_value=0.0, max_value=15.0, 
            value=float(st.session_state.pdf_creatinina), step=0.01
        )
        st.caption(f"Encuesta: {crea_survey} mg/dL")
        st.session_state.pdf_creatinina = creatinina_profesional

    # --- COLUMNA 3: CÁLCULO VFG TIEMPO REAL ---
    with col3:
        ...
        if peso_profesional == 0 or creatinina_profesional == 0:
            color_indicador, riesgo_estado = "#888", "Pendiente"
        elif vfg_dinamico <= 30:
            color_indicador, riesgo_estado = "#FF4B4B", "Alto Riesgo"
        elif vfg_dinamico <= 59:
            color_indicador, riesgo_estado = "#FFA500", "Riesgo Intermedio"
        else:
            color_indicador, riesgo_estado = "#00D26A", "Sin Riesgo"
            
        st.markdown("**V.F.G. Calculado**")
        st.metric(label="Resultado (ml/min)", value=f"{vfg_dinamico:.2f}")
        st.markdown(f"<span style='color:{color_indicador}; font-weight:bold;'>{riesgo_estado}</span>", unsafe_allow_html=True)

    # 3. Guardar estado final para PDF
                ...
                try:
                    ...
                    if vfg_float > 0:
                        ...
                        if is_contraste:
                            ...
                            pdf.data_field("RESULTADO VFG", "__________ ml/min (Estudio sin medio de contraste)", h=5)
