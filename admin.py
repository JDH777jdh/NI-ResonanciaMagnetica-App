...
def filtrar_y_sincronizar_pacientes():
    ...
    if st.button("🔄 Actualizar", use_container_width=True, key="btn_manual_refresh"):
        st.rerun()
    if st.button("🧹 Limpiar Lista", use_container_width=True, key="btn_limpiar_lista"):
        st.session_state.paciente_seleccionado = None
        st.session_state.doc_completo = {}
        # Opcional: Borrar de Firestore los ya validados para no saturar la bandeja
...
if datos_doc:
    ...
    with c2:
            ...
            with st.expander("🧪 6. EVALUACIÓN DE LA FUNCIÓN RENAL", expanded=True):
                ...
                st.markdown("---")
                st.markdown("<span style='font-size: 13px; color: #666;'><b>📊 Referencia de Riesgo y Maduración Renal (VFG)</b></span>", unsafe_allow_html=True)
                tabla_vfg = pd.DataFrame({
                    "Estadio ERC / Maduración": ["Normal / Alto", "Ligero descenso", "Moderado a grave", "Grave", "Fallo Renal"],
                    "VFG (ml/min/1.73m²)": ["≥ 90", "60 - 89", "30 - 59", "15 - 29", "< 15"],
                    "Riesgo Medio de Contraste": ["Sin Riesgo", "Precaución / Bajo", "Riesgo Intermedio", "Alto Riesgo", "Contraindicado"]
                })
                st.table(tabla_vfg)
...
# --- SECCIÓN 7: REGISTRO DE ADMINISTRACIÓN DE CONTRASTE ---
        st.markdown("### 💉 Registro de Administración de Contraste (Sección 7)")

        # 1. Ajustes del Acceso Venoso (Fila superior)
        col_a, col_b, col_c = st.columns(3)
        with col_a:
            acceso_venoso = st.selectbox("Acceso Venoso", ["Bránula", "Mariposa", "CVC", "No aplica"])
        with col_b:
            calibre = st.text_input("Calibre (G)", placeholder="Ej: 20G")
        with col_c:
            sitio_puncion = st.text_input("Sitio de Punción", placeholder="Ej: Pliegue anterior brazo derecho")

        # 2. Encabezados de la Tabla de Contrastes
        c1, c2, c3 = st.columns([2, 1, 1])
        c1.markdown("<b>Medio de Contraste</b>", unsafe_allow_html=True)
        c2.markdown("<b>Administración</b>", unsafe_allow_html=True)
        c3.markdown("<b>Cantidad (ml)</b>", unsafe_allow_html=True)

        # 3. Definición de los elementos (Para facilitar mantenimiento)
        lista_contrastes = [
            {"nombre": "Ac. Gadotérico (Clariscan)", "ruta": "Intravenosa", "key": "vol_clariscan"},
            {"nombre": "Gadopiclenol (Elucirem)", "ruta": "Intravenosa", "key": "vol_elucirem"},
            {"nombre": "Ac. Gadoxético (Primovist)", "ruta": "Intravenosa", "key": "vol_primovist"},
            {"nombre": "Gel de ultrasonido", "ruta": "Intracavitario", "key": "vol_gel"},
            {"nombre": "Contraste neutro (H2O)", "ruta": "Oral", "key": "vol_h2o"},
            {"nombre": "Suero fisiológico (NaCl 0.9%)", "ruta": "Intravenosa", "key": "vol_nacl"}
        ]

        # Inicializamos el diccionario de volúmenes en session_state si no existe
        if 'pdf_volumenes' not in st.session_state:
            st.session_state.pdf_volumenes = {}

        # 4. Generación dinámica de la tabla de inputs
        for item in lista_contrastes:
            r1, r2, r3 = st.columns([2, 1, 1])
            r1.write(item["nombre"])
            r2.write(item["ruta"])
            
            # Input numérico con label invisible (para mantener el diseño limpio)
            valor = r3.number_input(
                f"ml {item['nombre']}", 
                min_value=0.0, 
                step=0.5, 
                key=item["key"], 
            ...
            # Guardamos el valor actual en el diccionario del session_state
            st.session_state.pdf_volumenes[item["nombre"]] = valor

        # 5. Persistencia de variables de acceso (para tu lógica de PDF)
        st.session_state.pdf_acceso = acceso_venoso
        st.session_state.pdf_calibre = calibre
        st.session_state.pdf_sitio = sitio_puncion
