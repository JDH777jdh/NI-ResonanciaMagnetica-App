...
# --- BOTÓN DE CIERRE DE CIRCUITO CLÍNICO ---
        ...
        if canvas_profesional is not None and canvas_profesional.json_data is not None and len(canvas_profesional.json_data["objects"]) > 0:
            with st.spinner("Estampando firma del profesional y consolidando documento..."):
                try:
                    ...
                    # =====================================================================
                    # 📄 GENERACIÓN DEL PDF: IDENTICO A LA APP DE PACIENTES (app.py)
                    # =====================================================================
                    ...
                    datos_doc = doc_completo # Usamos doc_completo que ya tiene todos los campos de la BD
                    ...
                    # OBTENER LA RUTA INTERNA DE LA FIRMA DEL PACIENTE EN STORAGE
                    ...
                    # CLASE FPDF COPIADA EXACTAMENTE DESDE APP.PY
                    ...
                    pdf.cell(0, 5, safe_text(f"Fecha Validación Clínica: {datetime.now(tz_chile).strftime('%d/%m/%Y')}"), 0, 1, 'R') 
                    pdf.ln(2)
                    ...
                    pdf.set_font('Arial', 'B', 9)
                    pdf.cell(18, 5, "Telefono: ", 0, 0)
                    pdf.set_font('Arial', '', 9)
                    pdf.cell(w_col - 18, 5, safe_text(datos_doc.get('telefono', 'No Registrado')), 0, 1)
                    pdf.ln(4)

                    # 2. EVALUACIÓN Y ANAMNESIS (Mapeo de datos guardados en Firestore)
                    pdf.section_title("2", "ANAMNESIS Y EVALUACION CLINICA RM")
                    pdf.set_text_color(0, 0, 0)
                    
                    clinicos = [
                        ("Alergias", datos_doc.get('alergias', 'No')),
                        ("Detalle", datos_doc.get('alergias_detalles', 'N/A')),
                        ("Asma", datos_doc.get('asma', 'No')),
                        ("Diabetes", datos_doc.get('diabetes', 'No')),
                        ("Toma Metformina", datos_doc.get('metformina', 'No')),
                        ("Insuficiencia Renal", datos_doc.get('insuf_renal', 'No')),
                        ("Embarazo", datos_doc.get('embarazo', 'No'))
                    ]

                    col_width = pdf.w / 2.2 
                    for i in range(0, len(clinicos), 2):
                        linea = clinicos[i:i+2]
                        for item, valor in linea:
                            pdf.set_font('Arial', 'B', 8)
                            pdf.cell(35, 4.5, safe_text(f"{item}:"), 0, 0)
                            pdf.set_font('Arial', '', 8)
                            
                            # Destacar positivos en riesgo
                            if valor in ["Sí", "SI", "si"]:
                                pdf.set_text_color(128, 0, 32)
                            else:
                                pdf.set_text_color(0, 0, 0)
                                
                            pdf.cell(col_width - 35, 4.5, safe_text(str(valor)), 0, 0)
                            pdf.set_text_color(0, 0, 0)
                        pdf.ln(4.5) 
                    pdf.ln(2)

                    # 3. FUNCIÓN RENAL (Cálculos directos)
                    pdf.section_title("3", "EVALUACION DE LA FUNCION RENAL")
                    crea = datos_doc.get('creatinina', 'N/A')
                    peso = datos_doc.get('peso', 'N/A')
                    vfg_real = datos_doc.get('vfg', 0.0)

                    pdf.set_font('Arial', '', 9)
                    pdf.data_field("Creatinina", f"{crea} mg/dL")
                    pdf.data_field("Peso", f"{peso} kg")
                    ...
                    if vfg_real > 0:
                        ...
                        pdf.data_field("RESULTADO VFG", "No calculada / Examen sin contraste")
                    pdf.ln(5)

                    # --- PÁGINA 2: TEXTOS LEGALES ---
                    pdf.add_page()
                    ...
                    sections = {
                        "OBJETIVOS": (
                            "La Resonancia Magnetica (RM) es una segura tecnica de Diagnostico, que permite la adquisicion "
                            "de imagenes de gran sensibilidad en todos los planos del espacio de las estructuras del cuerpo. "
                            "Tiene como objetivo obtener Informacion, datos funcionales y morfologicos para detectar precozmente una enfermedad.\n\n"
                            "Para este examen eventualmente se puede requerir la utilizacion de un medio de contraste paramagnetico "
                            "de administracion endovenosa llamado gadolinio, que permite realzar ciertos tejidos del cuerpo para un mejor diagnostico."
                        ),
                        "CARACTERISTICAS": (
                            "La Resonancia utiliza fuertes campos magneticos y ondas de radiofrecuencia, por lo que es muy importante "
                            "dejar fuera de la sala absolutamente todo lo que lleve consigo de tipo metalico y/o electronico (relojes, pulseras, "
                            "telefonos, tarjetas magneticas, etc). Si lleva material de este tipo en su cuerpo (fijaciones dentales, piercings, "
                            "algunos tatuajes, balas o esquirlas metalicas) ciertos tipos de protesis (valvulares, de cadera, de rodilla, "
                            "clips metalicos, etc), o implantes, asi como dispositivos electronicos de caracter medico como bombas de insulina, "
                            "protesis auditivas, marcapasos, desfibriladores, etc. Avisenos, ya que puede contraindicar de manera absoluta la realizacion de este examen.\n\n"
                            "Usted sera posicionado en la camilla del equipo, segun el estudio a realizar y se colocaran cerca de la zona a estudiar "
                            "unos dispositivos (bobinas) que pueden ser de diversos tamanos. Esta exploracion suele ser larga (entre 20 min y 1 hr segun los casos). "
                            "Notara ruido derivado del funcionamiento de la RM (por lo que le facilitaremos unos protectores auditivos), todo esto es normal "
                            "y se le vigilara constantemente desde la sala de control.\n\n"
                            "Es muy importante que permanezca quieto durante el estudio y siga las instrucciones del Tecnologo Medico."
                        ),
                        "POTENCIALES RIESGOS": (
                            "Existe una muy baja posibilidad de que se presente una reaccion adversa al medio de contraste (0.07-2.4%) "
                            "la mayoria de caracter leve fundamentalmente nauseas o cefaleas al momento de la inyeccion.\n\n"
                    ...
                    consentimiento_texto = (
                        ...
                        "dejandolo constatado por escrito y firmado por mi o mi representante.\n\n"
                        "Autorizo la realizacion del procedimiento anteriormente especificado y las acciones que sean necesarias "
                        "en caso de surgir complicaciones durante el procedimiento. Ademas, doy consentimiento para que se administren "
                        "medicamentos y/o infusiones que se requieran para la realizacion de este."
                    )
                    ...
                    # 1. DESCARGA OFICIAL DE FIRMA PACIENTE DESDE STORAGE
                    if ruta_firma_paciente_storage:
                        try:
                            # Accede directo al Bucket nativo de Firebase
                            ...
                            # Estampa firma paciente a la izquierda
                    ...
                    pdf.cell(65, 4, safe_text(f"TM {profesional_nombre} (Reg: {profesional_registro})"), 0, 1, 'C')

                    # Almacenar en la sesión
                    st.session_state.pdf_bytes_data = pdf.output(dest='S').encode('latin-1', 'replace')
                    st.session_state.pdf_filename = f"Consentimiento_Clinico_Validado_{paciente_rut}_{datetime.now(tz_chile).strftime('%Y%m%d')}.pdf"
    ...
    if st.session_state.pdf_ready and st.session_state.pdf_bytes_data is not None:
        ...
        st.write(f"El consentimiento institucional de **{st.session_state.paciente_nombre_val}** ha sido visado con ambas firmas.")
