            ...
            with st.spinner("Estampando firma del profesional y consolidando documento..."):
                try:
                   ...
                   # =====================================================================
                    ...
                    # =====================================================================
                    # ✍️ ESTAMPADO GEOMÉTRICO DE LA DOBLE FIRMA DIGITAL (CORREGIDO)
                    # =====================================================================
                    # Subimos un poco el margen inicial (+16) para dar aire a los nombres sobre la línea
                    y_base_firmas = pdf.get_y() + 16  
                    x_paciente = 15
                    x_profesional = 115
                    ancho_linea = 70

                    # --- 1. RENDERIZADO DE IMÁGENES DE FIRMA (PNG TRANSPARENTE) ---
                    # Se procesan primero para que el texto del nombre pueda montarse encima limpiamente
                    ...
                    # --- 2. IMPRESIÓN DE NOMBRES EN LA PARTE SUPERIOR (FUENTE TAMAÑO 9) ---
                    # Nombre del Paciente (o firmante principal) sobre la línea
                    pdf.set_font('Arial', '', 9)
                    pdf.set_xy(x_paciente + 2, y_base_firmas - 4.5)
                    pdf.cell(ancho_linea, 4, safe_text(paciente_nombre[:40]), 0, 0, 'L')

                    # Nombre del Profesional sobre la línea (¡Eliminado de la parte inferior!)
                    pdf.set_xy(x_profesional + 2, y_base_firmas - 4.5)
                    pdf.cell(ancho_linea, 4, safe_text(profesional_nombre[:40]), 0, 0, 'L')


                    # --- 3. DIBUJADO DE LAS LÍNEAS DE RECUADRO/FIRMA ---
                    ...
                    # --- 4. TEXTOS DESCRIPTIVOS INFERIORES (DEGRADACIÓN DE FUENTE GRADUAL) ---

                    # ----------------------------
                    # 🧑‍ TALÓN IZQUIERDO: PACIENTE / TUTOR
                    # ----------------------------
                    # Nivel 1: Etiqueta del bloque (Tamaño 8 - Negrita)
                    pdf.set_font('Arial', 'B', 8)
                    pdf.set_xy(x_paciente, y_base_firmas + 2)
                    pdf.cell(ancho_linea, 4, safe_text("FIRMA PACIENTE O REPRESENTANTE LEGAL"), 0, 0, 'L')

                    # Nivel 2: Validación de Minoría de Edad (Tamaño 7.5 - Regular)
                    # Si existe un tutor registrado, se asume minoría de edad y se estampa abajo
                    if 'nombre_tutor' in locals() and nombre_tutor:
                        pdf.set_font('Arial', '', 7.5)
                        pdf.set_xy(x_paciente, y_base_firmas + 6)
                        pdf.cell(ancho_linea, 4, safe_text(f"R.L.: {nombre_tutor[:40]}"), 0, 0, 'L')


                    # ----------------------------
                    # 🩺 TALÓN DERECHO: TECNÓLOGO MÉDICO
                    # ----------------------------
                    # Nivel 1: Etiqueta del bloque (Tamaño 8 - Negrita)
                    pdf.set_font('Arial', 'B', 8)
                    pdf.set_xy(x_profesional, y_base_firmas + 2)
                    pdf.cell(ancho_linea, 4, safe_text("FIRMA PROFESIONAL RESPONSABLE"), 0, 0, 'L')

                    # Nivel 2: Título Profesional (Tamaño 7.5 - Regular)
                    pdf.set_font('Arial', '', 7.5)
                    pdf.set_xy(x_profesional, y_base_firmas + 6)
                    pdf.cell(ancho_linea, 4, safe_text("Tecnólogo Médico"), 0, 0, 'L')

                    # Nivel 3: Especialidad Mención (Tamaño 7.0 - Regular)
                    pdf.set_font('Arial', '', 7.0)
                    pdf.set_xy(x_profesional, y_base_firmas + 10)
                    pdf.cell(ancho_linea, 4, safe_text("Esp. Resonancia Magnética"), 0, 0, 'L')

                    # Nivel 4: Identificador institucional (Tamaño 6.5 - Regular)
                    pdf.set_font('Arial', '', 6.5)
                    pdf.set_xy(x_profesional, y_base_firmas + 14)
                    pdf.cell(ancho_linea, 4, safe_text(f"Registro SIS: {profesional_registro}"), 0, 0, 'L')


                    # --- 5. RESTABLECER FLUJO NATURAL DEL PDF ---
                    # Dejamos la coordenada Y lista para cualquier texto que continúe abajo
                    pdf.set_xy(15, y_base_firmas + 20)
                    pdf.ln(2)
