...
if st.session_state.get("doc_completo") is not None:
    ...
    # =========================================================================
    # 🩹 MICRO-CIRUGÍA 1 REPARADA: MAPEO REAL BASADO EN APP.PY / FIRESTORE
    # =========================================================================
    datos_doc = doc_completo # Mantenemos compatibilidad de herencia
    
    # 👤 Demográficos Básicos Directos
    ...
    # 🧲 Bioseguridad Real (Sincronizada con las llaves de tu app.py)
    marcapasos = datos_doc.get('marcapaso', 'No')
    implantes = datos_doc.get('implantes', 'No')
    det_bio = datos_doc.get('bio_detalle', '').strip()

    # 🚨 Triaje de Riesgos Clínicos Reales (Sin prefijos falsos, llaves limpias de app.py)
    clin_ayuno = datos_doc.get('ayuno', 'No')
    clin_asma = datos_doc.get('asma', 'No')
    clin_alergico = datos_doc.get('alergico', 'No')
    clin_hiperten = datos_doc.get('hiperten', 'No')
    clin_hipertiroid = datos_doc.get('hipertiroid', 'No')
    clin_diabetes = datos_doc.get('diabetes', 'No')
    clin_metformina = datos_doc.get('metformina', 'No')
    clin_renal = datos_doc.get('renal', 'No')
    clin_dialisis = datos_doc.get('dialisis', 'No')
    clin_embarazo = datos_doc.get('embarazo', 'No')
    clin_lactancia = datos_doc.get('lactancia', 'No')
    clin_claustro = datos_doc.get('claustrofobia', 'No')
