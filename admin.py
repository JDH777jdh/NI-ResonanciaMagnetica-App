...
def filtrar_y_sincronizar_pacientes():
    ...
    # PRUEBA DE DIAGNÓSTICO (Pon esto en lugar de tu bloque try/except actual)
    try:
        # Consulta de fuerza bruta: trae TODO lo que hay en la colección, sin filtros
        docs_ref = db.collection("encuestas").limit(10).stream() 
        listado_pacientes = []
        for doc in docs_ref:
            data = doc.to_dict()
            listado_pacientes.append({
                ...
                "ESTADO_REAL": data.get("estado_validacion", "CAMPO_NO_EXISTE") # Esto nos dirá qué estado tiene
            })
        
        # Muestra en pantalla lo que encontró para debuguear
        st.write("Datos encontrados en Firestore (sin filtros):", listado_pacientes)
    ...
    if not listado_pacientes:
        st.info("✅ No hay pacientes pendientes de validación en este momento.")
        st.session_state.paciente_seleccionado = None
        st.session_state.doc_completo = None
        
    else:
        # 2. Solo armamos el selector si hay datos
        ...
        # 3. Descarga de datos inmediata según la selección
...
# Fin de la función (mantén tu llamado a filtrar_y_sincronizar_pacientes() abajo)
