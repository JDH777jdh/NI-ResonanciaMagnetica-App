...
# Inicializadores de control global (Obligatorios para evitar pacientes fantasma)
...
# 1. Definimos el fragmento aislado
...
def filtrar_y_sincronizar_pacientes():
    ...
    # Extraer pacientes pendientes de Firebase
    try:
        db = firestore.client()
    ...
    # Si no hay pacientes, limpiamos la pantalla globalmente
    ...
    # Clave dinámica: Cambia cuando presionas "Limpiar Bandeja" para borrar la memoria del widget
    key_dinamica = f"selector_pacientes_async_k_{st.session_state.selector_refresh_key}"

    # Función Callback: Se ejecuta ANTES del rerun para guardar el paciente que elegiste
    def al_cambiar_paciente():
        st.session_state.id_seleccionado_anterior = st.session_state[key_dinamica]
        st.rerun() # Fuerza la actualización del formulario clínico inferior

    # Si el paciente anterior ya no está en la lista (porque se firmó), forzamos al primero de la fila
    idx_actual = 0
    ...
    # Descargar y armar la ficha clínica en memoria del paciente enfocado
...
if st.session_state.get("doc_completo") is not None:
    ...
    if st.session_state.pdf_ready and st.session_state.pdf_bytes_data is not None:
        ...
        if st.button("🧼 LIMPIAR BANDEJA Y CONTINUAR", use_container_width=True):
            st.session_state.selector_refresh_key += 1  # <-- ESTO DESTRUYE AL PACIENTE FANTASMA
            st.session_state.id_seleccionado_anterior = None
            st.session_state.paciente_seleccionado = None
            st.session_state.doc_completo = None
