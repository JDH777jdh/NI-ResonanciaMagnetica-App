        ...
        if canvas_profesional is not None and canvas_profesional.json_data is not None and len(canvas_profesional.json_data["objects"]) > 0:
            with st.spinner("Estampando firma del profesional y consolidando documento..."):
                try:
                    ...
                    if is_contraste:
                        # 🚀 EXTRACCIÓN EN TIEMPO REAL: Clonamos quirúrgicamente los datos validados en pantalla
                        crea_float = st.session_state.get('pdf_creatinina', 0.0)
                        peso_float = st.session_state.get('pdf_peso', 0.0)
                        vfg_float = st.session_state.get('pdf_vfg', 0.0)
                        ...
                        # Acoplamos el rango clínico dinámico evaluando las alertas visuales por color
                        if vfg_float > 0 and peso_float > 0:
                            if vfg_float <= 30.0:
                            ...
                            # 🔥 SOLUCIÓN COLOR: Escribimos el campo saltándonos data_field para que respete el color clínico
                            pdf.set_font('Arial', 'B', 9)
                        ...
                        # 🧠 MEJORA OPERATIVA: Si es un estudio basal pero el profesional igual registró datos, 
                        # los mostramos limpios en lugar de dejar las líneas vacías.
                        crea_basal = st.session_state.get('pdf_creatinina', 0.0)
                        peso_basal = st.session_state.get('pdf_peso', 0.0)
                        
                        txt_crea_b = f"{crea_basal:.2f} mg/dL" if crea_basal > 0 else "__________ mg/dL"
                        txt_peso_b = f"{peso_basal:.1f} kg" if peso_basal > 0 else "__________ kg"
