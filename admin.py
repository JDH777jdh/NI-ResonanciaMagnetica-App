...
from fpdf import FPDF # <--- Importación global para el Motor PDF Institucional Único
...
# 🛠️ CORRECCIÓN: se consolidó aquí el page_title/layout que antes estaba duplicado
#    más abajo (línea ~583 original). Streamlit solo permite UNA llamada a
#    set_page_config() por app; la segunda llamada original habría detenido
#    la ejecución con StreamlitAPIException en cada carga.
st.set_page_config(
    page_title="Panel de Validación Técnica - RM",
    page_icon=img_icono,
    layout="wide"
)
...
def generar_qr_firma_institucional(
    id_verificacion,
    ...
    """
    ═══════════════════════════════════════════════════════════════════════
    MOTOR QR + SHA-256 ÚNICO — NORMA Ley 19.799 + NT N°820 MINSAL
    ═══════════════════════════════════════════════════════════════════════
    Función unificada que reemplaza generar_qr_firma_receta() y
    generar_qr_firma_certificado(). Un solo motor, paramétrico por tipo.

    ► POR QUÉ UNA SOLA FUNCIÓN es conforme a la legislación chilena:
      La Ley 19.799 Art. 2(f) y el Decreto 41 MINSAL NO exigen funciones
      separadas por tipo de documento. Exigen que el hash sea:
        1. Único por documento → el sufijo tipo_doc evita colisiones
        2. Ligado a la autoría → profesional_registro en semilla
        3. Ligado al tiempo   → fecha_str en semilla
        4. Ligado al CONTENIDO CLÍNICO → payload_clinico en semilla ✅
           (sin esto solo tienes "timestamp seal", no "content seal";
            incumple GCL 3.1/3.2 Acreditación MINSAL y Art.12 Ley 20.584)

    ► payload_clinico DEBE incluir los campos que no deben alterarse:
      RUT paciente, nombre, procedimiento, VFG, contraste, etc.
      Si está vacío, el hash es legado (solo autoría+tiempo, sin content seal).

    ► RETORNA: (huella_corta, ruta_png_qr_temporal)
    ═══════════════════════════════════════════════════════════════════════
    """
    SUFIJOS = {
        'RECETA':         'RECETA_MEDICA_ELECTRONICA_NI',
        'CERTIFICADO':    'CERTIFICADO_INSTITUCIONAL_NI',
        'CONSENTIMIENTO': 'CONSENTIMIENTO_INFORMADO_NI',
        'EVENTO':         'EVENTO_SEGURIDAD_GCL23_NI',
        'REPORTE':        'REPORTE_INSTITUCIONAL_NI',
    }
    sufijo = SUFIJOS.get(tipo_doc, str(tipo_doc).upper())

    # ── SEMILLA BLINDADA CON PAYLOAD CLÍNICO (NT N°820 MINSAL) ───────────
    semilla = f"{id_verificacion}|{profesional_registro}|{fecha_str}|{sufijo}"
    if payload_clinico:
        semilla += f"|PAYLOAD_CLINICO:{payload_clinico}"

    hash_completo = hashlib.sha256(semilla.encode('utf-8')).hexdigest().upper()
    huella_corta  = f"{hash_completo[:8]}-{hash_completo[-8:]}"

    URL_BASE = f"https://cdnorteimagen.cl/validar?h={huella_corta}"
    URL_SIS  = f"https://cdnorteimagen.cl/static/certificados_sis/{profesional_registro}.pdf"

    TEXTOS_QR = {
        'RECETA': (
            f"RECETA ELECTRONICA AVANZADA\n"
            f"ID: {id_verificacion}\n"
            f"HUELLA SHA-256: {huella_corta}\n"
            f"VALIDACION: {URL_BASE}"
        ),
        'CERTIFICADO': (
            f"CERTIFICADO DIGITAL NORTE IMAGEN\n"
            f"ID: {id_verificacion}\n"
            f"HUELLA SHA-256: {huella_corta}\n"
            f"VALIDAR: {URL_BASE}\n"
            f"REG. PROFESIONAL: {URL_SIS}"
        ),
        'CONSENTIMIENTO': (
            f"CONSENTIMIENTO INFORMADO DIGITAL\n"
            f"ID: {id_verificacion}\n"
            f"HUELLA SHA-256: {huella_corta}\n"
            f"VALIDAR: {URL_BASE}\n"
            f"EMISOR: CENTRO IMAGENOLOGIA NORTE IMAGEN"
        ),
        'EVENTO': (
            f"EVENTO DE SEGURIDAD CLINICA GCL 2.3\n"
            f"FOLIO: {id_verificacion}\n"
            f"HUELLA SHA-256: {huella_corta}\n"
            f"VALIDAR: {URL_BASE}"
        ),
    }
    texto_qr = TEXTOS_QR.get(
        tipo_doc,
        f"DOCUMENTO NORTE IMAGEN\nID: {id_verificacion}\nHUELLA: {huella_corta}\nVALIDAR: {URL_BASE}"
    )
...
def generar_qr_firma_receta(id_verificacion, medico_rut, fecha_str):
    """Alias de compatibilidad → generar_qr_firma_institucional tipo RECETA."""
    return generar_qr_firma_institucional(id_verificacion, medico_rut, fecha_str, 'RECETA')


def generar_qr_firma_certificado(id_verificacion, profesional_registro, fecha_str):
    """Alias de compatibilidad → generar_qr_firma_institucional tipo CERTIFICADO."""
    return generar_qr_firma_institucional(id_verificacion, profesional_registro, fecha_str, 'CERTIFICADO')


# ─── estampar_sello_criptografico_medico (PRESERVADO EXACTO) ─────────────
...
# =========================================================================
# 🔐 FUNCIONES CRIPTOGRÁFICAS DE CERTIFICADOS (ÁMBITO GLOBAL)
# Elevadas al ámbito global para que Trazabilidad, Bandeja de Firmas y
# cualquier otra sección las use sin duplicarlas.
# La semilla usa |CERTIFICADO_INSTITUCIONAL para que el hash nunca colisione
# con el de una Receta (que usa |RECETA_MEDICA_ELECTRONICA).
# =========================================================================
def estampar_sello_criptografico(pdf_obj, prof_nombre, prof_registro, rol_usuario, huella_corta, ruta_qr_temporal):
    """Renderiza QR + Sello PNG + metadatos del Profesional centrado en la hoja."""
    pdf_obj.ln(15)
    y_pos_firmas = pdf_obj.get_y()
    qr_size = 18
    sello_size = 28
    espacio_entre = 4
    ancho_total_bloque = qr_size + espacio_entre + sello_size
    inicio_x = (210 - ancho_total_bloque) / 2
    qr_x = inicio_x
    ...
    pdf_obj.set_text_color(50, 50, 50)
    pdf_obj.set_y(sello_y + sello_size + 2)
    DICCIONARIO_ROL_CARGO = {
        'tm': 'TECNÓLOGO MÉDICO', 'tm_coordinador': 'TECNÓLOGO MÉDICO COORDINADOR',
        'tens': 'TÉCNICO EN ENFERMERÍA DE NIVEL SUPERIOR', 'secretaria': 'SECRETARIA',
        'calidad': 'ENCARGADA DE CALIDAD', 'owner': 'DIRECCIÓN TÉCNICA',
        'radiologo_coordinador': 'MÉDICO RADIÓLOGO COORDINADOR'
    }
    cargo_mostrado = DICCIONARIO_ROL_CARGO.get(rol_usuario, rol_usuario.upper().replace('_', ' '))
    pdf_obj.set_font('Arial', 'B', 6)
    pdf_obj.set_x(inicio_x)
    pdf_obj.cell(ancho_total_bloque, 3.5, f"VALIDADO Y FIRMADO POR: {prof_nombre.upper()}", 0, 1, 'C')
    pdf_obj.set_font('Arial', '', 5.5)
    pdf_obj.set_x(inicio_x)
    pdf_obj.cell(ancho_total_bloque, 2.5, cargo_mostrado, 0, 1, 'C')
    pdf_obj.set_x(inicio_x)
...
def validar_pin_tm(pin_ingresado, current_user):
    """Valida el PIN ingresado contra todos los campos posibles de la sesión actual."""
    if not pin_ingresado:
        return False
    hash_guardado  = current_user.get('password_hash', '')
    pin_plano      = current_user.get('pin_plano', '')
    password_directo = current_user.get('password', '')
    if hash_guardado and check_password_hash(hash_guardado, pin_ingresado):
        return True
    if pin_plano and pin_ingresado == pin_plano:
        return True
    if password_directo and pin_ingresado == password_directo:
        return True
    return False

# =========================================================================
# 📄 MOTOR DE CUERPO PDF CERTIFICADO ASISTENCIA — FUNCIÓN ÚNICA
# =========================================================================
# Un solo cuerpo de PDF para los tres orígenes posibles:
#   - Certificado 48h (TM firma directo)
#   - Certificado 48h (Secre/TENS → genera borrador o envía a firma)
#   - Reingreso Histórico (TM firma directo / Secre envía a firma)
#
# La diferencia entre los tres es únicamente:
#   - id_verificacion y correlativo (ASIST vs ASIST_HIST → proporcionados al llamar)
#   - fecha_texto_cuerpo (hoy vs fecha histórica → parámetro)
#   - nombre_paciente, rut_paciente, sucursal, procedimientos, horas (parámetros)
#   - modo_firma:  'tm_firma'    → estampa sello criptográfico (TM/Coordinador)
#                 'borrador'     → línea en blanco para firma manual (urgencia)
#                 'pendiente'    → sin sello, datos completos para bandeja de firmas
# =========================================================================
def construir_cuerpo_certificado_asistencia(
    pdf,                  # Objeto PDF_Certificado ya instanciado con id_verificacion
    nombre_paciente,
    rut_paciente,
    sucursal,
    procedimientos,       # lista de strings
    fecha_texto_cuerpo,   # string "DD/MM/YYYY"
    hora_llegada,
    hora_salida,
    incluir_acompanante=False,
    nombre_acompanante="",
    parentesco_acompanante="",
    observaciones="",
    dest_nombre="",       # destinatario (médico derivador)
    dest_cargo="",
    dest_empresa="",
    modo_firma='tm_firma',    # 'tm_firma' | 'borrador' | 'pendiente'
    prof_nombre="",           # para el sello TM
    prof_sis="",
    prof_rol="",
):
    """Construye el CUERPO del certificado de asistencia en el objeto PDF recibido.
    Retorna pdf_bytes listo para descargar.
    Esta función NO instancia ni cierra el PDF — eso lo hace el llamador."""
    pdf.alias_nb_pages()
    pdf.add_page()
    ...
    # Saludo al destinatario (opcional)
    if dest_nombre:
    ...
    if len(procedimientos) > 1:
        texto_principal = (
            f"Se extiende el presente documento para dejar constancia y certificar que el paciente "
            f"{nombre_paciente.upper()}, con número de RUT {rut_paciente.upper()}, asistió a nuestro "
            f"centro diagnóstico ubicado en la sucursal {sucursal.upper()} el día {fecha_texto_cuerpo} "
            f"para realizarse los siguientes estudios:"
        )
    else:
        texto_principal = (
            f"Se extiende el presente documento para dejar constancia y certificar que el paciente "
            f"{nombre_paciente.upper()}, con número de RUT {rut_paciente.upper()}, asistió a nuestro "
            f"centro diagnóstico ubicado en la sucursal {sucursal.upper()} el día {fecha_texto_cuerpo} "
            f"para realizarse el siguiente estudio:"
        )
    ...
    for idx, proc_item in enumerate(procedimientos):
        pdf.cell(15, 7, f" {idx + 1}", 1, 0, 'C', fill=True)
        pdf.cell(145, 7, f" {proc_item.upper()}", 1, 1, 'L', fill=True)
    ...
    if incluir_acompanante and nombre_acompanante:
        ...
        texto_acomp = (
            f"Se deja constancia formal de que el paciente, asistió a su examen acompañado del señor(a) "
            f"{nombre_acompanante.upper()}{txt_par} y representante legal."
        )
        pdf.set_font('Arial', '', 9)
    ...
    if observaciones:
        ...
        pdf.set_font('Arial', '', 9)
        pdf.multi_cell(0, 6, pdf.clean_txt(observaciones.upper()))
        pdf.ln(6)
    ...
    # ── CIERRE SEGÚN MODO ──────────────────────────────────────────────
    if modo_firma == 'tm_firma':
        # TM / Coordinador: sello criptográfico real
        fecha_ahora_str = datetime.now(pytz.timezone('America/Santiago')).strftime("%d/%m/%Y %H:%M:%S")
        huella, ruta_qr = generar_qr_firma_certificado(pdf.id_verificacion, prof_sis, fecha_ahora_str)
        estampar_sello_criptografico(pdf, prof_nombre, prof_sis, prof_rol, huella, ruta_qr)
        try:
            os.unlink(ruta_qr)
        except Exception:
            pass
    elif modo_firma == 'borrador':
        # Secre/TENS urgencia: línea manual para que el TM firme físicamente
        pdf.ln(30)
        pdf.cell(0, 4, "________________________________________", 0, 1, 'C')
        pdf.set_font('Arial', 'B', 8)
        pdf.cell(0, 4, "FIRMA PROFESIONAL A CARGO", 0, 1, 'C')
    # modo 'pendiente' → sin sello, datos completos, para la bandeja de firmas
...
# =========================================================================
# 🔐 MOTOR CRIPTOGRÁFICO AES-256 GCM (Grado Militar) — Ley 19.628
# Idéntico al de app_v2.py: misma llave (st.secrets["aes"]["master_key"]),
# por lo que cualquier payload encriptado aquí puede ser desencriptado por
# el portal de pacientes y viceversa. 100% interoperable.
# =========================================================================
...
# Instancia global única del motor criptográfico (reutilizada en todo el panel)
cripto = GestorCriptografico()

def construir_fhir_validacion_tm(id_documento, paciente_nombre, paciente_rut, profesional_nombre, profesional_registro, hash_firma, huella_corta, fecha_validacion_str, es_adendum=False):
    """HL7 FHIR R4 — Recurso Provenance: deja constancia interoperable de QUIÉN validó y firmó
    electrónicamente la ficha clínica, y CUÁNDO. Se almacena encriptado en AES-256 GCM."""
    return {
        "resourceType": "Provenance",
        "id": id_documento,
        "target": [{"reference": f"QuestionnaireResponse/{id_documento}", "display": paciente_nombre}],
        "recorded": datetime.now(pytz.timezone('America/Santiago')).isoformat(),
        "activity": {
            "coding": [{
                "system": "http://terminology.hl7.org/CodeSystem/v3-DataOperation",
                "code": "AMEND" if es_adendum else "CREATE",
                "display": "Enmienda Ley 20.584" if es_adendum else "Validación Profesional"
            }]
        },
        "agent": [{
            "type": {"coding": [{"system": "http://terminology.hl7.org/CodeSystem/provenance-participant-type", "code": "verifier"}]},
            "who": {"display": profesional_nombre, "identifier": {"value": profesional_registro}}
        }],
        "entity": [{"role": "source", "what": {"display": f"RUT Paciente: {paciente_rut}"}}],
        "signature": [{
            "type": [{"system": "urn:iso-astm:E1762-95:2013", "code": "1.2.840.10065.1.12.1.5", "display": "Verification Signature"}],
            "when": fecha_validacion_str,
            "who": {"display": profesional_nombre},
            "sigFormat": "SHA-256/FES-Ley19799",
            "data": huella_corta
        }],
        "extension": [{"url": "http://minsal.cl/fhir/extension/hash-sha256-completo", "valueString": hash_firma}]
    }
    ...
    try:
        ...
        st.markdown(f'''
            <a href="{data_url}" target="_blank" style="text-decoration:none;">
                <div class="ni-card ni-card-info" style="text-align:center; padding:10px; border-radius:5px; cursor:pointer; margin-bottom:5px;">
...
# --- CONFIGURACIÓN DE PÁGINA --- (ya consolidada al inicio del archivo, línea ~67)
tz_chile = pytz.timezone('America/Santiago')

# =========================================================================
# 🏛️ MOTOR PDF INSTITUCIONAL ÚNICO — Norte Imagen RM
# =========================================================================
# Un solo motor para TODOS los documentos y certificados del sistema:
# Consentimiento, Certificados, Recetas, Eventos de Seguridad, Reportes de
# Insumos y Trazabilidad. Replica exactamente el encabezado, título y pie
# de página del PDF de Consentimiento Validado (la referencia institucional).
#
# Cada subclase específica (PDF_Certificado, PDF_Receta_Professional, etc.)
# solo necesita fijar estos atributos ANTES de pdf.add_page():
#   self.pdf_titulo_lineas = [("LINEA 1", 12), ("LINEA GRANDE FINAL", 16)]
#   self.pdf_subtitulo     = "Texto gris pequeño opcional bajo el título"
#   self.pdf_pie_texto     = "Texto del pie de página (izquierda)"
#   self.pdf_alerta_header = "Texto rojo opcional sobre el título (ej. adendum)"
#   self.pdf_pie_adendum   = (autor, motivo)  # opcional, agrega 2 líneas rojas al pie
# El CUERPO de cada documento (tablas, campos, sellos) no cambia ni una línea.
# =========================================================================
class PDFInstitucionalBase(FPDF):
    ...
    def _resolver(self, valor):
        """Permite que los atributos pdf_* sean texto estático O una función
        sin argumentos (lambda: ...) evaluada en el momento del renderizado.
        Esto preserva el comportamiento de lectura diferida del PDF original
        (ej. self.id_verificacion sobrescrito después de crear el objeto)."""
        return valor() if callable(valor) else valor

    def header(self):
        logo_path = getattr(self, 'pdf_logo_path', 'logoNI.png')
        if os.path.exists(logo_path):
            self.image(logo_path, 11, 11, 30)
        else:
            # Respaldo visual: caja granate con "NI" si el logo no está disponible
            self.set_fill_color(128, 0, 32)
            self.rect(11, 11, 14, 14, 'F')
            self.set_font('Arial', 'B', 8)
            self.set_text_color(255, 255, 255)
            self.text(13.5, 19.5, "NI")

        alerta_header = self._resolver(getattr(self, 'pdf_alerta_header', None))
        if alerta_header:
            self.set_font('Arial', 'B', 9)
            self.set_text_color(255, 0, 0)
            self.cell(0, 5, self.clean_txt(alerta_header), 0, 1, 'R')

        lineas_titulo = self._resolver(getattr(self, 'pdf_titulo_lineas', None)) or [("DOCUMENTO INSTITUCIONAL", 12), ("RESONANCIA MAGNETICA", 16)]
        for texto_linea, tamano in lineas_titulo[:-1]:
            self.set_font('Arial', 'B', tamano)
            self.set_text_color(128, 0, 32)
            self.cell(0, 7, self.clean_txt(texto_linea), 0, 1, 'R')
        texto_final, tamano_final = lineas_titulo[-1]
        self.set_font('Arial', 'B', tamano_final)
        self.set_text_color(128, 0, 32)
        self.cell(0, 8, self.clean_txt(texto_final), 0, 1, 'R')

        subtitulo = self._resolver(getattr(self, 'pdf_subtitulo', None))
        if subtitulo:
            self.set_font('Arial', 'B', 9)
            self.set_text_color(100, 100, 100)
            self.cell(0, 5, self.clean_txt(subtitulo), 0, 1, 'R')

        self.ln(getattr(self, 'pdf_ln_header', 10))

    def footer(self):
        pie_adendum = self._resolver(getattr(self, 'pdf_pie_adendum', None))  # tupla opcional (autor, motivo)
        altura_adicional = 15 if pie_adendum else 0
        self.set_y(-15 - altura_adicional)

        if pie_adendum:
            autor_enmienda, motivo_enmienda = pie_adendum
            self.set_font('Arial', 'B', 7)
            self.set_text_color(255, 0, 0)
            self.cell(0, 3, self.clean_txt(f"ADENDUM LEY 20.584: Este documento fue reabierto y rectificado por {autor_enmienda}."), 0, 1, 'L')
            self.cell(0, 3, self.clean_txt(f"Motivo: {motivo_enmienda}"), 0, 1, 'L')
            self.ln(2)

        self.set_font('Arial', 'I', 7)
        self.set_text_color(150, 150, 150)
        pie_texto = self._resolver(getattr(self, 'pdf_pie_texto', None)) or "Documento Digital Norte Imagen — Resonancia Magnética"
        pie_derecha_attr = getattr(self, 'pdf_pie_derecha', None)
        pie_derecha = self._resolver(pie_derecha_attr) if pie_derecha_attr is not None else f"Página {self.page_no()}/{{nb}}"
        self.cell(0, 10, self.clean_txt(pie_texto), 0, 0, 'L')
        self.cell(0, 10, self.clean_txt(pie_derecha), 0, 0, 'R')
    ...
    def data_field(self, label, value, h=5):
        ...
        self.write(h, f"{self.clean_txt(label)}: ")
        self.set_font('Arial', '', 9)
        self.set_text_color(0, 0, 0)
        self.write(h, f"{self.clean_txt(value)}\n")
...
# =============================================================================
# 🎨 SISTEMA DE DISEÑO DUAL — DARK & LIGHT MODE PROFESIONAL
# =============================================================================
# Usa CSS Custom Properties (variables) que cambian según el
# prefers-color-scheme del navegador/SO del usuario.
# Cada profesional ve automáticamente el modo que tiene configurado.
# El usuario también puede forzar el modo desde "MI PERFIL".
# Paleta granate institucional (128,0,32) se mantiene en ambos modos.
# =============================================================================

# ── Lee preferencia guardada en session_state ─────────────────────────────
_modo_forzado = st.session_state.get("modo_color", "auto")  # 'auto' | 'dark' | 'light'

if _modo_forzado == "dark":
    _attr_color_scheme = "class=\"dark-forced\""
elif _modo_forzado == "light":
    _attr_color_scheme = "class=\"light-forced\""
else:
    _attr_color_scheme = ""  # sigue prefers-color-scheme del SO

st.markdown(f"""
<style>
/* ═══════════════════════════════════════════════════════════════════════════
   VARIABLES LIGHT (por defecto o modo claro forzado)
   ─────────────────────────────────────────────────────────────────────── */
:root, .light-forced {{
  --ni-bg:           #F5F6FA;
  --ni-surface:      #FFFFFF;
  --ni-surface-2:    #EEF0F5;
  --ni-border:       #D9DCE3;
  --ni-shadow:       rgba(0,0,0,0.08);

  --ni-primary:      #800020;          /* Granate institucional */
  --ni-primary-h:    #9A0026;
  --ni-primary-t:    rgba(128,0,32,0.12);
  --ni-accent:       #1A56A0;
  --ni-accent-t:     rgba(26,86,160,0.12);
  ...
  --ni-success:      #0B7A4E;
  --ni-success-bg:   #E6F5EF;
  --ni-warning:      #854D0E;
  --ni-warning-bg:   #FEF9E7;
  --ni-danger:       #B91C1C;
  --ni-danger-bg:    #FDE8E8;
  --ni-info:         #1A56A0;
  --ni-info-bg:      #EBF3FD;

  --ni-sidebar-bg:   #FFFFFF;
  --ni-menu-sel:     #800020;
  --ni-menu-sel-txt: #FFFFFF;
  --ni-menu-txt:     #3D4555;
  --ni-menu-hover:   #EEF0F5;

  --ni-input-bg:     #FFFFFF;
  --ni-input-border: #C4C9D4;
  --ni-input-focus:  #800020;

  --ni-table-hdr:    #EEF0F5;
  --ni-table-row:    #FFFFFF;
  --ni-table-alt:    #F8F9FC;
  --ni-table-border: #E2E5EC;

  --ni-badge-r-bg:   #FDE8E8;
  --ni-badge-r-txt:  #800020;
  --ni-badge-w-bg:   #FEF9E7;
  --ni-badge-w-txt:  #854D0E;
  --ni-badge-g-bg:   #E6F5EF;
  --ni-badge-g-txt:  #0B7A4E;
  --ni-badge-b-bg:   #EBF3FD;
  --ni-badge-b-txt:  #1A56A0;
  ...
  --ni-vfg-ok:       #0B7A4E;
  --ni-vfg-warn:     #854D0E;
  --ni-vfg-danger:   #B91C1C;

  --ni-sello-border: #D0D3DC;
  --ni-firma-area:   #F8F9FC;
}}

/* ═══════════════════════════════════════════════════════════════════════════
   VARIABLES DARK (preferencia del SO O modo oscuro forzado)
   ─────────────────────────────────────────────────────────────────────── */
@media (prefers-color-scheme: dark) {{
  :root {{
    ...
    --ni-surface-2:    #1E2531;
    --ni-border:       #2D3549;
    --ni-shadow:       rgba(0,0,0,0.35);

    --ni-primary:      #C0294A;
    --ni-primary-h:    #D43054;
    --ni-primary-t:    rgba(192,41,74,0.20);
    --ni-accent:       #4A90D9;
    --ni-accent-t:     rgba(74,144,217,0.20);
    ...
    --ni-success:      #2DD4BF;
    --ni-success-bg:   rgba(45,212,191,0.12);
    --ni-warning:      #FBBF24;
    --ni-warning-bg:   rgba(251,191,36,0.12);
    --ni-danger:       #F87171;
    --ni-danger-bg:    rgba(248,113,113,0.12);
    --ni-info:         #60A5FA;
    --ni-info-bg:      rgba(96,165,250,0.12);

    --ni-sidebar-bg:   #111822;
    --ni-menu-sel:     #C0294A;
    --ni-menu-sel-txt: #FFFFFF;
    --ni-menu-txt:     #B0BAC8;
    --ni-menu-hover:   #1E2531;

    --ni-input-bg:     #1A2130;
    --ni-input-border: #2D3549;
    --ni-input-focus:  #C0294A;

    --ni-table-hdr:    #1E2531;
    --ni-table-row:    #161B24;
    --ni-table-alt:    #1A2130;
    --ni-table-border: #2D3549;

    --ni-badge-r-bg:   rgba(248,113,113,0.15);
    --ni-badge-r-txt:  #F87171;
    --ni-badge-w-bg:   rgba(251,191,36,0.15);
    --ni-badge-w-txt:  #FBBF24;
    --ni-badge-g-bg:   rgba(45,212,191,0.15);
    --ni-badge-g-txt:  #2DD4BF;
    --ni-badge-b-bg:   rgba(96,165,250,0.15);
    --ni-badge-b-txt:  #60A5FA;
    ...
    --ni-vfg-ok:       #2DD4BF;
    --ni-vfg-warn:     #FBBF24;
    --ni-vfg-danger:   #F87171;

    --ni-sello-border: #2D3549;
    --ni-firma-area:   #1A2130;
  }}
}}

/* Clase para forzar dark sin depender del SO */
.dark-forced {{
    --ni-bg:           #0D1117;
    --ni-surface:      #161B24;
    --ni-surface-2:    #1E2531;
...
html, body, [data-testid="stAppViewContainer"] {{
    ...
    color: var(--ni-txt-1) !important;
}}
h1, h2, h3, h4, h5, h6 {{ color: var(--ni-txt-1) !important; }}
p, span, label, li, caption, small {{ color: var(--ni-txt-2) !important; }}

/* ═══════════════════════════════════════════════════════════════════════════
   SIDEBAR
   ─────────────────────────────────────────────────────────────────────── */
[data-testid="stSidebar"] {{
    background-color: var(--ni-sidebar-bg) !important;
    border-right: 1px solid var(--ni-border) !important;
}}
[data-testid="stSidebar"] *          {{ color: var(--ni-txt-2) !important; }}
[data-testid="stSidebar"] h1,
[data-testid="stSidebar"] h2,
[data-testid="stSidebar"] h3,
[data-testid="stSidebar"] strong     {{ color: var(--ni-txt-1) !important; }}
[data-testid="stSidebar"] .stDivider {{ border-color: var(--ni-border) !important; }}
...
.stFormSubmitButton > button {{
    background: linear-gradient(135deg, var(--ni-primary), var(--ni-primary-h)) !important;
    color: #fff !important;
    border: none !important;
    border-radius: 7px !important;
    font-weight: 600 !important;
    transition: all 0.22s ease !important;
    box-shadow: 0 2px 6px var(--ni-shadow) !important;
}}
.stButton > button:hover,
.stFormSubmitButton > button:hover {{
    transform: translateY(-2px) !important;
    box-shadow: 0 4px 14px var(--ni-primary-t) !important;
...
.stLinkButton > a {{
    ...
    color: var(--ni-txt-2) !important;
}}
.stLinkButton > a:hover {{
    border-color: var(--ni-primary) !important;
    box-shadow: 0 3px 8px var(--ni-shadow) !important;
...
div[data-baseweb="select"] > div {{
    ...
    border: 1px solid var(--ni-input-border) !important;
...
div[data-baseweb="select"] div {{
    color: var(--ni-txt-1) !important;
    -webkit-text-fill-color: var(--ni-txt-1) !important;
    background-color: transparent !important;
}}
div[data-baseweb="input"] > div:focus-within,
div[data-baseweb="select"] > div:focus-within {{
    border: 1.5px solid var(--ni-input-focus) !important;
    box-shadow: 0 0 0 3px var(--ni-primary-t) !important;
}}
...
div[data-baseweb="popover"] ul {{
    background: var(--ni-surface) !important;
...
div[data-baseweb="popover"] ul li:hover {{
    background: var(--ni-surface-2) !important;
}}
...
button[data-baseweb="tab"] {{
    ...
    color: var(--ni-txt-3) !important;
    border-radius: 6px !important;
    font-weight: 500 !important;
    transition: all 0.18s !important;
}}
button[data-baseweb="tab"][aria-selected="true"] {{
    background: var(--ni-primary) !important;
    color: #fff !important;
    box-shadow: 0 2px 6px var(--ni-primary-t) !important;
}}
...
details > summary:hover {{ background: var(--ni-surface-2) !important; }}
...
[data-testid="stAlert"][data-type="success"] {{
    background: var(--ni-success-bg) !important;
    border-left: 4px solid var(--ni-success) !important;
}}
[data-testid="stAlert"][data-type="warning"] {{
    background: var(--ni-warning-bg) !important;
    border-left: 4px solid var(--ni-warning) !important;
}}
[data-testid="stAlert"][data-type="error"] {{
    background: var(--ni-danger-bg) !important;
    border-left: 4px solid var(--ni-danger) !important;
}}
[data-testid="stAlert"][data-type="info"] {{
    background: var(--ni-info-bg) !important;
    border-left: 4px solid var(--ni-info) !important;
}}
[data-testid="stAlert"] * {{ color: var(--ni-txt-1) !important; }}
...
div[class*="stForm"] {{
    ...
    box-shadow: 0 1px 4px var(--ni-shadow) !important;
    transition: box-shadow 0.2s !important;
}}
[data-testid="stVerticalBlock"] [data-testid="stContainer"]:hover {{
    box-shadow: 0 3px 10px var(--ni-shadow) !important;
}}
...
[data-testid="stMetricLabel"] * {{ color: var(--ni-txt-3) !important; }}
[data-testid="stMetricValue"]  * {{ color: var(--ni-txt-1) !important; }}

/* ═══════════════════════════════════════════════════════════════════════════
   TABLAS / DATAFRAMES
   ─────────────────────────────────────────────────────────────────────── */
[data-testid="stDataFrame"] th {{
    background: var(--ni-table-hdr) !important;
    color: var(--ni-txt-1) !important;
    border-bottom: 1px solid var(--ni-table-border) !important;
}}
[data-testid="stDataFrame"] td {{
    color: var(--ni-txt-2) !important;
    border-bottom: 1px solid var(--ni-table-border) !important;
}}
[data-testid="stDataFrame"] {{ background: var(--ni-surface) !important; }}

/* ═══════════════════════════════════════════════════════════════════════════
   DIVIDERS
   ─────────────────────────────────────────────────────────────────────── */
hr, [data-testid="stDivider"] {{ border-color: var(--ni-border) !important; }}

/* ═══════════════════════════════════════════════════════════════════════════
   RADIO / CHECKBOX / TOGGLE
   ─────────────────────────────────────────────────────────────────────── */
[data-baseweb="radio"] label,
[data-baseweb="checkbox"] label {{ color: var(--ni-txt-2) !important; }}
...
@media screen and (min-width: 768px) {{
    iframe[title*="streamlit_option_menu"] {{ height: 235px !important; }}
}}

/* ═══════════════════════════════════════════════════════════════════════════
   TOKENS SEMÁNTICOS REUTILIZABLES (clases de utilidad)
   Usados en los HTML strings con st.markdown() para que
   los 5 bloques de color inline también cambien con el modo.
   ─────────────────────────────────────────────────────────────────────── */
.ni-card {{
    ...
    padding: 16px 20px;
    margin: 8px 0;
    box-shadow: 0 1px 4px var(--ni-shadow);
    transition: box-shadow 0.2s;
}}
.ni-card:hover {{ box-shadow: 0 3px 10px var(--ni-shadow); }}
.ni-card-danger  {{ border-left: 5px solid var(--ni-danger)  !important; background: var(--ni-danger-bg)  !important; }}
.ni-card-warning {{ border-left: 5px solid var(--ni-warning) !important; background: var(--ni-warning-bg) !important; }}
.ni-card-success {{ border-left: 5px solid var(--ni-success) !important; background: var(--ni-success-bg) !important; }}
.ni-card-info    {{ border-left: 5px solid var(--ni-info)    !important; background: var(--ni-info-bg)    !important; }}
.ni-card-primary {{ border-left: 5px solid var(--ni-primary) !important; background: var(--ni-primary-t)  !important; }}

.ni-badge-r {{ background:var(--ni-badge-r-bg); color:var(--ni-badge-r-txt); padding:2px 10px; border-radius:20px; font-size:0.75rem; font-weight:700; }}
.ni-badge-w {{ background:var(--ni-badge-w-bg); color:var(--ni-badge-w-txt); padding:2px 10px; border-radius:20px; font-size:0.75rem; font-weight:700; }}
.ni-badge-g {{ background:var(--ni-badge-g-bg); color:var(--ni-badge-g-txt); padding:2px 10px; border-radius:20px; font-size:0.75rem; font-weight:700; }}
.ni-badge-b {{ background:var(--ni-badge-b-bg); color:var(--ni-badge-b-txt); padding:2px 10px; border-radius:20px; font-size:0.75rem; font-weight:700; }}

.ni-code {{
    font-family: 'JetBrains Mono', 'Fira Code', 'Courier New', monospace;
    background: var(--ni-code-bg);
    color: var(--ni-code-txt);
    padding: 2px 8px;
    border-radius: 4px;
    font-size: 0.82rem;
}}

.ni-txt-1 {{ color: var(--ni-txt-1) !important; }}
.ni-txt-2 {{ color: var(--ni-txt-2) !important; }}
.ni-txt-3 {{ color: var(--ni-txt-3) !important; }}
.ni-txt-primary {{ color: var(--ni-primary) !important; }}
.ni-txt-success {{ color: var(--ni-success) !important; }}
.ni-txt-warning {{ color: var(--ni-warning) !important; }}
.ni-txt-danger  {{ color: var(--ni-danger)  !important; }}
.ni-txt-info    {{ color: var(--ni-info)    !important; }}

.ni-vfg-ok     {{ color: var(--ni-vfg-ok)     !important; font-weight: 700; }}
.ni-vfg-warn   {{ color: var(--ni-vfg-warn)   !important; font-weight: 700; }}
.ni-vfg-danger {{ color: var(--ni-vfg-danger) !important; font-weight: 700; }}

.ni-firma-area {{
    background: var(--ni-firma-area);
    border: 1px solid var(--ni-sello-border);
    border-radius: 8px;
    padding: 20px;
    text-align: center;
    margin: 12px 0;
...
/* ── Parches para 5 colores hardcodeados que no podían cambiarse antes ── */
/* Enmienda/Adendum banner */
.banner-enmienda {{
    background: var(--ni-warning-bg) !important;
    border-left: 6px solid var(--ni-warning) !important;
    border-radius: 4px;
    padding: 15px;
    margin-bottom: 20px;
}}
.banner-enmienda h4 {{ color: var(--ni-warning) !important; margin: 0; }}
.banner-enmienda p  {{ color: var(--ni-txt-2) !important; margin: 5px 0 0; }}

/* Banner crítico (eventos, modo enmienda) */
.banner-critico {{
    background: var(--ni-danger-bg) !important;
    border-left: 6px solid var(--ni-danger) !important;
    border-radius: 5px;
    padding: 16px;
    margin-bottom: 15px;
}}
.banner-critico h3 {{ color: var(--ni-danger) !important; margin: 0 0 5px; }}
.banner-critico p  {{ color: var(--ni-txt-2) !important; margin: 0; }}

/* Portal links (info cards) */
.portal-card {{
    background: var(--ni-info-bg) !important;
    border-left: 6px solid var(--ni-info) !important;
    border-radius: 5px;
    padding: 16px;
}}
.portal-card p {{ color: var(--ni-txt-2) !important; }}
.portal-card b {{ color: var(--ni-info) !important; }}
</style>
""", unsafe_allow_html=True)
...
if st.session_state.get('authenticated', False) and st.session_state.get('current_user') is not None:
    ...
    if not es_coordinador_o_master():
        ...
        with st.sidebar.expander("👤 MI PERFIL (Seguridad)", expanded=False):
            ...
            # ── MODO DE COLOR ──────────────────────────────────────────────
            st.markdown("---")
            st.markdown("<small><b>🎨 Apariencia de la Interfaz</b></small>", unsafe_allow_html=True)
            st.caption("Elige el modo visual. 'Automático' sigue la configuración de tu sistema operativo.")
            _modo_actual = st.session_state.get("modo_color", "auto")
            _opciones_modo = {"☀️ Modo Claro": "light", "🌙 Modo Oscuro": "dark", "⚙️ Automático (SO)": "auto"}
            _label_actual  = next((k for k, v in _opciones_modo.items() if v == _modo_actual), "⚙️ Automático (SO)")
            _sel_modo = st.selectbox(
                "Modo de color:", list(_opciones_modo.keys()),
                index=list(_opciones_modo.keys()).index(_label_actual),
                key="sb_modo_color", label_visibility="collapsed"
            )
            if _opciones_modo[_sel_modo] != _modo_actual:
                st.session_state["modo_color"] = _opciones_modo[_sel_modo]
                st.rerun()
...
if st.session_state.vista_actual == "principal":
    ...
    if st.session_state.get("modo_enmienda_activo", False):
        ...
        st.markdown(
            f'''
            <div class="banner-enmienda">
                <h4>⚠️ CONTROL ASIGNADO POR MOTOR DE RESCATE</h4>
                <p>
            ...
            with col_botones:
                ...
                if st.button("🗑️ Eliminar", help="Borra forzosamente al paciente actual de la bandeja", width="stretch", key="btn_eliminar_paciente_llena"):
                    if paciente_seleccionado:
                        # 🛠️ CORRECCIÓN: 'nombre_borrado' no estaba definido (NameError al pulsar el botón)
                        nombre_borrado = next((p["Nombre del paciente"] for p in listado_pacientes if p["ID_Documento"] == paciente_seleccionado), "Desconocido")
...
# =============================================================================
    ...
    # =========================================================================
    # 🚀 MOTOR ATÓMICO DE CORRELATIVOS Y TRAZABILIDAD — SISTEMA COMPLETO
    # =========================================================================
    # Cubre TODOS los documentos del sistema bajo un único contador Firebase
    # ('configuracion' > 'contadores_documentos'), inmune a colisiones incluso
    # con múltiples usuarios simultáneos gracias a firestore.Increment().
    #
    # Prefijos institucionales estandarizados:
    #   CERTIFICADOS   → CDARM / CDAHRM / CDSRM
    #   EVENTO SEGUR.  → EVNI
    #   INSUMOS SOL.   → SOLNI
    #   INSUMOS AJUST. → AJUNI
    #   RECETA MEDICA  → RECNI
    ...
    def generar_metadatos_certificado(tipo_doc, db_client, nombre_pac, rut_pac):
        """
        Motor Atómico universal — retorna (correlativo_str, id_verificacion, nombre_archivo).
        tipo_doc acepta: 'ASIST' | 'ASIST_HIST' | 'SUGER' | 'EVENTO' | 'SOL_INSUMO' |
                         'AJU_INSUMO' | 'RECETA' | 'CONSENT' |
                         'REP_INSUMOS' | 'REP_EVENTOS' | 'REP_RECETAS'
        nombre_pac / rut_pac → usados en el nombre del archivo (se pueden pasar vacíos
        para documentos de tipo reporte donde no aplican).
        """
        ref_contador = db_client.collection('configuracion').document('contadores_documentos')
        try:
        ...
        nom_limpio = str(nombre_pac).replace(' ', '_').upper() if nombre_pac else 'SIN_NOMBRE'
        rut_limpio = str(rut_pac).replace('.', '').upper() if rut_pac else 'SR'

        # ── TABLA MAESTRA DE NOMENCLATURA ────────────────────────────
        TABLA = {
            # Certificados de atención (igual que antes, sin cambio visible)
            'ASIST':       (f"CDARM{correlativo_str}",       f"C-ASIST-{nom_limpio}_{rut_limpio}_{correlativo_str}.pdf"),
            'ASIST_HIST':  (f"CDAHRM{correlativo_str}",      f"C-ASIST_HIST-{nom_limpio}_{rut_limpio}_{correlativo_str}.pdf"),
            'SUGER':       (f"CDSRM{correlativo_str}",       f"C-SUGER-{nom_limpio}_{rut_limpio}_{correlativo_str}.pdf"),
            # Evento de Seguridad
            'EVENTO':      (f"EVNI{correlativo_str}",        f"EV-SEG-{nom_limpio}_{correlativo_str}.pdf"),
            # Solicitud de Insumos
            'SOL_INSUMO':  (f"SOLNI{correlativo_str}",      f"SOL-INS-{correlativo_str}.pdf"),
            # Ajuste de Insumos
            'AJU_INSUMO':  (f"AJUNI{correlativo_str}",      f"AJU-INS-{correlativo_str}.pdf"),
            # Receta Médica
            'RECETA':      (f"RECNI{correlativo_str}",       f"REC-MED-{nom_limpio}_{rut_limpio}_{correlativo_str}.pdf"),
            # ── CONSENTIMIENTO INFORMADO ORIGINAL ──────────────────────
            # "REGVALID" = Registro de Validación
            # Contador propio: nunca comparte número con adéndums.
            'CONSENT':     (f"REGVALID{correlativo_str}",    f"CONSENT-{nom_limpio}_{rut_limpio}_{correlativo_str}.pdf"),
            # ── ADÉNDUM / REVALIDACIÓN (Ley 20.584 Enmienda) ──────────
            # Contador COMPLETAMENTE INDEPENDIENTE → REGVALID-AD{000000}
            # Un adéndum NUNCA puede tener el mismo número que el original.
            'CONSENT_AD':  (f"REGVALID-AD{correlativo_str}", f"CONSENT-AD-{nom_limpio}_{rut_limpio}_{correlativo_str}.pdf"),
            # Reportes Mensuales
            'REP_INSUMOS': (f"RINSNI{correlativo_str}",     f"REP-INS-{nom_limpio}_{correlativo_str}.pdf"),
            'REP_EVENTOS': (f"REVNI{correlativo_str}",      f"REP-EV-{nom_limpio}_{correlativo_str}.pdf"),
            'REP_RECETAS': (f"RRECNI{correlativo_str}",     f"REP-REC-{nom_limpio}_{correlativo_str}.pdf"),
        }

        if tipo_doc in TABLA:
            id_verificacion, nombre_archivo = TABLA[tipo_doc]
        else:
    ...
    class PDF_Certificado(PDFInstitucionalBase):
        def __init__(self, tipo_documento, rut_paciente):
            ...
            # 🏛️ Motor PDF Único: mismo encabezado/título/pie que el PDF de Consentimiento Validado
            self.pdf_titulo_lineas = [(self.tipo_documento, 12), ("DOCUMENTO INSTITUCIONAL", 12), ("RESONANCIA MAGNETICA", 16)]
            self.pdf_subtitulo = lambda: f"Fecha de certificado: {self.fecha_emision}"
            self.pdf_pie_texto = lambda: f"Certificado Digital Norte Imagen - RM: {self.fecha_emision} - Paciente RUT: {self.rut_paciente} - VALIDADO TM."
            self.pdf_pie_derecha = lambda: f"Pag. {self.page_no()}/{{nb}} | ID VERIFICACION: {self.id_verificacion}"

    # =========================================================================
    # 🔐 MOTOR CRIPTOGRÁFICO DE FIRMAS (Reemplaza a estampar_firma_tm)
    # =========================================================================
    # ── Las funciones validar_pin_tm(), generar_qr_firma_certificado() y
    # ── estampar_sello_criptografico() son ahora globales (definidas una sola vez
    # ── antes del bloque de vistas). Se usan directamente sin redefinirlas aquí.
                ...
                if st.button("📄 GENERAR CERTIFICADO Y FIRMAR", width="stretch", type="primary", key=f"btn_cert_{paciente_id_cert}"):
                    if not validar_pin_tm(pin_p1_tm, st.session_state.current_user):
                        ...
                        with st.spinner("Compilando documento oficial y estampando Sello Digital..."):
                            ...
                            # 📄 MOTOR ÚNICO: un solo cuerpo para todos los certificados de asistencia
                            pdf_bytes = construir_cuerpo_certificado_asistencia(
                                pdf=pdf,
                                nombre_paciente=registro_sel['nombre'],
                                rut_paciente=registro_sel['rut'],
                                sucursal=suc_48,
                                procedimientos=procs,
                                fecha_texto_cuerpo=datetime.now(tz_chile).strftime("%d/%m/%Y"),
                                hora_llegada=hora_llegada,
                                hora_salida=hora_salida,
                                incluir_acompanante=incluir_acompanante,
                                nombre_acompanante=nombre_acompanante,
                                parentesco_acompanante=parentesco_acompanante,
                                observaciones=glosa_48,
                                dest_nombre=dest_nombre,
                                dest_cargo=dest_cargo,
                                dest_empresa=dest_empresa,
                                modo_firma='tm_firma',
                                prof_nombre=st.session_state.current_user.get('nombre', 'TM'),
                                prof_sis=st.session_state.current_user.get('sis', 'S/R'),
                                prof_rol=str(st.session_state.current_user.get('rol', '')).lower(),
                            )
                ...
                if col_sec1.button("📄 DESCARGAR SIN FIRMA (Borrador)", width="stretch", key=f"btn_sec_nofirma_{paciente_id_cert}"):
                     if hora_llegada and hora_salida:
                        with st.spinner("Compilando documento en blanco para firma manual..."):
                            # 🚀 MOTOR ATÓMICO (Borrador trazable con correlativo propio)
                            ...
                            # 📄 MOTOR ÚNICO: modo 'borrador' → línea manual para urgencias
                            pdf_bytes = construir_cuerpo_certificado_asistencia(
                                pdf=pdf,
                                nombre_paciente=registro_sel['nombre'],
                                rut_paciente=registro_sel['rut'],
                                sucursal=suc_48,
                                procedimientos=procs,
                                fecha_texto_cuerpo=pdf.fecha_emision,
                                hora_llegada=hora_llegada,
                                hora_salida=hora_salida,
                                incluir_acompanante=incluir_acompanante,
                                nombre_acompanante=nombre_acompanante,
                                ...
                                dest_cargo=dest_cargo,
                                dest_empresa=dest_empresa,
                                modo_firma='borrador',  # ← línea manual, sin sello digital
                            )
                    ...
                    if st.session_state.carrito_insumos:
                        ...
                        with col_env:
                            if st.button("🚀 Enviar Pedido Completo", type="primary", width="stretch"):
                                # 🚀 MOTOR ATÓMICO — reemplaza lectura de CSV para correlativo
                                # (el CSV puede tener vacíos, ids de otro formato, o cargarse mal en concurrencia)
                                _, id_bloque, _ = generar_metadatos_certificado(
                                    'SOL_INSUMO', db, suc_sel, ''
                                )
                    ...
                    with sub_ajuste:
                        ...
                        if st.button("⚖️ Forzar Cuadratura", type="primary", width="stretch"):
                            ...
                            if rol_actual != 'owner':
                                ...
                                # 🚀 MOTOR ATÓMICO — reemplaza lectura de CSV para correlativo de ajuste
                                _, id_ajuste, _ = generar_metadatos_certificado(
                                    'AJU_INSUMO', db, ins_ajus, bodega_ajus
                                )
                    ...
                    if meses_disponibles:
                        ...
                        if st.button("🖨️ Generar Reporte PDF (Detallado)", width="stretch", type="primary"):
                            with st.spinner("Compilando PDF de Auditoría Detallada..."):
                                class PDF_Balance_Avanzado(PDFInstitucionalBase):
                                ...
                                # 🚀 MOTOR ATÓMICO — ID único de trazabilidad para este reporte mensual
                                _, id_rep_ins, _ = generar_metadatos_certificado('REP_INSUMOS', db, mes_texto, '')
                                # 🏛️ Motor PDF Único: mismo encabezado/título/pie que el PDF de Consentimiento Validado
                                pdf.pdf_titulo_lineas = [("REPORTE OFICIAL DE TRAZABILIDAD", 12), ("GESTIÓN DE INSUMOS CLÍNICOS", 12), ("RESONANCIA MAGNETICA", 16)]
                                pdf.pdf_subtitulo = f"Período Auditado: {mes_texto}"
                                pdf.pdf_pie_texto = f"Sistema Norte Imagen - RM | ID: {id_rep_ins} | Trazabilidad: {mes_texto} | Generado: {datetime.now(tz_chile).strftime('%d/%m/%Y %H:%M')}"
                                pdf.alias_nb_pages()
                                pdf.add_page()
    ...
    with tab_tens:
        ...
        if rol_usuario not in ["TENS", "TM", "TM_COORDINADOR", "ADMIN", "OWNER"]:
            ...
            if paciente_tens_id:
                ...
                # 📏 BLOQUE ANTROPOMÉTRICO Y DIAGNÓSTICO INTELIGENTE
                st.markdown("#### 📏 Parámetros Clínicos e Información Médica")
                    ...
                    if st.button("📄 EMITIR RECETA Y FIRMAR", type="primary", width="stretch"):
                        if not pin_medico_input:
                            ...
                            with st.spinner("Ejecutando firma criptográfica, adaptando a HL7 FHIR y compilando PDF institucional..."):
                                ...
                                # 1. Motor de Correlativos Inviolables — Atómico Firebase
                                if "correlativo_receta" in datos:
                                    correlativo_id = datos["correlativo_receta"]
                                else:
                                    # 🚀 MOTOR ATÓMICO — reemplaza timestamp fallback (RM{YYMMDDHHmmss})
                                    pac_rec = datos.get('nombre', '')
                                    rut_rec = datos.get('rut', datos.get('run', ''))
                                    _, correlativo_id, _ = generar_metadatos_certificado(
                                        'RECETA', db, pac_rec, rut_rec
                                    )
                                ...
                                # 5. Generación de la Firma Electrónica con PAYLOAD CLÍNICO (NT N°820 MINSAL)
                                _farmacos_str = "|".join([f["nombre"] for f in lista_farmacos_indicados])
                                payload_clinico_receta = (
                                    f"PAC:{p_med.get('nombre','SR')}|RUT:{p_med.get('rut', p_med.get('run','SR'))}|"
                                    f"PROC:{p_med.get('procedimiento','SR')}|FARM:{_farmacos_str}"
                                )
                                huella_crypto, ruta_qr_file = generar_qr_firma_institucional(
                                    correlativo_id, med_rut, fecha_emision_str, 'RECETA', payload_clinico_receta
                                )
                                
                                # =========================================================
                                # 📄 RENDERIZADO DEL PDF (Estructura corporativa intacta)
                                # =========================================================
                                class PDF_Receta_Professional(PDFInstitucionalBase):
                                    def __init__(self, num_correlativo, nombre_medico, registro_sis):
                                        ...
                                        self.RGB_BURDEO = (128, 0, 32)
                                        self.RGB_GRIS_TITULO = (235, 235, 235)
                                        self.RGB_GRIS_CELDA = (248, 248, 248)
                                        # 🏛️ Motor PDF Único: mismo encabezado/título/pie que el PDF de Consentimiento Validado
                                        self.pdf_titulo_lineas = [("RECETA Y CERTIFICADO CLÍNICO", 12), ("DIRECCIÓN MÉDICA INSTITUCIONAL", 12), ("RESONANCIA MAGNETICA", 16)]
                                        self.pdf_subtitulo = "UNIDAD DE RESONANCIA MAGNÉTICA"
                                        self.pdf_pie_texto = lambda: f"ID VERIFICACIÓN: {self.num_correlativo}"
                                ...
                                db.collection("encuestas").document(paciente_med_id).update({
                                    ...
                                    "payload_fhir_auditoria_aes256_gcm": cripto.encriptar(fhir_bundle_receta) # 🔐 AES-256 GCM
                                })
...
# =========================================================================
        ...
        # =========================================================================
        class PDF_Recetas_Institucional(PDFInstitucionalBase):
            def __init__(self, mes_texto, tz_chile, *args, **kwargs):
                super().__init__(*args, **kwargs)
                self.mes_texto = mes_texto
                self.tz_chile = tz_chile
                # 🏛️ Motor PDF Único: mismo encabezado/título/pie que el PDF de Consentimiento Validado
                self.pdf_titulo_lineas = [("REPORTE OFICIAL DE TRAZABILIDAD", 12), ("GESTIÓN DE RECETAS MÉDICAS", 12), ("RESONANCIA MAGNETICA", 16)]
                self.pdf_subtitulo = f"Período Auditado: {mes_texto}"
                self.pdf_pie_texto = lambda: f"Sistema Norte Imagen - RM | Trazabilidad: {self.mes_texto} | Generado: {datetime.now(self.tz_chile).strftime('%d/%m/%Y %H:%M')}"
        ...
        def generar_pdf_reporte_mensual_recetas(datos_tabla):
            ...
            # 🚀 MOTOR ATÓMICO — ID único de trazabilidad para este reporte mensual de recetas
            _, id_rep_rec, _ = generar_metadatos_certificado('REP_RECETAS', db, mes_texto, '')
            pdf = PDF_Recetas_Institucional(mes_texto=mes_texto, tz_chile=tz_chile)
            pdf.pdf_pie_texto = lambda: f"Sistema Norte Imagen - RM | ID: {id_rep_rec} | Trazabilidad: {mes_texto} | Generado: {datetime.now(tz_chile).strftime('%d/%m/%Y %H:%M')}"
            pdf.alias_nb_pages()
            pdf.add_page()
...
# =============================================================================
    ...
    def calcular_edad_exacta(fecha_nac):
        ...
        if dias < 0:
            ...
            dias += calendar.monthrange(ano_anterior, mes_anterior)[1]
    ...
    tab_registro, tab_validacion, tab_historial, tab_reportes = st.tabs([
        "📝 1. Ingresar Evento", 
            ...
            with st.container(border=True):
                ...
                if st.button("💾 GUARDAR INCIDENTE EN BANDEJA", type="primary", width="stretch"):
                    if cat_incidente == "Seleccione Tipo de Evento..." or not desc_narrativa or not medidas_inmediatas:
                        ...
                        folio_id = f"RM-EV-{prefijo}-{sufijo}"   # ← conservado como folio legible
                        # 🚀 MOTOR ATÓMICO: ID de trazabilidad único, inmune a colisiones
                        corr_ev, id_ver_ev, _ = generar_metadatos_certificado(
                            'EVENTO', db, nombre_paciente, rut_paciente
                        )
                        ...
                        doc_evento = {
                            ...
                            "correlativo": corr_ev,       # 🚀 MOTOR ATÓMICO — trazabilidad única
                            "id_verificacion": id_ver_ev  # 🚀 MOTOR ATÓMICO — ID institucional
                        }
                        # 🔐 HL7 FHIR R4 AdverseEvent — Encriptado AES-256 GCM (GCL 2.3 MINSAL)
                        doc_evento["fhir_adverse_event_aes256_gcm"] = cripto.encriptar(exportar_a_fhir_adverse_event(doc_evento))

                        db.collection("eventos_seguridad").document(folio_id).set(doc_evento)
                        # 🛠️ CORRECCIÓN: se eliminó una escritura duplicada idéntica a la línea anterior
                        ...
                        with st.container(border=True):
                            ...
                            with c_v2:
                                ...
                                if st.button("✅ Validar", key=f"val_{ev.get('folio', '')}", use_container_width=True):
                                    # 🛠️ CORRECCIÓN DE SEGURIDAD: el check anterior comparaba pin_firma contra
                                    # sí mismo cuando el usuario no tenía campo 'pin' (validaba con PIN en blanco).
                                    # Ahora reutiliza validar_pin_medico(), la misma función que ya usan
                                    # Certificados y Fármacos.
                                    if validar_pin_medico(pin_firma, st.session_state.current_user):
                                        fecha_val_evento = datetime.now(tz_chile).strftime("%d/%m/%Y %H:%M:%S")
                                        # ── PAYLOAD CLÍNICO BLINDADO (GCL 2.3 + NT N°820 MINSAL) ──────────
                                        payload_clinico_ev = (
                                            f"RUT:{ev.get('rut_paciente','SR')}|"
                                            f"PAC:{ev.get('nombre_paciente','SR')}|"
                                            f"CAT:{ev.get('categoria_incidente','SR')}|"
                                            f"ZONA:{ev.get('zonificacion','SR')}|"
                                            f"FECHA_EV:{ev.get('fecha_evento','SR')}"
                                        )
                                        huella_ev, ruta_qr_ev = generar_qr_firma_institucional(
                                            ev['folio'],
                                        ...
                                        hash_firma_ev = hashlib.sha256(
                                            f"{ev['folio']}|{st.session_state.current_user['nombre']}|{fecha_val_evento}|EVENTO_SEGURIDAD_GCL23_NI|PAYLOAD_CLINICO:{payload_clinico_ev}".encode('utf-8')
                                        ).hexdigest().upper()
                                        huella_ev = f"{hash_firma_ev[:8]}-{hash_firma_ev[-8:]}"
                                        fhir_validacion_ev = construir_fhir_validacion_tm(
                                            id_documento=ev['folio'], paciente_nombre=ev.get('nombre_paciente', ''),
                                            paciente_rut=ev.get('rut_paciente', ''),
                                            profesional_nombre=st.session_state.current_user['nombre'],
                                            profesional_registro=st.session_state.current_user.get('sis', 'S/R'),
                                            hash_firma=hash_firma_ev, huella_corta=huella_ev,
                                            fecha_validacion_str=fecha_val_evento
                                        )
                                        db.collection("eventos_seguridad").document(ev['folio']).update({
                                            ...
                                            "fecha_validacion": fecha_val_evento,
                                            "huella_sha256_validacion": huella_ev,          # 🆕 FES Ley 19.799
                                            "payload_clinico_hash": payload_clinico_ev,     # 🆕 Content seal NT N°820
                                            "fhir_validacion_aes256_gcm": cripto.encriptar(fhir_validacion_ev)  # 🔐 AES-256
                                        })
                                        st.rerun()
                                    else:
            ...
            if not lista_historial:
                ...
                for ev in sorted(lista_filtrada, key=lambda x: x['fecha_hora_sistema'], reverse=True):
                    with st.container(border=True):
                        ...
                        c_h1.write(f"**Tipo:** {ev['categoria_incidente']} | **Fecha Registro:** {ev['fecha_hora_sistema']}")
                        ...
                        with c_h2:
                            if st.button("📄 Generar PDF", key=f"pdf_{ev['folio']}", width="stretch"):
                                with st.spinner("Compilando Documento e Inyectando Firma..."):
                                    ...
                                    class PDF_Incidente_Institucional(PDFInstitucionalBase):
                                        def __init__(self, ev, tz_chile, *args, **kwargs):
                                            super().__init__(*args, **kwargs)
                                            self.ev = ev
                                            self.tz_chile = tz_chile
                                            # 🏛️ Motor PDF Único: mismo encabezado/título/pie que el PDF de Consentimiento Validado
                                            self.pdf_titulo_lineas = [("REPORTE OFICIAL DE INCIDENTE", 12), ("DEPARTAMENTO DE CALIDAD Y SEGURIDAD DEL PACIENTE", 12), ("RESONANCIA MAGNETICA", 16)]
                                            self.pdf_subtitulo = lambda: f"FOLIO TRAZABILIDAD: {self.ev['folio']}"
                                            self.pdf_ln_header = 5  # el cuerpo se fija a Y=45 vía set_margins más abajo
                                            self.pdf_pie_texto = lambda: f"Centro de Imagenología RM | Documento Autenticado Electrónicamente | Folio: {self.ev['folio']} | Descarga: {datetime.now(self.tz_chile).strftime('%d/%m/%Y %H:%M:%S')}"
                                    ...
                                    detalles = [
                                        ...
                                        ("Zonificación Bioseguridad:", f"{ev['zonificacion']} - {ev.get('ubicacion_especifica', 'N/A')}"),
                                    ...
                                    for label, value in detalles:
                                        # ESCALA DE GRISES PURA
                                        if alternar_sombreado: pdf.set_fill_color(240, 240, 240)
                                        else: pdf.set_fill_color(250, 250, 250)
                                        
                                        pdf.set_text_color(40, 40, 40)
                                        pdf.set_font('Arial', 'B', 8.5)
                                        pdf.cell(50, 6.5, pdf.clean_txt(f" {label}"), 1, 0, 'L', fill=True)
                                        
                                        pdf.set_text_color(0, 0, 0)
                                        pdf.set_font('Arial', '', 8.5)
                                        pdf.cell(124, 6.5, pdf.clean_txt(f" {value}"), 1, 1, 'L', fill=True)
                                        alternar_sombreado = not alternar_sombreado
                                    ...
                                    if ev.get('validado_por'):
                                        ...
                                        pdf.set_font('Arial', 'B', 8)
                                        pdf.set_text_color(0, 0, 0)
                                        pdf.cell(0, 4, pdf.clean_txt(f"VALIDADO POR: {ev['validado_por'].upper()}"), 0, 1, 'C')
                                        
                                        pdf.set_font('Arial', '', 7.5)
                                        rol_raw = ev.get('rol_validador', 'calidad').lower()
                                        rol_imprimir = diccionario_roles.get(rol_raw, rol_raw).upper()
                                        
                                        pdf.cell(0, 4, pdf.clean_txt(f"ROL: {rol_imprimir}"), 0, 1, 'C')
                                        pdf.cell(0, 4, pdf.clean_txt("ESPECIALIDAD RESONANCIA MAGNÉTICA"), 0, 1, 'C')
                                        pdf.set_font('Arial', 'B', 7)
                                        pdf.set_text_color(80, 80, 80)
                                        pdf.cell(0, 4, pdf.clean_txt(f"HUELLA SHA-256: {hash_digital}"), 0, 1, 'C')
        ...
        if not lista_rep:
            ...
            if st.button("🖨️ EMITIR Y FIRMAR CERTIFICADO PDF", type="primary", use_container_width=True):
                with st.spinner("Compilando Reporte Institucional..."):
                    ...
                    class PDF_Mensual_Institucional(PDFInstitucionalBase):
                        ...
                        def print_event(self, ev):
                            ...
                            self.cell(45, 6, str(ev.get('fecha_hora_sistema', '')), 1, 0, 'L', fill=True)
                            self.set_font('Arial', 'B', 8)
                            self.cell(20, 6, "Folio:", 1, 0, 'L', fill=True)
                            self.set_font('Arial', '', 8)
                            self.cell(77, 6, str(ev.get('folio', '')), 1, 1, 'L', fill=True)
                            
                            self.set_font('Arial', 'B', 8)
                            self.cell(32, 6, "Paciente:", 1, 0, 'L', fill=True)
                            self.set_font('Arial', '', 8)
                            paciente_texto = ev.get('nombre_paciente', 'S/I').encode('latin-1', 'replace').decode('latin-1')
                            self.cell(142, 6, paciente_texto[:60], 1, 1, 'L', fill=True)
                            
                            self.set_font('Arial', 'B', 8)
                            self.cell(32, 6, "Incidente:", 1, 0, 'L', fill=True)
                            self.set_font('Arial', '', 8)
                    ...
                    # INSTANCIACIÓN CON MOTOR ATÓMICO
                    # 🚀 MOTOR ATÓMICO — ID único de trazabilidad para este reporte mensual de eventos
                    _, id_rep_ev, _ = generar_metadatos_certificado('REP_EVENTOS', db, f"{mes_sel}-{ano_sel}", '')
                    pdf_rep = PDF_Mensual_Institucional()
                    # 🏛️ Motor PDF Único: mismo encabezado/título/pie que el PDF de Consentimiento Validado
                    pdf_rep.pdf_titulo_lineas = [("CONSOLIDADO ESTADÍSTICO GCL 2.3", 12), ("DEPARTAMENTO DE CALIDAD Y SEGURIDAD DEL PACIENTE", 12), ("RESONANCIA MAGNETICA", 16)]
                    pdf_rep.pdf_subtitulo = f"PERIODO: {mes_sel}/{ano_sel} | ID Reporte: {id_rep_ev}"
                    pdf_rep.pdf_pie_texto = lambda: f"Centro de Imagenología RM | ID: {id_rep_ev} | Reporte Extraído Electrónicamente | Descarga: {datetime.now(tz_chile).strftime('%d/%m/%Y %H:%M:%S')}"
...
if datos_doc.get("es_enmienda"):
    st.markdown(f'''
        <div class="banner-critico">
            <h3>🛑 MODO ENMIENDA LEY 20.584 ACTIVO</h3>
            <p>
...
with c1:
    with st.expander("👤 1. FICHA CLÍNICA: DATOS DEL PACIENTE", expanded=True):
        ...
        if es_menor_de_edad:
            ...
            st.markdown(f'''
                <div class="portal-card" style="padding:16px; border-radius:5px; margin-bottom:15px;">
                    <p style="margin:0 0 12px 0; font-weight:bold; font-size:16px;" class="ni-txt-info">
                        ⚠️ Datos del Representante Legal
                    </p>
                    <div style="display:flex; flex-wrap:wrap; gap:20px; font-size:15px;" class="ni-txt-2">
                        <div style="flex:1; min-width:250px;">
                            <b>Nombre:</b> {nombre_t} <span class="ni-txt-3">({parentesco_t})</span>
                        </div>
                        <div style="flex:1; min-width:250px;">
                            {documento_tutor}
                        </div>
        ...
        if not acceso_concedido:
            ...
            with st.spinner("Generando Hash Criptográfico, compilando Código QR y Sello Institucional..."):
                try:
                    ...
                    # ── PAYLOAD CLÍNICO BLINDADO (Ley 20.584 Art.12 + NT N°820 MINSAL) ──
                    # El hash ahora amarra el CONTENIDO clínico, no solo la autoría y el tiempo.
                    # Si alguien altera el PDF impreso (procedimiento, VFG, contraste, RUT),
                    # el hash de verificación ya no coincidirá → documento inválido.
                    _pac_rut_hash    = str(datos_doc.get('rut', datos_doc.get('run', 'SR')))
                    _pac_nombre_hash = str(datos_doc.get('nombre', 'PACIENTE'))
                    _proc_hash       = str(procedimiento_oficial)
                    _contraste_hash  = str(tiene_contraste_real)
                    _vfg_hash        = str(round(float(st.session_state.get('pdf_vfg', 0.0)), 2))
                    _creat_hash      = str(round(float(st.session_state.get('pdf_creatinina', 0.0)), 2))
                    payload_clinico_consent = (
                        f"RUT:{_pac_rut_hash}|PAC:{_pac_nombre_hash}|"
                        f"PROC:{_proc_hash}|CONTR:{_contraste_hash}|"
                        f"VFG:{_vfg_hash}|CREAT:{_creat_hash}"
                    )
                    # Usamos el motor unificado con payload clínico completo
                    huella_corta, ruta_qr_consent = generar_qr_firma_institucional(
                        id_verificacion=id_documento_paciente,
                        profesional_registro=profesional_registro,
                        fecha_str=fecha_validacion_str,
                        tipo_doc='CONSENTIMIENTO',
                        payload_clinico=payload_clinico_consent
                    )
                    hash_firma = hashlib.sha256(
                        f"{id_documento_paciente}|{profesional_registro}|{fecha_validacion_str}|{texto_adendum}|PAYLOAD_CLINICO:{payload_clinico_consent}".encode('utf-8')
                    ).hexdigest().upper()
                    huella_corta = f"{hash_firma[:8]}-{hash_firma[-8:]}"
                    # El QR ya fue generado por generar_qr_firma_institucional() arriba.
                    # ruta_qr_consent → ruta_qr_temporal para compatibilidad con el código PDF.
                    ruta_qr_temporal = ruta_qr_consent
                    ...
                    # 🔐 HL7 FHIR R4 Provenance — Constancia interoperable de la firma del TM, encriptada AES-256 GCM
                    fhir_validacion_tm = construir_fhir_validacion_tm(
                        id_documento=id_documento_paciente,
                        ...
                        huella_corta=huella_corta,
                        fecha_validacion_str=fecha_validacion_str,
                        es_adendum=es_adendum
                    )
                    ...
                    db.collection("encuestas").document(id_documento_paciente).update({
                        ...
                        "fhir_validacion_aes256_gcm": cripto.encriptar(fhir_validacion_tm)  # 🔐 AES-256 GCM
                    })
                    ...
                    class PDF_Institucional(PDFInstitucionalBase):
                        def __init__(self, p_nombre, p_rut, p_ip, f_val):
                            ...
                            self.p_ip = p_ip
                            self.f_val = f_val
                            # 🏛️ Motor PDF Único — esta es la clase de referencia institucional;
                            # las demás clases PDF replican exactamente este encabezado/título/pie.
                            self.pdf_titulo_lineas = [("ENCUESTA DE RIESGOS ASOCIADOS Y", 12), ("CONSENTIMIENTO INFORMADO", 12), ("RESONANCIA MAGNETICA", 16)]
                            self.pdf_alerta_header = self._calcular_alerta_header
                            self.pdf_pie_adendum = self._calcular_pie_adendum
                            self.pdf_pie_texto = self._calcular_pie_texto

                        def _es_adendum(self):
                            return bool(hasattr(self, 'datos_doc') and self.datos_doc.get('adendum_texto'))

                        def _calcular_alerta_header(self):
                            return 'DOCUMENTO RECTIFICADO / ADENDUM CLÍNICO' if self._es_adendum() else None

                        def _calcular_pie_adendum(self):
                            if not self._es_adendum():
                                return None
                            ...
                            return (autor_enmienda, motivo_enmienda)

                        def _calcular_pie_texto(self):
                            ...
                            estado_val = "REVALIDADO TM" if self._es_adendum() else "VALIDADO TM"
                            return f"Certificado Digital Norte Imagen - RM: {self.f_val} - ID Registro: {id_registro} - {estado_val}."
                    ...
                    # 🚀 MOTOR ATÓMICO — ID de trazabilidad único por tipo de documento
                    # REGVALID{N}      → consentimiento original
                    # REGVALID-AD{N}   → adéndum / revalidación Ley 20.584 (contador independiente)
                    tipo_consent = 'CONSENT_AD' if es_adendum else 'CONSENT'
                    corr_cons, id_ver_cons, nombre_arch_cons = generar_metadatos_certificado(
                        tipo_consent, db, paciente_nombre, paciente_rut
                    )
                    ...
                    pdf.id_verificacion_consent = id_ver_cons   # 🚀 MOTOR ATÓMICO — disponible en footer/sello

                    # 🚀 MOTOR ATÓMICO — persiste ID de trazabilidad del consentimiento en Firestore
                    db.collection("encuestas").document(id_documento_paciente).update({
                        "correlativo_consent": corr_cons,
                        "id_verificacion_consent": id_ver_cons
                    })
