...
if st.session_state.get("doc_completo") is not None:
    ...
    # =========================================================================
    # 🩹 EXTRACCIÓN Y SANITIZACIÓN QUIRÚRGICA DE VARIABLES DEMOGRÁFICAS
    # =========================================================================
    datos_doc = doc_completo # Clonamos para compatibilidad con tu HTML de la columna 2
    ...
    # Asegurar formato string de la fecha de nacimiento si viene como objeto datetime
    ...
    with c1:
        ...
        st.write(f"**Creatinina:** {datos_doc.get('creatinina', 'N/A')} mg/dL")
        st.write(f"**Peso:** {datos_doc.get('peso', 'N/A')} Kg")
    ...
    with c2:
        ...
        # Ahora este bloque f-string ejecutará a la perfección porque las variables ya existen
        st.markdown(f"""
    ...
    with c2:
        # --- 1. TARJETA DE INFORMACIÓN DE FILTRADO Y EXAMEN ---
        # Aseguramos que las variables de contraste y procedimiento se extraigan limpiamente
        is_contraste_visual = doc_completo.get('tiene_contraste', False) in [True, "Sí", "SI", "si", "Si"]
        procedimiento_val_visual = doc_completo.get('procedimiento', 'No especificado')
        ...
        with st.expander("🧲 SECCIÓN 2: AUDITORÍA DE BIOSEGURIDAD MAGNÉTICA", expanded=True):
            col_bio1, col_bio2 = st.columns(2)
            
            with col_bio1:
                # Lectura estricta de las respuestas de bioseguridad desde Firestore
                marcapasos = datos_doc.get('bio_marcapasos', 'No')
                implantes = datos_doc.get('bio_implante', 'No')
                
                # Semáforo clínico visual para el Tecnólogo Médico
            ...
            with col_bio2:
                # Caja de texto libre para observaciones de objetos metálicos
                st.markdown("**Detalle de Bioseguridad / Comentarios del Paciente:**")
        ...
        # Si el examen actual de la bandeja NO es de contraste, forzamos que en el estado de la app el peso sea 0
