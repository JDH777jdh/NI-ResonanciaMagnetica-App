            ...
            with st.spinner("Estampando firma del profesional y consolidando documento..."):
                try:
                   ...
                   # =====================================================================
                    ...
                    # --- COLUMNA 2: CENTRADO DE FIRMA TECNÓLOGO MÉDICO ---
                    # (Reemplazar 'ruta_tm_local' por la variable exacta de la firma del profesional si aplica)
                    ...
                    # Ajustamos posición para las líneas de base
                    pdf.set_y(y_pos_firmas + 13)
                    
                    # Líneas de firma perfectamente simétricas y centradas en su bloque
                    pdf.cell(95, 4, "________________________________________", 0, 0, 'C')
                    pdf.cell(95, 4, "________________________________________", 0, 1, 'C')
                    
                    # Etiquetas institucionales de firma
                    pdf.set_font('Arial', 'B', 8)
                    pdf.cell(95, 4, safe_text("FIRMA PACIENTE O REPRESENTANTE LEGAL"), 0, 0, 'C')
                    pdf.cell(95, 4, safe_text("FIRMA TECNÓLOGO MÉDICO"), 0, 1, 'C')
                    
                    # LÓGICA ESTRICTA: Despliegue del Representante Legal bajo la firma si existe
                    pdf.set_font('Arial', '', 8)
                    nombre_tutor_pdf = datos_doc.get('nombre_tutor', '').strip()
                    rut_tutor_pdf = datos_doc.get('rut_tutor', '').strip()
                    
                    if nombre_tutor_pdf:
                        texto_tutor_footer = f"Rep: {nombre_tutor_pdf} (RUT: {rut_tutor_pdf})"
                        pdf.cell(95, 4, safe_text(texto_tutor_footer), 0, 0, 'C')
                    else:
                        pdf.cell(95, 4, "", 0, 0, 'C')
                        
                    pdf.cell(95, 4, safe_text("Validación Técnica Norte Imagen"), 0, 1, 'C')
                    pdf.ln(4)
