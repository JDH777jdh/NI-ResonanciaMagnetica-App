        ...
        if canvas_profesional is not None and canvas_profesional.json_data is not None and len(canvas_profesional.json_data["objects"]) > 0:
            with st.spinner("Estampando firma del profesional y asegurando registro..."):
                try:
                    ...
                    # =====================================================================
                    # 🛠️ SOLUCIÓN REAL: Mensaje unificado usando la variable 'doc'
                    # =====================================================================
                    st.success(f"🎉 ¡Consentimiento Validado con Éxito! Paciente {doc.to_dict().get('nombre', 'Seleccionado')} visado correctamente por TM {profesional_nombre}.")
                    st.balloons()
                    
                    # ⏳ Pausa de 3 segundos para que el Tecnólogo alcance a ver la confirmación y los globos
                    import time
                    time.sleep(3.0)
