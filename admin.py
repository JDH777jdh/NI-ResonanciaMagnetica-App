...
if st.session_state.get("doc_completo") is not None:
    ...
    # ---------------------------------------------------------------------
    # INSERTA EL BLOQUE DE CÁLCULO DINÁMICO:
    # ---------------------------------------------------------------------
    es_pediatrico = int(datos_doc.get('edad', 0)) < 18
    
    if es_pediatrico:
        if talla_profesional > 0 and creatinina_profesional > 0:
            vfg_dinamico = round((0.413 * talla_profesional) / creatinina_profesional, 2)
        else:
            vfg_dinamico = 0.0
    else:
        if peso_profesional > 0 and creatinina_profesional > 0:
            es_mujer = 'fem' in str(datos_doc.get('sexo', datos_doc.get('genero', 'M'))).lower() or datos_doc.get('es_mujer', False)
            vfg_bruto = ((140 - int(datos_doc.get('edad', 0))) * peso_profesional) / (72 * creatinina_profesional) * (0.85 if es_mujer else 1.0)
            vfg_dinamico = round(vfg_bruto, 2)
        else:
            vfg_dinamico = 0.0

    # --- AHORA ACTUALIZAS EL ESTADO PARA QUE EL PDF USE ESTE VALOR ---
    st.session_state.doc_completo = datos_doc
    st.session_state.doc_completo["vfg_resultado"] = vfg_dinamico
    # ---------------------------------------------------------------------
...
if datos_doc:
    ...
    with c1:
        ...
        # Mapeo avanzado de Identidad y Sexo Registral / Biológico
        idx_gen = str(datos_doc.get('genero_idx', '0'))
        idx_bio = str(datos_doc.get('sexo_bio_idx', '0'))
        
        # Intentar extraer el texto directo del OCR como respaldo
        ocr_bio = str(datos_doc.get('genero_biologico', '')).strip().capitalize()
        
        if idx_gen == "1":
            sexo_paciente = "Femenino"
        elif idx_gen == "2" or str(datos_doc.get('sexo')) == "No binario":
            # Si el índice de Streamlit marca No Binario, calculamos el sexo asignado al nacer
            if ocr_bio in ["Masculino", "Femenino"]:
                sexo_bio_str = ocr_bio
            else:
                sexo_bio_str = "Femenino" if idx_bio == "1" else "Masculino"
            
            sexo_paciente = f"No binario (Bio: {sexo_bio_str})"
        else:
        ...
        with st.expander("🧲 2. BIOSEGURIDAD MAGNÉTICA", expanded=True):
            ...
            if tiene_implantes:
                ...
                for implante in lista_implantes:
                    ...
                    clasificacion_actual = st.session_state[key_estado]
    ...
    with c2:
        ...
        with st.expander("🧪 6. EVALUACIÓN DE LA FUNCIÓN RENAL", expanded=True):
           ...
           # 2. Celdas de edición interactiva para el profesional
            ...
            # 1. Definimos es_pediatrico para control de interfaz
            es_pediatrico = (int(datos_doc.get('edad_num', 40)) < 18)

            with col_p:
                peso_base = float(datos_doc.get('peso', 0.0))
                # SI ES PEDIÁTRICO, EL PESO SE DESHABILITA Y SE FUERZA A 0.0
                peso_profesional = st.number_input(
                    "Peso (kg):",
                    min_value=0.0, max_value=250.0, 
                    value=0.0 if es_pediatrico else peso_base, 
                    step=1.0,
                    disabled=es_pediatrico,  # <--- BLOQUEO DE EDICIÓN EN NIÑOS
                    help="Deshabilitado en pacientes pediátricos." if es_pediatrico else "Deje en 0.0 para estudios basales."
                )
            ...
            with col_t:
                talla_base = float(datos_doc.get('talla', 0.0))
                # SI ES ADULTO, LA TALLA SE DESHABILITA
                talla_profesional = st.number_input(
                    "Talla (cm):",
                    min_value=0.0, max_value=250.0, value=talla_base, step=1.0,
                    disabled=not es_pediatrico, # Solo habilitado si es pediatrico
                    help="Obligatorio para pediatría."
                )

            # 3. Lógica de cálculo estricta (Evita contaminación de datos)
            vfg_dinamico = 0.0
            
            if creatinina_profesional > 0:
                if es_pediatrico:
                    if talla_profesional > 0:
                        vfg_dinamico = round((0.413 * talla_profesional) / creatinina_profesional, 2)
                else:
                    if peso_profesional > 0:
                        # Cockcroft-Gault para adultos
                        edad_paciente = int(datos_doc.get('edad_num', 40))
                        sexo_raw = str(datos_doc.get('sexo', 'M')).lower()
                        factor_sexo = 0.85 if ('fem' in sexo_raw or 'mujer' in sexo_raw) else 1.0
                        vfg_dinamico = round(((140 - edad_paciente) * peso_profesional) / (72 * creatinina_profesional) * factor_sexo, 2)
            ...
            if peso_profesional == 0 or creatinina_profesional == 0:
                ...
                riesgo_estado = "Alto Riesgo para la administración de medio de contraste"
                color_indicador = "#FF4B4B" # Rojo
                flecha = "▼"
            elif vfg_dinamico <= 59.0:
                riesgo_estado = "Riesgo Intermedio para la administración de medio de contraste"
                color_indicador = "#FFA500" # Naranja
                flecha = "▼"
            else:
                riesgo_estado = "Sin Riesgo para la administración de medio de contraste"
                color_indicador = "#00D26A" # Verde
                flecha = "▲"

            # 5. Renderizado visual HTML
            st.markdown(f"""
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px; margin-top: 10px; border-top: 1px solid #eee;">
                ...
                <div style="text-align: center;">
                    ...
                    <span style="font-size: 16px; font-weight: bold; color: {color_indicador};">{flecha} {vfg_dinamico:.2f} <span style="font-size: 10px;">ml/min</span></span>
                </div>
            </div>
            ...
            with st.spinner("Estampando firma del profesional y consolidando documento..."):
                try:
                   ...
                   # --- INSTANCIACIÓN DEL MOTOR DE REPORTES ---
                    ...
                    y_fila2 = pdf.get_y()
                    pdf.set_font('Arial', 'B', 9)
                    pdf.cell(32, 5, "Documento / RUT: ", 0, 0) # <- Ajustado
                    ...
                    # 🚀 EXTRACCIÓN CENTRALIZADA Y SEGURA (Inmune a Nulls o Strings inesperados)
                    try:
                    ...
                    try:
                        # Extraemos la edad para la validación pediátrica
                        edad_int = int(st.session_state.get('pdf_edad', 18))
                    except (ValueError, TypeError):
                        edad_int = 18

                    es_pediatrico = (edad_int < 18)
                    
                    # 📊 IMPRESIÓN DE ANTECEDENTES (Se muestran siempre que tengan valores válidos)
                    ...
                    if es_pediatrico:
                        talla_texto = f"{talla_float:.1f} cm" if talla_float > 0 else "__________ cm"
                        pdf.data_field("Talla (Paciente Pediátrico)", talla_texto, h=5)
                    else:
                        peso_texto = f"{peso_float:.1f} kg" if peso_float > 0 else "__________ kg"
                        pdf.data_field("Peso", peso_texto, h=5)
                    
                    # 🔍 SOLUCIÓN DEFINITIVA: Si el VFG existe en pantalla, se calcula y se grafica con color SEA BASAL O CON CONTRASTE
                    if vfg_float > 0:
                        if vfg_float <= 30.0:
                        ...
                        # Acotación de seguridad si el profesional decide calcularlo en un examen basal
                        if not is_contraste:
                            msg_riesgo += " (Calculado preventivamente en examen basal)"

                        # Identificamos el método para el PDF
                        metodo_calculo = "Ecuación Schwartz Bedside 2009" if es_pediatrico else "Ecuación Cockcroft-Gault"

                        # 🔥 INYECTAR COLOR EN EL REPORTE
                        pdf.set_font('Arial', 'B', 9)
                        pdf.set_text_color(50, 50, 50) # Gris para la etiqueta V.F.G
                        pdf.write(5, safe_text(f"V.F.G ({metodo_calculo}): "))
                        
                        pdf.set_font('Arial', 'B', 9)
                        pdf.set_text_color(r, g, b)    # Aplica el color clínico dinámico
                        pdf.write(5, safe_text(f"{vfg_float:.2f} ml/min  ({msg_riesgo})\n"))
                        
                        pdf.set_text_color(0, 0, 0)    # Volver a negro estándar inmediatamente
                    else:
                        # Si el VFG realmente viene vacío o en 0.0 desde la interfaz
                        if is_contraste:
