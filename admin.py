...
# --- BOTÓN DE CIERRE DE CIRCUITO CLÍNICO ---
        ...
        if canvas_profesional is not None and canvas_profesional.json_data is not None and len(canvas_profesional.json_data["objects"]) > 0:
            with st.spinner("Estampando firma del profesional y asegurando registro..."):
                try:
                    ...
                    # Almacenar datos binarios listos en la sesión de Streamlit
                    st.session_state.pdf_bytes_data = pdf.output(dest='S').encode('latin-1')
                    st.session_state.pdf_filename = f"Consentimiento_{paciente_rut}_{datetime.now(tz_chile).strftime('%Y%m%d')}.pdf"
                    st.session_state.pdf_ready = True
                    
                    # Efectos visuales de éxito inmediato
    ...
    if st.session_state.pdf_ready and st.session_state.pdf_bytes_data is not None:
        ...
        st.write(f"El consentimiento de **{st.session_state.paciente_nombre_val}** ha sido visado con ambas firmas electrónicas.")
        ...
        if st.button("🧼 LIMPIAR BANDEJA Y CONTINUAR CON SIGUIENTE PACIENTE", use_container_width=True):
            # Limpiar rastro de la memoria del estado para la siguiente carga limpia
            st.session_state.pdf_ready = False
            st.session_state.pdf_bytes_data = None
            st.session_state.pdf_filename = ""
            st.session_state.paciente_nombre_val = ""
