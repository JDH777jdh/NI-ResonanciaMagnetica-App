...
st.markdown("<style>.main { background-color: #f8f9fa; } h1, h2, h3 { color: #800020; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; } div[st-decorator='true'] { background-color: #800020; } .stButton>button { background-color: #800020; color: white; border-radius: 6px; border: none; padding: 0.5rem 1rem; font-weight: bold; } .stButton>button:hover { background-color: #a31d3d; color: white; }</style>", unsafe_html=True)
...
if not listado_pacientes:
    ...
    # --- BLOQUE DE DOBLE FIRMA (Correctamente indentado dentro del bloque ELSE) ---
    st.divider()
    ...
    with col_f1:
        profesional_nombre = st.text_input("Nombre del Tecnólogo Médico / Profesional:", key="tm_nom")
        profesional_registro = st.text_input("N° Registro Superintendencia de Salud (SIS):", key="tm_sis")
        
        st.markdown("<br>", unsafe_html=True)
        st.warning("⚠️ Al presionar 'Aprobar y Sincronizar', usted certifica que ha evaluado la tasa de filtración glomerular (VFG) y los factores de riesgo del paciente para la ejecución segura del examen.")
    ...
    if st.button("🚀 APROBAR ENCUESTA Y SINCRONIZAR A DRIVE", use_container_width=True):
        if profesional_nombre and profesional_registro and canvas_profesional.image_data is not None:
            with st.spinner("Estampando firmas cruzadas y asegurando expediente en Google Drive..."):
                try:
                    # 1. Procesar la firma del profesional
                    ...
                    # 4. Clonar el diccionario de datos para el reporte integrado
                    datos_completos_pdf = doc_completo.copy()
                    datos_completos_pdf["profesional_nombre"] = profesional_nombre
                    datos_completos_pdf["profesional_registro"] = profesional_registro
                    datos_completos_pdf["fecha_validacion"] = fecha_validacion_str
                    datos_completos_pdf["firma_tm_local"] = ruta_firma_tm_local

                    st.success(f"✅ Paciente {datos_completos_pdf.get('nombre')} visado con éxito. El expediente integrado se ha archivado de forma definitiva.")
                    st.balloons()
                    
                    # Forzar recarga de la bandeja
                    st.rerun()

                except Exception as ex_admin:
                    st.error(f"🚨 Error operativo al cerrar el protocolo: {ex_admin}")
        else:
            st.error("🚨 Campos incompletos. Debe ingresar su Nombre, Registro SIS y dibujar su firma para visar el procedimiento.")