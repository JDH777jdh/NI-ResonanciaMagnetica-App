...
# =============================================================================
# 📄 MÓDULO DE EMISIÓN DE CERTIFICADOS INSTITUCIONALES
# =============================================================================
    ...
    try:
        # Reutilizamos la lógica de rescate de 48 hrs
        docs_ref = db.collection("encuestas").where("estado_validacion", "==", "VALIDADO").stream()
        for doc in docs_ref:
            data = doc.to_dict()
    ...
    if not listado_cert:
        st.info("No hay pacientes validados dentro del umbral de las últimas 48 horas para emitir certificados.")
    else:
        ...
        if paciente_id_cert:
            ...
            tab1, tab2, tab3 = st.tabs([
                ...
                "🕰️ 3. Reingreso Histórico (Próximamente)"
            ])
            ...
            with tab1:
                ...
                hora_llegada = col_h1.time_input("Hora de Llegada (Cita)", value=None)
                hora_salida = col_h2.time_input("Hora de Término (Salida)", value=None)
                
                incluir_acompanante = st.checkbox("Incluir constancia de acompañante")
                nombre_acompanante = ""
                if incluir_acompanante:
                    nombre_acompanante = st.text_input("Nombre completo del acompañante:")
                    
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("📄 GENERAR CERTIFICADO DE ATENCIÓN (PDF)", use_container_width=True, type="primary"):
                    st.info("⏳ Botón conectado. Esperando inyección del motor PDF en la siguiente fase.")

            # PESTAÑA 2: SUGERENCIA
            with tab2:
                ...
                texto_sugerencia = st.text_area(
                    ...
                    placeholder="Ej: Estimado colega, debido a cuadro de claustrofobia severa no fue posible concretar el examen. Se sugiere derivar nuevamente con indicación de premedicación ansiolítica...",
                    height=150
                )
                
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("📄 GENERAR INFORME DE SUGERENCIA (PDF)", use_container_width=True, type="primary"):
                    st.info("⏳ Botón conectado. Esperando inyección del motor PDF en la siguiente fase.")
                    
            # PESTAÑA 3: REINGRESO (INACTIVA)
            with tab3:
                st.markdown("<br>", unsafe_allow_html=True)
                st.info("🛠️ **Módulo en Desarrollo.** Esta sección permitirá cargar y adjuntar consentimientos PDF antiguos firmados en papel o sistemas previos, para mantener la trazabilidad completa en la plataforma.")
