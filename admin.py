...
if not firebase_admin._apps:
    try:
        fb_credentials = {
            ...
            # ESTA LÍNEA ES CRÍTICA: Reemplaza la barra invertida doble por un salto de línea real
        ...
        if canvas_profesional.image_data is not None:
            with st.spinner("Estampando firma del profesional y asegurando registro..."):
                try:
                    ...
                    # Limpieza del residuo temporal local en el servidor
                    try:
                        import os
                    ...
                    # CORREGIDO: Se cambia doc_completo por datos_paciente según tu admin.py original
                    st.success(f"✅ Paciente {datos_paciente.get('nombre', 'Seleccionado')} visado con éxito por TM {profesional_nombre}.")
                    st.balloons()
