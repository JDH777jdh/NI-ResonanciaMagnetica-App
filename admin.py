...
if st.session_state.get("doc_completo") is not None:
    ...
    # --- LOGO DE LA EMPRESA (Asegúrate de tener el archivo logo.png en la raíz o ajusta la ruta) ---
# st.image("logo.png", width=200) 

# --- REORGANIZACIÓN VISUAL DEL PANEL ---
c1, c2 = st.columns([1, 1])

# --- COLUMNA IZQUIERDA: FICHA Y PARÁMETROS ---
with c1:
    st.markdown("#### 👤 Ficha de Atención Clínica")
    
    # Tabla estilo profesional para datos personales
    st.markdown(f"""
    <div style="background-color: #f0f2f6; padding: 15px; border-radius: 10px; border-left: 5px solid #0066cc;">
        <table style='width:100%;'>
            <tr><td><b>Nombre:</b></td><td>{paciente_nombre}</td></tr>
            <tr><td><b>RUT:</b></td><td>{paciente_rut}</td></tr>
            <tr><td><b>Edad:</b></td><td>{paciente_edad} años</td></tr>
            <tr><td><b>Teléfono:</b></td><td>{datos_doc.get('telefono', 'No registrado')}</td></tr>
        </table>
    </div>
    """, unsafe_allow_html=True)

    # Lógica Representante Legal
    if int(paciente_edad) < 18:
        st.markdown("#### 👶 Representante Legal")
        st.info(f"**Nombre:** {datos_doc.get('rep_legal_nombre', 'No registrado')}\n\n**RUT:** {datos_doc.get('rep_legal_rut', 'N/A')}")

    st.markdown("#### 🧪 Parámetros Clínicos")
    # Estructura de tabla para parámetros
    col_p1, col_p2 = st.columns(2)
    with col_p1:
        st.metric("Creatinina", f"{float(creatinina_val):.2f} mg/dL")
    with col_p2:
        st.metric("Peso", f"{float(peso_val):.2f} Kg")
    
    # VFG con lógica de semáforo
    try:
        vfg_val = float(vfg_valor)
        if vfg_val < 30.0:
            st.error(f"🔴 **VFG:** {vfg_val:.2f} mL/min/1.73m² (CONTRAINDICACIÓN CRÍTICA)")
        elif vfg_val < 60.0:
            st.warning(f"🟡 **VFG:** {vfg_val:.2f} mL/min/1.73m² (Riesgo Moderado)")
        else:
            st.success(f"🟢 **VFG:** {vfg_val:.2f} mL/min/1.73m² (Normal)")
    except:
        st.error("⚠️ VFG: No calculable")

# --- COLUMNA DERECHA: BIOSEGURIDAD Y RIESGOS ---
with c2:
    # 1. BIOSEGURIDAD
    with st.expander("🧲 BIOSEGURIDAD MAGNÉTICA", expanded=True):
        col_bio1, col_bio2 = st.columns(2)
        with col_bio1:
            status_marcapasos = "🚨 SÍ" if marcapasos in [True, "Sí", "SI", "si", "Si"] else "✅ No"
            status_implantes = "🚨 SÍ" if implantes in [True, "Sí", "SI", "si", "Si"] else "✅ No"
            st.write(f"**Marcapasos:** {status_marcapasos}")
            st.write(f"**Implantes/Clips:** {status_implantes}")
        with col_bio2:
            st.write("**Comentarios:**")
            st.caption(det_bio if det_bio else "Sin observaciones")

    # 2. FACTORES DE RIESGO
    with st.expander("🚨 FACTORES DE RIESGO CLÍNICOS", expanded=True):
        # Lógica de Alertas Clínicas
        if clin_renal in [True, "Sí", "SI"] or clin_dialisis in [True, "Sí", "SI"]:
            st.error("⚠️ **ALERTA:** Paciente de riesgo para la administración de medio de contraste (Insuficiencia Renal/Diálisis).")
        
        if clin_embarazo in [True, "Sí", "SI"]:
    ...
    try:
        ...
        if ruta_firma_storage:
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as tmp_img:
                ...
                st.image(Image.open(tmp_img.name), width=250)
        else:
            st.caption("No hay firma disponible.")
    except Exception as e:
        st.caption("No se pudo cargar la firma.")
