        ...
        if canvas_profesional is not None and canvas_profesional.json_data is not None and len(canvas_profesional.json_data["objects"]) > 0:
            with st.spinner("Estampando firma del profesional y asegurando registro..."):
                try:
                    ...
                    # 3. Actualizar el documento en Firestore (CORREGIDO APUNTANDO AL ID)
                    fecha_validacion_str = datetime.now(tz_chile).strftime("%d/%m/%Y %H:%M:%S")
                    
                    # CORRECCIÓN AQUÍ: Agregamos .id para pasar el String correcto del documento
                    ...
                    # Mensaje de éxito visual para el Tecnólogo
                    st.success("🎉 ¡Consentimiento Validado con Éxito! El circuito clínico se ha cerrado de forma segura.")
                    st.balloons()
                    st.rerun()
                    
                except Exception as e_error:
                    st.error(f"Error operativo al cerrar el protocolo: {e_error}")
                finally:
                    try:
                        import os
                   ...
                   # =====================================================================
                    # 🛠️ SOLUCIÓN REAL: Se usa la variable correcta 'doc'
                    # =====================================================================
                    st.success(f"✅ Paciente {doc.get('nombre', 'Seleccionado')} visado con éxito por TM {profesional_nombre}.")
                    st.balloons()
