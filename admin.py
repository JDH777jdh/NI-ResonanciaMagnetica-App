...
# === INICIALIZACIÓN INTELIGENTE Y SEGURA DE FIREBASE ADMIN SDK ===
if not firebase_admin._apps:
    try:
        cred_dict = None
        
        # Caso A: Si los secretos usan la nueva estructura JSON blindada
        if "firebase_json" in st.secrets:
            json_texto = st.secrets["firebase_json"]["datos"]
            cred_dict = json.loads(json_texto)
            
        # Caso B: Si los secretos usan la estructura tradicional antigua
        elif "firebase" in st.secrets:
            cred_dict = dict(st.secrets["firebase"])
            if "private_key" in cred_dict and isinstance(cred_dict["private_key"], str):
                cred_dict["private_key"] = cred_dict["private_key"].replace("\\n", "\n")
        
        # Si encontró alguna de las dos configuraciones, procedemos
        if cred_dict:
            # Extraemos el bucket de forma limpia
            ...
            st.error("🚨 No se encontró ninguna configuración de Firebase válida en los Secrets.")
            
    except Exception as e:
        st.error(f"🚨 Error crítico al inicializar Firebase Admin: {e}")
else:
    # Rescate secundario del bucket si la app ya estaba en memoria
    try:
        if "firebase_json" in st.secrets:
            url_bucket = json.loads(st.secrets["firebase_json"]["datos"]).get("bucket_url", "")
        else:
            url_bucket = st.secrets["firebase"].get("bucket_url", "")
    except:
        url_bucket = ""

# Conectores globales de base de datos y archivos
db = firestore.client()
