...
if not firebase_admin._apps:
    try:
        ...
        # 2. LIMPIEZA ULTRAESTRICTA DE LA PRIVATE KEY (Evita errores de copiado/pegado)
        if "private_key" in cred_dict and isinstance(cred_dict["private_key"], str):
            pk = cred_dict["private_key"].strip()
            # Quitamos comillas externas si se pegaron por error
            if pk.startswith('"') and pk.endswith('"'):
                pk = pk[1:-1]
            if pk.startswith("'") and pk.endswith("'"):
                pk = pk[1:-1]
            # Normalizamos los saltos de línea explícitos
            pk = pk.replace("\\n", "\n")
            cred_dict["private_key"] = pk
            
        cred = credentials.Certificate(cred_dict)

        # 3. Inicialización pura
