...
if not firebase_admin._apps:
    try:
        ...
        # 1. Extraemos el bucket_url de forma externa
        ...
        # 2. Inicialización limpia y directa (Igual a app.py, asegurando conexión inmediata)
        firebase_admin.initialize_app(cred)
    except Exception as e:
        st.error(f"🚨 Error crítico al inicializar Firebase Admin: {e}")
else:
    # Si la app ya estaba inicializada, rescatamos el bucket de los secretos de igual forma
    url_bucket = st.secrets["firebase"].get("bucket_url", "")

# 3. Conectores globales (Se le asigna el bucket directamente al cliente de almacenamiento)
db = firestore.client()
