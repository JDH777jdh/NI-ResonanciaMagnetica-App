...
# 1. Definimos el fragmento aislado que se refrescará solo sin recargar toda la página
...
def filtrar_y_sincronizar_pacientes():
    ...
    with col_btn:
        # Botón manual instantáneo por si el paciente acaba de soltar la tablet en recepción
        if st.button("🔄 Actualizar Lista", use_container_width=True, key="btn_manual_refresh"):
            st.rerun()
            
    # Reloj en tiempo real para el control operativo en el servicio
    ...
    try:
        # Consulta exacta basada en tu arquitectura de base de datos
    ...
    # Candado de Enfoque: Evita que el temporizador cambie de paciente al TM mientras escribe
    idx_actual = 0
    if "id_seleccionado_anterior" in st.session_state and st.session_state.id_seleccionado_anterior in options_list:
        idx_actual = options_list.index(st.session_state.id_seleccionado_anterior)

    # Selector reactivo unificado
    paciente_seleccionado = st.selectbox(
        ...
        key="selector_pacientes_async",
        on_change=st.rerun
    )

    # Inyección segura en el Session State global de la aplicación
    st.session_state.id_seleccionado_anterior = paciente_seleccionado
    st.session_state.paciente_seleccionado = paciente_seleccionado

    # Extracción y estructuración asíncrona del documento clínico
    doc_completo = db.collection("encuestas").document(paciente_seleccionado).get().to_dict()
    if doc_completo is None:
        doc_completo = {}
    st.session_state.doc_completo = doc_completo

# 2. Invocamos la función para renderizar la bandeja reactiva
        ...
        if canvas_profesional is not None and canvas_profesional.json_data is not None and len(canvas_profesional.json_data["objects"]) > 0:
            with st.spinner("Estampando firma del profesional y consolidando documento..."):
                try:
                    ...
                    # Líneas y Textos debajo de las firmas
