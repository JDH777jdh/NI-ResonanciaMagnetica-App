...
def filtrar_y_sincronizar_pacientes():
    ...
    with col_btn:
        ...
        if st.button("🧹 Limpiar Lista", use_container_width=True, key="btn_limpiar_lista"):
            ...
            time.sleep(0.5) # <--- AÑADIR UNA PEQUEÑA PAUSA PARA DAR TIEMPO A FIRESTORE
            st.rerun()
...
if datos_doc:
    ...
    with c1:
        ...
        st.markdown("#### 👤 1. Ficha de Atención Clínica")
        ...
        with sub_c1:
            ...
            # LÍNEA MODIFICADA PARA EDAD EXACTA:
            st.write(f"**Edad:**\n{calcular_edad_exacta(datos_doc.get('fecha_nac'))}")
            ...
            with st.expander("🧪 6. EVALUACIÓN DE LA FUNCIÓN RENAL", expanded=True):
               ...
               # --- NUEVO BLOQUE: TABLA DE REFERENCIA VFG ---
                ...
                with col_a:
                    ...
                    medio_contraste = st.selectbox("Medio de Contraste / Medio Físico", ["Ninguno", "Ac. Gadotérico (Clariscan)", "Gadopiclenol (Elucirem)", "Ac. Gadoxético (Primovist)", "Gel de ultrasonido", "Contraste neutro (H2O)", "Suero fisiológico (NaCl 0,9%)"])
                    otros_meds = st.text_input("Otros medicamentos administrados:")
                with col_b:
                    sitio_puncion = st.text_input("Sitio de punción (Ej. Pliegue antebrazo derecho):")
                    via_admin = st.selectbox("Vía de administración", ["Endovenosa", "Oral", "Rectal", "Intraarticular", "No aplica"])
                    cantidad_ml = st.number_input("Cantidad administrada (ml/cc):", min_value=0.0, step=0.5)
