        ...
        if canvas_profesional is not None and canvas_profesional.json_data is not None and len(canvas_profesional.json_data["objects"]) > 0:
            with st.spinner("Estampando firma del profesional y consolidando documento..."):
                try:
                    ...
                    # =====================================================================
                    # Forzamos la salida directamente a una cadena limpia de bytes 'latin-1'
                    try:
                        raw_data = pdf.output(dest='S')
                    except TypeError:
                        raw_data = pdf.output()

                    # Aseguramos la conversión estricta a formato binario reproducible
                    ...
                    # Guardamos los bytes puros e inmutables en el estado de la sesión
