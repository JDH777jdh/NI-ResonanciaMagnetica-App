...
# === INICIALIZACIÓN DE INGENIERÍA AVANZADA: RECONSTRUCTOR DE LLAVE ===
if not firebase_admin._apps:
    try:
        # 1. Cargamos el diccionario original de los secretos
        cred_dict = dict(st.secrets["firebase"])
        
        # 2. Extraemos el bucket
        url_bucket = cred_dict.get("bucket_url", "")
        if "bucket_url" in cred_dict:
            del cred_dict["bucket_url"]

        # 3. RECONSTRUCCIÓN RIGUROSA DE LA PRIVATE KEY
        if "private_key" in cred_dict and isinstance(cred_dict["private_key"], str):
            pk = cred_dict["private_key"]
            
            # Removemos cualquier comilla interna/externa residual
            pk = pk.strip().strip("'").strip('"')
            
            # Forzamos la limpieza eliminando saltos de línea físicos reales que rompen el byte 1625
            lineas_limpias = [linea.strip() for linea in pk.splitlines() if linea.strip()]
            pk_unificada = "\n".join(lineas_limpias)
            
            # Aseguramos el mapeo de los caracteres de escape lógicos
            pk_unificada = pk_unificada.replace("\\n", "\n")
            
            cred_dict["private_key"] = pk_unificada
