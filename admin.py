                ...
                try:
                    ...
                    class PDF_Institucional(FPDF):
                       ...
                       def footer(self):
                            ...
                            # Obtenemos las iniciales del paciente de forma segura
                            iniciales = "".join([p[0].upper() for p in self.p_nombre.split() if p])
        
                            # REPARACIÓN HISTÓRICA: Si p_ip no viene configurado correctamente, usamos un fallback seguro
                        ...
                        def section_title(self, num, title):
                            ...
                            # 🔥 CAMBIO BURDEO: Seteamos color burdeo (128, 0, 32) para el título
                            self.set_text_color(128, 0, 32)
                            self.cell(0, 5, safe_text(f"{num}. {title}"), ln=True)
                            self.ln(1)
                            # 🟢 RESTABLECER: Volvemos a negro absoluto para que el contenido no se pinte de burdeo
                    ...
                    # 🔥 REPARACIÓN PASO 3: Pasamos 'ip_dispositivo' al constructor del PDF
                    ip_cliente = datos_doc.get('ip_dispositivo', 'IP No detectada')
                    fecha_validacion_str = datetime.now(tz_chile).strftime("%d/%m/%Y %H:%M:%S")
                    ...
                    # 🟢 BLINDAJE EXTRA: Adjuntamos el documento completo para que el footer pueda leerlo como plan B
