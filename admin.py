...
if st.session_state.get("doc_completo") is not None:
    ...
    # =========================================================================
    # 🩹 MICRO-CIRUGÍA 1: EXTRACCIÓN Y SANITIZACIÓN DE VARIABLES MAESTRAS
    # =========================================================================
    datos_doc = doc_completo # Clonamos para mantener compatibilidad con tu HTML
    
    # 👤 Demográficos Básicos
    paciente_nombre = datos_doc.get('nombre', 'No registrado')
    paciente_rut = datos_doc.get('rut', 'No registrado')
    paciente_fnac = datos_doc.get('fecha_nac', datos_doc.get('fecha_nacimiento', 'N/A'))
    
    # Sanitizar objeto fecha a string legible si es necesario
    ...
    # 🧲 Bioseguridad (Alineado estrictamente en singular/plural con app.py)
    marcapasos = datos_doc.get('bio_marcapaso', 'No')
    implantes = datos_doc.get('bio_implantes', 'No')
    det_bio = datos_doc.get('bio_detalle', '').strip()

    # 🚨 Triaje de Riesgos Clínicos (Estandarizado con el prefijo clin_ de app.py)
    clin_ayuno = datos_doc.get('clin_ayuno', 'No')
    clin_asma = datos_doc.get('clin_asma', 'No')
    clin_alergico = datos_doc.get('clin_alergico', 'No')
    clin_hiperten = datos_doc.get('clin_hiperten', 'No')
    clin_hipertiroid = datos_doc.get('clin_hipertiroid', 'No')
    clin_diabetes = datos_doc.get('clin_diabetes', 'No')
    clin_metformina = datos_doc.get('clin_metformina', 'No')
    clin_renal = datos_doc.get('clin_renal', 'No')
    clin_dialisis = datos_doc.get('clin_dialisis', 'No')
    clin_embarazo = datos_doc.get('clin_embarazo', 'No')
    clin_lactancia = datos_doc.get('clin_lactancia', 'No')
    clin_claustro = datos_doc.get('clin_claustro', 'No')
    
    # 🧪 Parámetros Métricos
    creatinina_val = datos_doc.get('creatinina', 'N/A')
    peso_val = datos_doc.get('peso', 'N/A')
    vfg_valor = datos_doc.get('vfg', 0.0)
    ...
    with c1:
        ...
        st.write(f"**Nombre:** {paciente_nombre}")
        st.write(f"**RUT:** {paciente_rut}")
        st.write(f"**Edad:** {paciente_edad}")
        st.write(f"**Teléfono:** {datos_doc.get('telefono', 'No registrado')}")
        
        st.markdown("#### 🧪 Parámetros Clínicos")
        st.write(f"**Creatinina:** {creatinina_val} mg/dL")
        st.write(f"**Peso:** {peso_val} Kg")
        
        # Validación y despliegue del semáforo clínico renal (VFG)
        try:
            vfg_analizada = float(vfg_valor)
        except:
            vfg_analizada = 0.0
            
        if vfg_analizada < 30.0:
            st.error(f"⚠️ **VFG:** {vfg_analizada} mL/min/1.73m² (🚨 CONTRAINDICACIÓN CRÍTICA)")
        elif vfg_analizada < 60.0:
            st.warning(f"⚠️ **VFG:** {vfg_analizada} mL/min/1.73m² (Riesgo Moderado)")
        else:
            st.success(f"✅ **VFG:** {vfg_analizada} mL/min/1.73m² (Función Renal Normal)")

    with c2:
        # --- 1. TARJETA DE INFORMACIÓN DE FILTRADO Y EXAMEN ---
        st.markdown(f"""
        <div style="background-color: #f9f9f9; padding: 15px; border-radius: 8px; border-left: 5px solid #800020; margin-bottom: 20px;">
            <h3 style='margin-top:0; color:#800020; font-family:Arial;'>📋 Ficha de Atención Clínica</h3>
            <table style='width:100%; font-family:Arial; font-size:14px; border-collapse: collapse;'>
                <tr>
                    <td style='padding: 4px 0;'><b>Nombre Completo:</b> {paciente_nombre}</td>
                    <td style='padding: 4px 0;'><b>RUT:</b> {paciente_rut}</td>
                </tr>
                <tr>
                    <td style='padding: 4px 0;'><b>Edad:</b> {paciente_edad} ({paciente_fnac})</td>
                    <td style='padding: 4px 0;'><b>Email:</b> {datos_doc.get('email', 'N/A')}</td>
                </tr>
                <tr>
                    <td style='padding: 4px 0;'><b>Procedimiento Solicitado:</b> <span style='color:#0066cc; font-weight:bold;'>{procedimiento_val_visual}</span></td>
                    <td style='padding: 4px 0;'><b>Uso de Contraste:</b> {"🔴 SÍ (Requiere Filtro Renal)" if is_contraste_visual else "🟢 NO (Estudio Basal)"}</td>
                </tr>
            </table>
        </div>
        """, unsafe_allow_html=True)
    
        # --- 2. CONTROL COMPLETO DE BIOSEGURIDAD MAGNÉTICA (ST EXPANDER) ---
        with st.expander("🧲 BIOSEGURIDAD MAGNÉTICA", expanded=True):
            col_bio1, col_bio2 = st.columns(2)
            
            with col_bio1:
                # CORRECCIÓN DE LLAVES: Usando las variables sanitizadas del Segmento 1
                status_marcapasos = "🚨 <span style='color:red; font-weight:bold;'>SÍ CONTRAINDICADO</span>" if marcapasos in [True, "Sí", "SI", "si", "Si"] else "✅ <span style='color:green;'>No registra</span>"
                status_implantes = "🚨 <span style='color:darkorange; font-weight:bold;'>SÍ (Evaluar Compatibilidad)</span>" if implantes in [True, "Sí", "SI", "si", "Si"] else "✅ <span style='color:green;'>No registra</span>"
                
                st.markdown(f"**¿Posee Marcapasos Cardíaco?:** {status_marcapasos}", unsafe_allow_html=True)
                st.markdown(f"**¿Posee Implantes / Prótesis / Clips?:** {status_implantes}", unsafe_allow_html=True)
            
            with col_bio2:
                st.markdown("**Detalle de Bioseguridad / Comentarios:**")
                if det_bio and det_bio.lower() != 'sin observaciones':
                    st.warning(f"💬 {det_bio}")
                else:
                    st.caption("🟢 Sin observaciones particulares declaradas.")

        # --- 3. REINICIO PREVENTIVO DEL PESO FANTASMA ---
        if not is_contraste_visual:
            if 'peso' in st.session_state:
                st.session_state['peso'] = 0.0
                
        # --- 4. TRIAJE DE FACTORES DE RIESGO CLÍNICOS ---
        st.markdown("#### 🚨 Triaje de Factores de Riesgo")
        st.write(f"**¿Alergias?:** {clin_alergico}")
        st.write(f"**¿Asma?:** {clin_asma}")
        st.write(f"**¿Diabetes?:** {clin_diabetes} | **Toma Metformina:** {clin_metformina}")
        st.write(f"**¿Insuficiencia Renal?:** {clin_renal} | **Diálisis:** {clin_dialisis}")
        st.write(f"**¿Embarazo?:** {clin_embarazo} | **Lactancia:** {clin_lactancia}")
