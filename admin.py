...
def filtrar_y_sincronizar_pacientes():
    ...
    with col_btn:
        ...
        # Cambiamos el nombre para que quede claro que limpia el historial antiguo
        if st.button("🧹 Limpiar Bandeja", use_container_width=True, help="Elimina pacientes ya validados del historial", key="btn_limpiar_lista"):
            # ... (mismo código de borrado)

    # 2. Busca debajo de donde se muestra el "Procedimiento Asignado" (aprox. línea 455). Agrega esto:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("🗑️ Eliminar Paciente de la Bandeja", use_container_width=True):
            if paciente_seleccionado:
                # Borra específicamente este documento PENDIENTE
...
if datos_doc:
    ...
    with c1:
        ...
        with st.expander("👤 1. FICHA DE ATENCIÓN CLÍNICA", expanded=True):
        ...
        with sub_c1:
            # 1. Procedencia en mayúscula (Requerimiento Paso 1)
            procedencia = datos_doc.get('procedencia', 'AMBULATORIO')
            st.write(f"**Procedencia:** {procedencia.upper()}")
            
            # 2. Nombre
            st.write(f"**Nombre:** {datos_doc.get('nombre', 'N/A')}")
            
            # 3. Edad con formato completo (Año, Mes y Día)
            st.write(f"**Edad:** {calcular_edad_exacta(datos_doc.get('fecha_nac'))}")
            
            # 4. Sexo / Género
            st.write(f"**Sexo / Género:** {sexo_paciente}")
