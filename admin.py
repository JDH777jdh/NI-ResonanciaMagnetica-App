...
# 2. LA SOLUCIÓN: Agrega esta línea justo aquí
# Esto convierte el None en un diccionario vacío {} si no hay paciente seleccionado.
datos_doc = st.session_state.get('doc_completo') or {}

# 3. Solo si hay datos (el diccionario no está vacío), dibujamos la interfaz
if datos_doc:
    c1, c2 = st.columns([1, 1])
    with c1:
        st.markdown("#### 👤 Ficha de Atención Clínica")
        # Ahora el .get('nombre') no fallará porque datos_doc siempre es un diccionario
        st.write(f"**Nombre:**\n{datos_doc.get('nombre', 'N/A')}")
        # ... resto de tu código
else:
    # Mensaje de guía para cuando hay lista, pero no has clickeado a nadie
    st.info("👈 Por favor, selecciona un paciente de la lista de la izquierda para ver su ficha.")
...
with c2:
    ...
    with st.expander("🚨 FACTORES DE RIESGO CLÍNICOS", expanded=True):
        ...
        data_riesgos = {
            "Factor": ["Alergias", "Asma", "Diabetes", "Metformina", "Lactancia"],
            "Estado": [
