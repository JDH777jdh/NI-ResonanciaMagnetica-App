        ...
        if canvas_profesional is not None and canvas_profesional.json_data is not None and len(canvas_profesional.json_data["objects"]) > 0:
            with st.spinner("Estampando firma del profesional y consolidando documento..."):
                try:
                    ...
                    st.session_state.pdf_filename = f"REG-VALIDADO_{paciente_nombre_val.replace(' ', '-').upper()}_{paciente_rut}_{['', 'ENERO', 'FEBRERO', 'MARZO', 'ABRIL', 'MAYO', 'JUNIO', 'JULIO', 'AGOSTO', 'SEPTIEMBRE', 'OCTUBRE', 'NOVIEMBRE', 'DICIEMBRE'][datetime.now(tz_chile).month]}_{datetime.now(tz_chile).strftime('%Y')}.pdf"
